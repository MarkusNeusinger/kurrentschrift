...
def build_payload(
    ...
    for index, row in enumerate(layout["rows"]):
        ...
        x1_px = _px(layout["page_mm"]["width"] - 15.0 + ROW_PAD_MM)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"wrote {import_dir / 'payload.json'}: {len(payload['rows'])} rows, {flagged} with QC flags (~{dpi:.0f} DPI)")
