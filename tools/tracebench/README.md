...
| `view.py` | The duel: one self-contained HTML page per round — every method's trace over the crop beside the hand reference, toggleable, with the attached `--json` numbers, a writing-order animation (`stroke-dashoffset`, constant pen speed, pen lifts as pauses), and per word the residual profile: each candidate's distance to the hand along the headline's own DTW pairing (`DtwResult.pairs`), plotted over the hand's body arc with a hover probe onto the word image. Geometry through `BenchFrame`, bytes deterministic. |
