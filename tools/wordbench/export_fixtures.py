...
The MEASURED joins of the same specimens (`pair_instances`, filled by
tools/pairlab/harvest.py from THESE fixture cases, so the slot spaces coincide)
freeze alongside as ``pair_instances.json`` — the reference for the bench's
report-only "gemessen vs. komponiert" columns. It is frozen like every other
artifact: re-exporting it is a deliberate re-baseline of those columns.
``--only pair-instances`` writes JUST that file into the EXISTING fixture
...
# The measured-join artifact stays LEAN: only the QC flags a consumer needs to
# filter on travel with the geometry (the full dissection QC lives in the DB).
PAIR_MEASUREMENT_KEYS = ("fit_ok", "gen_chamfer")


def pair_instances_payload(rows: list[dict], kinds: set[str], specimen_ids: set[str]) -> dict:
    """The frozen measured joins of ONE fixture set (handmodell H2).

    Filters the source's `pair_instances` rows down to this set — matching the
    set's ``kind``(s) AND a specimen id the set actually froze, because a
    custom set (the abb22 Schülerschrift) shares the ``word`` kind with the
    default set and must never leak into it. ``hand_id`` reports the modal
    hand of the selected rows: statistics are a property of the writer, so a
    consumer can see at a glance which hand the reference belongs to.
    """
    selected = sorted(
        (r for r in rows if r["kind"] in kinds and r["specimen_id"] in specimen_ids),
        key=lambda r: (r["specimen_id"], r["slot"]),
    )
    hands = Counter(r["hand_id"] for r in selected if r.get("hand_id"))
    return {
        "hand_id": hands.most_common(1)[0][0] if hands else None,
        "rows": [
            {
                "left_key": r["left_key"],
                ...
                "geometry": r["geometry"],
                "measurements": {
                    k: (r.get("measurements") or {})[k]
                    for k in PAIR_MEASUREMENT_KEYS
                    if k in (r.get("measurements") or {})
                },
            }
            for r in selected
        ],
...
async def export(source_id: str, out_dir: Path, which: str, only: str | None = None) -> None:
    ...
    async with get_db_context() as session:
        ...
        # The measured joins of this source (handmodell H2) — read-only, like
        # everything else here; filtered per set on write.
        pair_rows = [
            {
                "left_key": p.left_key,
...
def main() -> None:
    ...
    asyncio.run(export(args.source, args.out, args.which, only=args.only))
