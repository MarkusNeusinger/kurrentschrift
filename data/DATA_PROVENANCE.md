...
| `humanbench` | [`humanbench/`](humanbench/SOURCE.md) | Menschliche Bewertungsdurchgänge über die gefitteten Vorkommen (Runden 01/02), über zwei Kompositionen desselben Wortes (Runden 05/06/07/08/10) und über zwei gefolgte BAHNEN desselben Wortes (Runde 09 — dieselbe Ablage, aber die Genauigkeitsfrage statt der Echtheitsfrage) — je Runde der Ergebnistext (`runde-<nn>-urteile.txt`), ein schmaler Schlüssel (`runde-<nn>-vorkommen.json`: uid → Glyph bzw. Wort, Vorlagenwort, Verdachtsklasse, `repeat_of` — ohne den wäre eine Ergebniszeile wie `S026:AW#81,76` unlesbar), der Provenienz-Stempel (`runde-<nn>-stempel.md`) und, wo die Auswertung aus dem Werkzeug kommt, deren Zahlen (`runde-<nn>-auswertung.json`: Zählungen und Anteile je Runde); dazu, wo eine Runde über eine MEHRZEILIGE Laufform-Karte lief, ihre Zerlegung je Kartenzeile (`runde-<nn>-zeilen.json`, bisher nur Runde 08 — kein Werkzeug-Ausgang, sondern im Ablage-PR gerechnet, mit den SHA-256 seiner privaten Eingänge in der Datei; eine Ein-Zeilen-Runde wie 10 braucht keine, dort IST die Klassentabelle der Auswertung die Zerlegung); enthält keine Geometrie und keine Kennzahlen je Vorkommen, voller Schlüssel, Payload, Arm-Dateien und Kandidaten-Karte bleiben außerhalb des Repos | eigenes Urheberrecht des Projektautors (wie `/data/samples/own-hand`) | eigene Erhebung; beurteilt wurden Ausschnitte aus `sources/suetterlin-1922` |
