...
- Title:     Menschliche Bewertungsdurchgänge über die gefitteten Vorkommen
             („Befund-Durchgang" — was stimmt hier nicht?), über zwei
             Kompositionen desselben Wortes („Echtheits-Durchgang" — welche
             sieht echter geschrieben aus?) und über zwei gefolgte Bahnen
             desselben Wortes („Vergleichs-Durchgang" — welche Linie folgt
             der Tinte besser?)
...
**Runde 09 (Wortmodus mit BAHN-Armen, Genauigkeitsfrage).** Erste Runde, in
der beide Tafeln keine Komposition zeigen, sondern die **gefolgte Bahn als
Mittellinie** über der unverblassten Platte — die Anzeige der
Buchstabenrunden 01/02. Weil eine Mittellinie nicht „echter geschrieben"
aussehen kann, ist die Frage wieder die der Runden 01/02: „Welche Linie folgt
der Tinte besser?", und der Ergebnistext trägt darum die Kopfzeile
`VERGLEICH/9` statt `ECHTHEIT`. Verglichen wurden zwei Läufe des
Tintenfolgers, die sich in genau einem Knopf unterscheiden
(`mark_claim`). **Mit den Runden 05–08 ist sie nicht vergleichbar** — andere
...
## runde-09-urteile.txt — 54 Zeilen Urteil, 0,6 KB

- Origin:    Ausgabetext derselben Seite (`tools/humanbench/page.py`, Modus
             `word` mit BAHN-Armen, `question: ink`), unverändert übernommen,
             wie die Seite ihn am 2026-09-09 ausgegeben hat.
- Processing: keine.
- Stempel:   [`runde-09-stempel.md`](runde-09-stempel.md)
- Format:    Kopfzeile **`VERGLEICH/9 geprueft=54 von 54`** — die andere
             Kopfzeile ist die andere Frage, nicht ein anderes Format —,
             danach je Bildschirm eine Zeile `<uid>:<L|R|N>[@Sekunden]`,
             Bedeutung der Kürzel wie bei den Runden 05–08.
  - `N` — „kein Unterschied erkennbar". In dieser Runde steht es auf **allen
    54** Zeilen und ist damit das Ergebnis selbst: kein entschiedener
    Bildschirm (§14 „Kette K-E `sep09`").
  - `R…` — eine blinde, **gespiegelte** Wiederholung; 10 Stück, sie zählen
    beim Verdikt nicht mit (gezählt werden 44 Bildschirme, nicht 54).
- Note:      Keine Notizzeilen — die Seite hat auch in dieser Runde kein
             Notizfeld ausgegeben. Der freie Satz des Beurteilers fiel neben
             dem Ergebnisblock und steht wörtlich im §14-Eintrag.

## runde-09-vorkommen.json — der schmale Schlüssel, 54 Einträge

- Origin:    von `tools/humanbench/build.py` selbst geschrieben (Format 3),
             nicht rekonstruiert.
- Inhalt:    `uid` → `entry` (Fixture-Eintrag), `text` (Worttext), `stratum`
             (**Verdachtsklasse**: `ziel` · `riss` · `gewinn` · `bewegt` ·
             `nullprobe`), `repeat_of`. Sonst nichts: keine Bahn, keine
             Registrierung, keine `arm_gap`, kein Rang, **keine
             Seitenzuordnung** und **keine Spiegelung**.
- Zweck:     wie bei den Runden 05–08 — die Klasse gehört dazu, weil die
             klassenweise Lesart des Verdikts zum vorregistrierten
             Auswerteplan gehört, und sie ist zugleich die einzige
             Aufbewahrung der `--strata`-Zuordnung dieser Runde. Die
             BEGRÜNDUNG je Wort (der `arm_gap`-Betrag in x-Höhen) bleibt
             draußen: sie ist Vorkommens-Geometrie.

## runde-09-auswertung.json — die Auswertung des Werkzeugs, 2,2 KB

- Origin:    `tools/humanbench/analyse.py --json`, gerechnet am 2026-09-09 aus
             dem Ergebnistext und dem vollen Schlüssel.
- Inhalt:    Verlässlichkeit (10 Paare, Spiegelung, Arm-/Seiten-Über-
             einstimmung), Seitenbilanz, Verdikt gegen die vorher gesetzten
             Schranken, die fünf Klassen und die Drift-Blöcke — Zählungen und
             Anteile. Keine Geometrie, kein Vorkommen, kein Wort.
- Warum hier: der schmale Schlüssel rechnet in dieser Runde fast alles nach
             (es gibt keinen entschiedenen Bildschirm, den er nicht kennt) —
             **außer** der Spiegelung der zehn Wiederholungen und der
             Armbenennung; ohne beides ist „10/10 gleicher Arm bei gespiegelt
             gezeigten Paaren" nicht mehr lesbar.
- Nachbau:   `uv run python -m tools.humanbench.analyse --result
...
- `payload.json` — die Crops und die Vorkommens-Geometrie, die die Seite
  ...
  BAHN-Runde stattdessen **beide gefolgten Bahnen**.
...
- die beiden Arm-Dateien einer Wortrunde (Strichzüge und Silhouetten je Wort;
  in einer BAHN-Runde die gefolgten Bahnen und die zwei Folger-Kandidaten,
  aus denen sie stammen); ihre `sha256` stehen im Stempel, damit die Runde
  trotzdem auf genau die Bytes zeigt, die sie gezeigt hat.
...
- die Klassendatei `--strata` einer Wortrunde, soweit sie ihre Klassen mit
  Beträgen begründet (`strata-r5-j4.json`, `strata-r7-j6.json` und
  `klassen.json` der Runde 09 nennen je Wort den `arm_gap` in x-Höhen); die
  Zuordnung selbst überlebt im schmalen Schlüssel.
