...
async def require_db(db: AsyncSession | None = Depends(get_db)) -> AsyncSession:
    if not is_db_configured() or db is None:
        raise HTTPException(503, detail="Database not configured. Set DATABASE_URL or INSTANCE_CONNECTION_NAME.")
    return db
...
async def require_source(source_id: str, db: AsyncSession = Depends(require_db)) -> Source:
    ...
    if source is None:
        raise HTTPException(404, detail=f"source {source_id!r} not found")
    return source
