...
# Anchor-count bound shared by the bbox + trace + resample bodies. Below 4 the
# resampler breaks (single-sample linspace / negative counts); far above it the
# anchor JSONB and the SVG renderers blow up. 1000 is generous headroom over the
# ~50 norm (the measured jitter knee, core.pipeline.DEFAULT_N_ANCHORS). None ==
# "unset": the bbox PUT preserves the stored count instead of rewriting it on
# every edit; trace/resample fall back to the stored bbox value.
NAnchors = Annotated[int | None, Field(default=None, ge=4, le=1000)]
...
class QuizWordOut(BaseModel):
    ...
    # The word bank tags each entry as present-day or historical vocabulary; the
    # quiz reveal uses it to gloss dated words (see `note`).
...
class BboxIn(BaseModel):
    ...
    baseline_y: int
    midband_y: int
    # Optional like `guides`/`locked`: an omitted value preserves the stored
    # count instead of silently rewriting it on every bbox edit. See NAnchors.
    n_anchors: NAnchors
...
class TraceRequest(BaseModel):
    ...
    # None falls back to the stored bbox value. See NAnchors.
    n_anchors: NAnchors
    variant: int = 0
