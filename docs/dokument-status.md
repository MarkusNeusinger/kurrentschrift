...
| [`reference/kurzglossar.md`](reference/kurzglossar.md) | einem Begriff, der die Zwei-von-drei-Schwelle des Kurzglossar-Kopfes neu erreicht (Code · Agenten-Dateien · letzte 40 PR-Beschreibungen) und in keine seiner drei Ausschluss-Klassen fällt — Kurzeintrag ergänzen; fällt ein Begriff unter die Schwelle, Kurzeintrag entfernen. Der volle Eintrag bleibt in `glossar.md` |
...
| [`reference/qualitaetsmetrik.md`](reference/qualitaetsmetrik.md) | jeder Änderung an `core/quality.py`, `core/quality_suetterlin.py`, `core/geometry.py`, `core/word_metric.py`, `core/quality_localize.py` (baut §5 nach, zieht bei dessen Re-Baseline mit) und jedem Re-Baseline der eingefrorenen Fixtures; die aktuellen Headlines im Status-Blockquote bleiben hier, das Journal der Läufe steht seit 2026-09-04 in `reference/messjournal.md` |
...
| [`reference/verfahren.md`](reference/verfahren.md) samt den sechs Verfahrensseiten (`verfahren-kette.md` · `verfahren-lotse.md` · `verfahren-inksight.md` · `verfahren-nullprobe.md` · `verfahren-uebergaenge.md` · seit 2026-09-11 `verfahren-tintenpfad.md`) | jedem §14-Eintrag, der einen Arm oder eine Stufe eines Duell-Verfahrens misst (adoptiert oder verworfen) — seit dem Autor-Entscheid A43 vom 2026-09-09 auch jedem Eintrag der Route „Übergänge“, die selbst nicht im Duell antritt, und seit 2026-09-11 der Route „Tintenpfad“ (seit demselben Tag in `ROUTE_PAGES` von `tools.docs_register`). Ledger-Zeile im selben PR; bei Adoption zusätzlich „Aktueller Stand“ der betroffenen Seite und ihr Stand in der Übersicht — für die fünf Duell-Routen die Stand-/„seit“-Spalte der Tabelle, für die Übergänge der Absatz „Übergänge — keine Duell-Route, trotzdem ein Ledger“ daneben (diese Route hat keine Tabellenzeile). Erzwungen von `tools.docs_register check` (CI-Job „Docs-Register“) |
