...
| Lotse v0.13 Stufe 0,8 (`aug19`) | das weite Entdrillungs-Fenster tötet auch GENUIN nahe echte Paare (mits t-Doppel 0,07 xh) — Geometrie allein kann Gewebe-Duplikat und echtes enges Doppel nicht trennen | **soll-budgetierte Entdrillung**: entdrillt wird nur, wo der Kandidat MEHR Kreuzungen trägt, als die Karte in der Nachbarschaft vorschreibt (das Soll kennt mits Doppel, die Gewebe nicht) | eigene Pre-Reg |
| Lotse v0.14 „all"+Entdrillung (`aug19`) | Tinten-Gewinne real (aiou +0,012, G erstmals fast hand-gleich geritten — Sichtbeweis), aber die Struktur kippt in GENAU den zwei schlimmsten Karten-Form-Regionen (G-Kopf-X stirbt an der formfremden G-Karte, p erfindet eines) — Netto 8 > 6 | **Wiedervorlage NACH den Laufform-ARMEN** (LF1 Lücken-Schluss G/W/k + LF2 p-Topologie — exakt die Gate-Brecher; §14-Nachträge `aug19`: KEIN Autorenschritt mehr; §7.7-Nachkalibrierung misst neu) — die Laufform-Arme zahlen doppelt; trocken mit injizierter Kandidaten-Karte vormessbar | LF1/LF2, dann Re-Messung |
| Lotse v0.15 soll-budgetierte Entdrillung (`aug19`) | das Budget erbt die Karten-Platzierungsfehler (unters e→r-Soll liegt neben der Tinte → echtes Paar stirbt trotz Budget) und die Radius-Zählung vetiert wills Fix (benachbartes echtes X in der Gewebe-Nachbarschaft) — dritte unabhängige Bestätigung der Karten-Form-Decke | Wiedervorlage GEMEINSAM mit v0.14 nach den Laufform-Armen (LF1/LF2), dann Positions- statt Zähl-Matching gegen die vertrauenswürdige Karte | LF1/LF2, dann eigene Pre-Reg |
