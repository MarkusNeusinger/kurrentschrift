...
| **Kette R3 Zwei-Züge-Modell** (`sep07`, Rettungsweg R3 der Kringel-Diagnose, Autor-Entscheid „2 ja") | **Der Mechanismus trifft sein Ziel und fällt an der Ausführung.** Die Feder herausrechnen statt den Klumpen skelettieren: keine Stützstelle näher als `w_pen` = 0,0968 an einer `offen`-Binnenfläche des Katalogs. Ergebnis: **106 von 106 korrigierten Schleifen offen bei 0,097** (vorher 101/106), Δ zur Platten-Erwartung in 85/106 innerhalb ±0,02 xh; `a`#0 0,2046 → 0,2751, `o`#0 0,1741 → 0,2535, `sz`#0 0,2231 → 0,3063. Alle Distanzmaße besser: dev-19 dtw-Median −0,000701 bei **14 : 3** (p = 0,0127), aiou 0,7583 → 0,7601, Chamfer beidseitig, 63er-k0-Soll 80 → 78. Verworfen an **Gate (b)** (`cross_missing` 12 → 13, `cross_spurious` 7 → 9 — **der ganze Betrag auf `will` und `Galoppieren`**, 17 der 19 dev-Wörter bewegen keinen Zähler) und **Gate (d)** (1 588 neue `kink`-Ereignisse, p90-Δ +16,74°). **Die Ursache ist gemessen und ist eine Länge:** die Stützstellen stehen 0,0265 xh auseinander, die deklarierte Anti-Raster-Glättung 0,0363 umfasst 1,37 davon und die C¹-Blende 0,0725 nur 2,74 — beide Fenster sind in Federbreiten gedacht und in Stützstellen zu kurz, also landet das Pixelraster der Platte (±0,5 px ≈ 0,015 xh) ungefiltert in der Federbahn | (1) **R3b — die Blende ans Lesefenster** (in derselben Runde vorregistriert und gefahren): Blenden-Maßstab von einer halben auf zwei Federn, Glättung damit auf eine Feder = das Fenster, mit dem der Sensor liest; abgeleitet, nicht gewählt, und mit vorab benanntem Preis an Gate (a); (2) **die enge Klasse** — nur die Schleifen korrigieren, deren Wort keinen Strukturzähler bewegt (`will` und `Galoppieren` sind zwei von 19; Asymmetrie-Regel `aug26`); (3) **der Öffnungs-Term im SOLVE statt danach** — jetzt belegt statt vermutet: eine punktweise Nachbearbeitung kann per Konstruktion nicht glatter sein als ihr Raster, ein Constraint im Folger-Solve schon (R4s Warnung „erst der Sensor" ist mit #556/#558 erfüllt); (4) **die Korrektur auf die EVIDENZ statt auf die Ausgabe** — dieselbe Entfaltung auf das Skelett gelegt, das der Lotse reitet und an dem sich die Kette misst (R4, Mittelachsen-Term), mit den 106 Zahlen dieses Arms als Eichung. Ausdrücklich KEIN Weg: die Feder dünner rechnen, ein Gate lockern, oder den Schub kappen, bis die Zähler wieder stimmen | (1) **gefahren `sep07` und ausgemessen**: Knicke 1 588 → 465, p90 +16,74° → +4,81°, `cross_spurious` zurück auf 7 — aber Gate (a) 85/106 → 62/110 und k0-Soll 78 → 82, also auch nicht adoptiert. **Der Weg ist damit geschlossen und trägt (3):** beides hängt an derselben Länge; (2) eigene Pre-Reg, jetzt auf EIN Wort verengt (`Galoppieren`); (3) **eigene Pre-Reg, der aussichtsreichste** — von R3b belegt, nicht mehr nur vermutet; (4) eigene Pre-Reg, nach (3) |
