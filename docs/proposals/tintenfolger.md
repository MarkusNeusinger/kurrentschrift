...
| K-D Tinten-Korridor (`aug21`) | GEGENSTANDSLOS NACH v4 — das vorregistrierte Exkursions-Inventar (Sprosse 0, ohne Solve) findet auf v4-Basis und v5-Anwärter KEIN Wort über 0,35 xh Papier-Exkursion (Set-Maximum zum 0,33; die aug20-Nadel-Klasse lag bei 0,5–0,83 xh): K-C hat die Wurzel (Fremdtinten-Magneten) geheilt, bevor das Symptom-Verbot gebaut war; kein Negativ der Mechanik | Wiedervorlage-Auslöser: ein künftiges Inventar (`kd_inventory`-Sensor, Minuten je Kandidat) oder ein neuer Arm zeigt eine neue Papier-Nadel-Klasse → Barriere mit frischer Pre-Reg, unter-Risiko unverändert benannt | Sensor steht; kein Anlauf ohne neue Klasse |
