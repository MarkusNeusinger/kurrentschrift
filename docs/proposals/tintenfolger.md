...
| **Rausch-Boden des Folgers** (sperrt jeden weiteren Init-Arm) | §14 „Kette K-F `sep04`", jetzt auch „Kette K-G `sep09`" | K-F hat unfreiwillig eine Nullprobe gefahren: 23 Wörter bekamen einen Init, der um höchstens 1,8·10⁻¹⁵ abwich, und trotzdem kippten 9 Wächter-Verdikte mit aiou −0,0298 … +0,0800 und 4 Verlierern unter der 0,003-Schranke. Solange dieser Boden nicht beziffert ist, kann kein Init- oder Startpunkt-Arm WORTWEISE entschieden werden. **K-G hat die Sperre `sep09` gegen sich selbst geprüft und die Auflage gefahren:** seine Störung liegt bei 0,047–0,615 xh und damit fünfzehn Größenordnungen über dem K-F-Rauschen, also ist der Arm als SET-Aussage entscheidbar — die Wortaussage bleibt gesperrt, und die Runde hat kein Wort einzeln adoptiert oder verworfen | offen und weiterhin fällig für WORTWEISE Aussagen: den Boden mit einer erklärten Null-Störung messen und veröffentlichen; danach (a) naht-verankertes Abspielen (`prodconn.replay` mit dem Versatz auf den Körper-Endpunkt), (b) das Wächter-Verdikt als Messgröße statt Etikett — K-G liefert dafür die erste Eichung (die Wörter, die auf `revert-init` wechseln, verlieren im Median 0,094 aiou, die, die es verlassen, gewinnen 0,102), (c) die Wiedervorlage von K-F; je eigene Pre-Reg |
| **Bedingte Saat-Registrierung** (Konversion 1+2 von K-G) | §14 „Kette K-G `sep09`", §7.9 | `--chain-seed grid` heilt, wo die Saat wirklich danebenliegt (`unter` +0,1072 aiou und 5 → 0 Papier-Umkehren), und schadet, wo sie es nicht tut (unterstes Spannweiten-Terzil 5 → 8) — plus ein Sonderfall: `regieren` startet gemischt, weil sein drittes `e` an der Blockschranke liegt und der Schalter solche Slots auslässt | offen, frische Pre-Reg: (1) eine vorab benannte Schwelle, unter der ein Slot KEINEN Seed bekommt; (2) den Slot an der Schranke klemmen statt auslassen. Beide sind eine andere AUSWAHL desselben Knopfs, also je ein neuer Mechanismus — kein weicheres Gate |
...
| **Die `d`-Zeile als eigener Laufform-Arm** | §14 „Laufform LF16 `sep08`" (Rettungsweg 1) | Runde 8 hat die 15-Zeilen-Karte als GANZE verworfen, ihre Zerlegung aber genau eine Zeile gefunden, die beide vorregistrierten Schranken räumt: `d` mit 10 : 2 von den entschiedenen Wörtern (83,3 %) bei 14,3 % Unentschieden — und ohne die vier `und…`-Wörter, wo `d` auf `u` trifft, sogar 10 : 0 ohne einen einzigen Tie. Der Beurteiler hat sie von sich aus genannt („also das d sieht definitiv besser aus"). Die Zahl ist bedingt: alle 15 Zeilen liefen zugleich | offen, eigene Pre-Reg — und ZUERST eine frische Ernte auf der heutigen Wurzel `ccb036a5eb20…` (die geurteilte Karte stammt von `eaa195aa7c84…`, §14 „Komma-Ausschluss `sep07`"); dann ein Overlay aus genau dieser Zeile und eine kurze Runde über die 14 Wörter, die sie zeichnen. Die Befehle stehen im §14-Eintrag. `n` und `h` bleiben mit 20,0 % bzw. **0 von 8** ausdrücklich zu |
