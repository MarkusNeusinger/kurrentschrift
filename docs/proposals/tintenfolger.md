...
| Lotse v0.13 Stufe 0,8 (`aug19`) | das weite Entdrillungs-Fenster tötet auch GENUIN nahe echte Paare (mits t-Doppel 0,07 xh) — Geometrie allein kann Gewebe-Duplikat und echtes enges Doppel nicht trennen. **Autopsie-Nachmessung `aug20`:** auch MIT Lineal-Soll-Budget bleibt 0,8 tot — Galoppieren würde komplett heilen (Netto 2 → 0), aber unter verliert ALLE drei X, weil die Karte unters Kreuzungs-ORTE nicht kennt (vierte Platzierungs-Bestätigung); der Punkt-Abstand trennt die Klassen nicht (0,27–0,29 gegen 0,17–0,32 xh, überlappend) | **präzisiert `aug20` (Karten-Soll-Autopsie):** die Platzierungskarte matcht 40/41 Hand-X (median 0,150 xh) — der 0,8-Blocker ist Soll-VOLLSTÄNDIGKEIT, nicht Platzierung (unters u-n-Doppel: Hand 2 X, Karte 1; jedes Zähl-Veto fällt am 12-Events-über-1-Soll-Cluster als Commons-Problem; das Reservierungs-Veto v0.17 rettet ein X, das ungedeckte fällt weiter) → erst der **Karten-Soll-Vollständigkeits-Arm** (Composer: Join-Schleifen-Kreuzungen, „Hand 34 > Komposition 25"), dann das Fenster wiedervorlegen | Composer-Arm zuerst |
