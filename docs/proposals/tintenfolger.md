...
| **Runde 10 urteilen — und danach der Write der EINEN `d`-Zeile** | §14 „Laufform LF17 `sep09`" (Autor-Entscheid A40) | der Arm steht, aber ein Laufform-Write ist ein Autorenvorgang und eine deklarierte Re-Baseline; und die Runde kann sich per Konstruktion keine Adoption selbst erteilen (26 bit-identische Nullproben sind ein Tie-Boden von 65 %, nötig für die blinden Wiederholungen) | offen: ~7 min Urteilszeit, 48 Bildschirme, `temp/runden-sep09/humanbench/runde-10-d-zeile.html`. Ein Freiheitsgrad (`d`), Basis-Tafeln byte-gleich mit Runde 8. Das Lineal ist mit 0,108153 → 0,108339 leicht gegen den Kandidaten, der Median-`arm_gap` liegt mit 0,1030 xh dicht an dem Versatz, bei dem Runde 5 entschieden hat. Räumt `d-rein` die Schranken, schreibt `temp/runden-sep09/schreibe-d-zeile.sh --apply` die eine Zeile |
