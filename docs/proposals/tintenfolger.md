...
| **2** | **P3 KOARTIKULATION (Owner-Priorität „zeitnah", 2026-08-15) — LÄUFT, vorregistriert §14 „Welle 2 · P3" `aug16`: die Vorstudie (248 Vorkommen) maß die Asymmetrie — Schwanz = Klassenkonstante (Chart-/Laufform-Frage), Kopf = echte Koartikulation nach Hoch-Exits (p < 0,0001, die Laufform kann sie bauartbedingt nicht tragen); drei Entry-Klassenregeln in der Reihenfolge K1 (Balken→Rund) → K3 (Deckstrich→Arkade) → K2 (d-Abgangs-WINKEL, nie der 2× verworfene Stub-Trim)** · B2 (Tiling) · A2 (SDM/DCD) · K3 (Owner: W-Trace + Laufform, z prüfen) | P3 zuerst; je eigene Vorregistrierung |
