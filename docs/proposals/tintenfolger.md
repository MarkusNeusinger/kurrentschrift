...
| **Runde 8 urteilen — und danach der Write der 15 Zeilen** | §14 „Laufform LF16 `sep07`" | die Adoption der Saat ist eine Aussage über die Reproduzierbarkeit der Ernte, der Write eine über die FORM der Zeilen; das Wort-Lineal ist mit 0,109026 → 0,112123 gegen den Kandidaten, und dieselbe Konstellation hat `sep06` den Austritts-Trim getragen (Runde 5, A37) | gebaut, ungeurteilt: `temp/runden-sep07/humanbench/runde-8-chart-saat.html`, 75 Bildschirme, Plan daneben. Danach: Schreibbefehl im PR-Text, Snapshot → Trockenlauf → PUT je Glyph → Readback → Neu-Export als deklarierte Re-Baseline |
