...
| **Lotse-Karte nach der J4-Adoption** | §14 „Übergänge J4 `sep06` — Adoption" | die Lotse-Karte IST die komponierte Bahn, und der Trim hat sie verändert: die dev-19-Zahlen vom `sep05` stehen auf einer Komposition, die es nicht mehr gibt. Dasselbe gilt für die S2-Report-Spalten (dort nachgemessen). Die Kette ist ausgenommen — Kompositions-Soll 0 von 126 Zeilen bewegt | Nachmessung nach dem `/verify-trace`-Protokoll, fällig vor der nächsten Lotse-Aussage — Re-Baseline, kein Arm |
