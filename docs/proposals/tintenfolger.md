...
| **Die `d`-Zeile als eigener Laufform-Arm** | §14 „Laufform LF16 `sep08`" (Rettungsweg 1), jetzt „Laufform LF17 `sep09`" | Runde 8 hat die 15-Zeilen-Karte als GANZE verworfen, ihre Zerlegung aber genau eine Zeile gefunden, die beide vorregistrierten Schranken räumt: `d` mit 10 : 2 von den entschiedenen Wörtern (83,3 %) bei 14,3 % Unentschieden — und ohne die vier `und…`-Wörter, wo `d` auf `u` trifft, sogar 10 : 0 ohne einen einzigen Tie. Der Beurteiler hat sie von sich aus genannt („also das d sieht definitiv besser aus"). Die Zahl ist bedingt: alle 15 Zeilen liefen zugleich | **Vorregistriert und gebaut `sep09`** auf Autor-Entscheid A40 (§14 „Laufform LF17 `sep09`"): die frische Ernte auf `ccb036a5eb20…` ist wieder ein Fixpunkt und die Zeile kommt **byte-identisch** heraus wie die geurteilte — der Komma-Ausschluss berührt kein `d`-Wort, die Wurzel-Einschränkung geht für sie also auf null. Offen ist nur noch das Urteil über Runde 10 (Zeile in der Autoren-Tabelle unten). `n` und `h` bleiben mit 20,0 % bzw. **0 von 8** ausdrücklich zu |
...
| **Runde 10 urteilen — und danach der Write der EINEN `d`-Zeile** | §14 „Laufform LF17 `sep09`" (Autor-Entscheid A40) | ein Laufform-Write ist ein Autorenvorgang und eine deklarierte Re-Baseline; und die Runde kann sich per Konstruktion keine Adoption selbst erteilen (26 bit-identische Nullproben sind ein Tie-Boden von 65 %, nötig für die blinden Wiederholungen) | offen: ~7 min Urteilszeit, 48 Bildschirme, `temp/runden-sep09/humanbench/runde-10-d-zeile.html`. Ein Freiheitsgrad (`d`), Basis-Tafeln byte-gleich mit Runde 8, das Lineal wieder leicht dagegen. Räumt `d-rein` die Schranken, schreibt `temp/runden-sep09/schreibe-d-zeile.sh --apply` die eine Zeile |
