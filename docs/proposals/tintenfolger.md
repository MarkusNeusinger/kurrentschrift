...
| **Die offenen J4-Konversionen** (2 von 4 sind gegangen) | §14 „Übergänge J4/J4b `sep02`", §7.9 | der Naht-Knick ist beseitigbar, aber `dconn` kann per Konstruktion nicht darüber urteilen: (2) `dspan` hat das `sep04` behoben, (3) die Wortrunde hat den Arm `sep06` mit 34 : 2 bestätigt — die Details stehen in §7.9 | **(1) die Ankunftsseite: beantwortet `sep06`** von der Nahtverhandlung (§14 „Übergänge J6") — sie schließt beide Nähte zugleich (Ankunft 10,80° → 0,01° im Betrags-Median über 240 Nähte) und löst die Austrittsseite dabei billiger als J4 selbst (12,67° → 0,02° bei +0,000376 statt +0,000582), ist aber an Gate (c) verworfen; die beiden Arme sind damit Alternativen, nicht Ergänzungen. Offen bleibt **(4) den Flick eine Stufe tiefer gar nicht erst lernen** (Endblenden-Familie LF5/LF6), unverändert ein Laufform-Arm ohne eigene Messung |
