...
> **1,5 xh** seit L-U (`aug26`). Wort-Lineal: Wörter **0,108339** ·
> Paare **0,148236** (Re-Baseline `sep10`, der `d`-Zeilen-Write;
...
> (seit `sep13` als Baustein im Repo, Vorgabe AUS) und Schlange (noch
> ungemergt) (§7.9).
>
...
> **K-E1/K-E2** (am Lineal `aug21` an Gate (3), vor dem Auge `sep09` mit
> 44 von 44 „kein Unterschied" — die Urteilsrunde war Rettungsweg (1) und
> schließt die Familie endgültig; offen bleibt allein der
> Distanzfeld-NUR-Claim) · **K-F Produktions-Init** (`sep04`) ·
...
> obwohl es jedes Gate bestand) · **J6 Nahtverhandlung** (am Lineal
> `sep06` an Gate (c), vor dem Auge `sep08` mit 39,1 % bei 63,5 %
> Unentschiedenen — der engere Arm J6b war nur bei ≥ 60 % lizenziert und
> bleibt damit zu). K-D
...
> `s` (je n = 1).
>
> **Geschrieben `sep10`: die `d`-Zeile allein, auf ein Menschenurteil**
> (§14 „Laufform LF17 `sep10`"). Die humanbench-Runde 10 hat den
> Freiheitsgrad isoliert, den Runde 8 nur als bedingte Spalte lesen
> konnte, und die Klasse `d-rein` räumt beide vorregistrierten Schranken
> **ohne einen Gegenbeleg** (0 : 10 bei 0 % Unentschieden, 8/8 Paare
> gleicher Arm). Der Autor hat daraufhin die EINE Zeile geschrieben
> (Entscheid **A44**), und der Neubau der Wurzeln danach ist die
> Re-Baseline, die die Wort-Zahl dieses Blocks trägt. Das Wort-Lineal ist
> dabei wieder leicht dagegen (+0,000186) — und in `d-rein` trägt sein
> Vorzeichen nachweislich keine Information über das Urteil. **Offen
> geblieben ist ein Gegenbefund:** in den vier `und…`-Wörtern geht die
> Zeile 4 : 0 an die BASIS. Das ist eine Naht-, keine Zeilenfrage und
> steht als eigener Arm auf der Route Übergänge (§7.11).
>
> **Offen (§7.11).** KI-messbar: LF4 (p) ·
> Abstandsterm/Schleifen-Halteterm für die 13 v5-Rückweisungen ·
> Distanzfeld-NUR-Claim (seit `sep09` der einzige verbliebene K-E-Weg, und
> nur noch mit der Sichtbarkeits-Auflage der Runde 9) · die **Saat-Distanz
> als Kette-Arm** (der Composer-Auftrag e-Breite, §7.2 — vom Beurteiler der
> Runde 9 als Hebel benannt) · Lotse-Zonen-Stufe · InkSight B2 und die
...
> beurteilbar) · von den zwei **J5-Konversionen** noch die Übergabe mit
> der gemessenen Anstrich-Pfeilhöhe statt der Sehne (der Sensor, der die
> Krümmung eines Aufstrichs sieht, ist seit `sep06` gebaut und an beiden
> Wortrunden abgenommen, §14 „Übergänge S2 gemessen") · der **Rausch-Boden des
> Folgers** und die zwei Arme
> dahinter (§14 „Kette K-F `sep04`") · formtreue Fenster-Pinnung · seit
> `sep07` die **zug-übergreifenden Schleifen in `loop_ranges`** (was das
> `t` braucht, vor dem nächsten Laufform-Write) und ein **Folger-Pfad
> für die Ernte** (ohne ihn erreicht kein Bahn-Arm die Kennzahl der 24
> schließenden Wörter) · seit `sep10` die **`d`→`u`-Naht** (der 4 : 0
> Gegenbefund der Runde 10, Route Übergänge) und die **Duell-Nachmessung
> auf der `sep10`-Wurzel** (der `d`-Zeilen-Write hat das Kompositions-Soll
> bewegt; Kette und Lotse stehen weiter auf `ccb036a5eb20…`). **Erledigt
> `sep04`:** die Duell-Nachmessung auf der damals heutigen Wurzel — beide
...
- **Fehlerzähler an den harten Stellen** (Kontrakt: detect auf beiden
  ...
  `retrace_arc_ratio`). **Die vierte Landmarke ist der KRINGEL** — seit
  `sep06` als eingefrorener Katalog je Buchstabe und je Schleife
  (Größenklasse `klein`/`mittel`/`groß` in Breiten der Plattenfeder,
  Zustand `offen`/`wechselnd`/Punktkringel), gelesen von
  `tools/tracebench/kringel.py` als **reine Report-Spalte**
  `kringel_lost`: nur eine Schleife, die die Platte in ≥ 80 % ihrer
  Vorkommen offen zeigt, ist ein Verlust, wenn sie zuläuft.
...
| **7** | humanbench-WORT-Runde (neuer Item-Renderer; Bias benannt: der Autor beurteilt eigene Nachfahrungen — Abkühl-Abstand oder Zweitrichter) | **erste Runde gefahren `sep02`** (Basis gegen LF11, Adoption ausgelöst); der K-E-Tie-Breaker ist am `sep09` als **Runde 9** gefahren und geht 44 : 0 unentschieden aus — der Modus entscheidet diesen Fall nicht |
...
- **A8 / K-D — Tinten-Korridor (Autor-Idee, 2026-08-20).** Eine
  ...
  **Der Auslöser ist `sep09` zum ersten Mal gezogen worden** (§14
  „Kette K-G `sep09`"): auf dem Saat-Arm steht `regieren` bei
  **0,427 xh** mit 0,16 xh Bogenlänge über der Schwelle, während die
  Basis desselben Tages kein Wort über 0,35 kennt (Maximum `Sporn`
  0,323). Der Arm ist NICHT adoptiert, die Produktions-Kette also
  unberührt — der Korridor bleibt geschlossen, aber die Auslöse-Zeile
  ist ab jetzt eine gemessene und keine hypothetische. Wer den
  Korridor wiedervorlegt, tut das gegen einen Arm, der die Klasse
  erzeugt, nicht gegen die Produktion.
...
| **Übergänge J5 Klassenregel vor dem Auge** (`sep05`, humanbench-Runde 6, Autor-Entscheid A36) | Das Menschenurteil verwirft die Regel als Ganzes: **Basis 20 : Kandidat 1** von 21 entschiedenen Bildschirmen (4,8 % gegen die vorregistrierten ≥ 60 %). Je Klasse `apex` 1/12 — und `stem` **0/7 bei EINEM Unentschieden, obwohl `stem_depart` am `sep04` jedes Gate bestanden und beide Headline-Zahlen verbessert hatte** (−0,000100 / −0,001441, 14 : 4). Schärfer noch: die fünf Wörter, die das Wort-Lineal dem Apex-Arm gutgeschrieben hatte (`Soldaten` · `schießen` · `fechten` · `muß-2` · `linken`), gehen **5 : 0 an die Basis** — wo die Kennzahl belohnt, verwirft das Auge. Das Instrument ist dabei sauber: **12/12 Nullproben** korrekt als „kein Unterschied" erkannt (12 der 13 Ties sind sie, unter den 22 bewegten Wörtern bleibt genau eines unentschieden — die Antwortoption ist benutzbar und wurde benutzt; über die LF11-Tie-Quote auf nicht identischen Tafeln sagt das nichts), Verlässlichkeit 4/4 gleicher Arm bei nur 1/4 gleicher Seite, keine Drift — aber **4 < `MIN_PAIRED_REPEATS` 6**, die Runde trägt also keinen ADOPTIONS-Anspruch (den sie nicht braucht: sie stützt den Standard, der ohnehin gilt). Der gemessene Grund steht im Payload und deckt sich mit dem Satz des Beurteilers („unnatürlich gerade Linien … obwohl die einen leichten Bogen haben sollten"): die Übergabe läuft 1,67–1,84 xh mit einer Pfeilhöhe von **0,001–0,002 xh** (in `streiten` 3,30 xh am Stück) und löscht dabei einen Anstrich, der 0,015–0,040 xh Bogen trug — die Tafelzelle trägt t 0,180 · ſ 0,136 · k 0,187 · ß 0,175 xh | (1) **Die Übergabe trägt den Bogen** (neuer Mechanismus): die erzeugte Übergabe bekommt die gemessene Pfeilhöhe ihres Zielbuchstabens mit statt der Sehne; Zielmenge sind die 14 Wörter dieser Runde, in denen die Regel feuert, Gate „Pfeilhöhe/Sehne im Band der Tafelzelle ±25 %" plus die Bedingung, dass `gleichzug_doublings` nicht steigt; (2) **ein Sensor, der die Krümmung sieht** (neuer Sensor): Pfeilhöhe über Sehne des letzten Zugs vor einem Scheitel bzw. des ersten nach einem Austritt, je Join — erst eingefrorene Report-Spalte, Abnahme wie bei `dspan` (Nullproben + Trennschärfe an den 21 entschiedenen Bildschirmen), dann erst Gate. Ohne ihn konnte `stem_depart` alle Gates bestehen; (3) **Autorenweg, von A4 schon benannt**: t/ſ/k im Wizard mit verbindungsfähigem Anstrich nachfahren — dann muss kein Generator einen Bogen erfinden; (4) **dieselbe Klasse mit ≥ 6 Paaren** — ausdrücklich keine weicheren Gates, sondern ein höherer Boden: `n − min_gap − REPEAT_JITTER` ≥ 6 verlangt bei `--min-repeat-gap 5` **≥ 36 Einträge**, für diese Klasse also 14 statt 12 Nullproben; fällig erst, wenn Weg (1) einen neuen Kandidaten liefert | (1) eigene Pre-Reg, jetzt mit Sensor messbar; (2) **gegangen `sep06`** (§14 „Übergänge S2"): der **Unstetigkeits-Sensor** ist gebaut, eingefroren und an BEIDEN Wortrunden abgenommen — Nullproben exakt (Neukomposition trifft die gespeicherten Verbinder auf 0,000e+00, die 3 + 12 identischen Wörter bewegen keine Spalte), und auf `apex` fällt `bow_join` 0,0066 → 0,0034 in 12 von 12 Wörtern, während `kink_max_deg` 38,46 → 50,46 steigt. Instrument gewonnen, mit einer benannten Grenze: KEINE einzelne Spalte ist über alle vier entschiedenen Klassen richtungstreu, und `bow_join` allein hätte Runde 6 falsch entschieden (Rahmen-Artefakt der Population, wie bei `dconn`/`dspan`) — ein Gate darauf braucht die Kanäle getrennt und den Rahmen fixiert; (3) Autorenschritt (Todoist); (4) nur mit neuem Kandidaten |
| **Ink-Clearance an die Feder** (`sep05`, Folgearm des Platten-Nibs, Autor-Entscheid statt Runde) | Jede Tinten-Clearance in Nib-Radien statt x-Höhen, am Kalibrierwert 0,07251 geklemmt. Verworfen an Gate 1 (Verdopplungen bei Halbbreite 0,097 **21 → 21**: `Wer` geheilt, `Seiten` re-segmentiert) und Gate 2 (`coverage` 0,101516 → 0,105761); Golden byte-gleich, Chart-Nib ziffernweise 0,109218 · 0,148198. **Der Grund ist geometrisch und trifft die ganze Klasse:** ACHT der neun neuen Verdopplungen sind der Deckungs-Join NEBEN dem Körper, in den er landet — eine Platzierungs-Clearance verschiebt Join und Landekörper GEMEINSAM, `sep` kriecht 0,176 → 0,178 gegen eine Bandobergrenze 0,262 (das Band zu verlassen bräuchte Skala ≈ 20); die neunte (`Galoppieren` r × e) ist Körper gegen Körper an einer gewollten Arm-Fusion, die per Konstruktion unter jeder Clearance liegt. Die nicht vorregistrierte Empfindlichkeits-Leiter zeigt dazu: Skala 2,0 drückt die Zahl auf 8, fabriziert dabei `regieren` neu, lässt sieben von acht Stellen in Wörtern stehen, die die Chart-Feder gar nicht kennt, vervierfacht den Deckungspreis und ist nicht monoton (9 bei 3,0) — **kein Punkt der Leiter besteht Gate 1**. Zweitbefund, vom Detektor per Konstruktion unsichtbar: neun Glyphen (`a` `o` `sz` `g` `r`-Chart `p`-Laufform `v` `k` `G`) verlieren unter der schweren Feder ihre Binnenfläche, `f` wird eingeschnürt — 31 der 63 Wortproben betroffen, der Befund des Autors | (1) **neuer MECHANISMUS: eine ANFLUG-Regel im Verbinder** statt eines Abstands — der Join dreht seinen letzten Abschnitt vom Landekörper weg, statt neben ihm herzulaufen (Verwandtschaft J4 `exit_trim`, das die Austrittsseite fasst); fällig erst, wenn die Feder wirklich gewechselt werden soll; (2) **Laufform-Zeile statt Chart-Form**, wo die Zeile die weitere Binnenfläche trägt (`r`: 0,0663 gegen 0,0428 — die Zeile überlebt, die Chart-Form nicht; beim `p` umgekehrt): die Öffnungsweite je Zeile als GATE in den nächsten Prod-Re-Harvest (LF12) aufnehmen, heute misst sie kein Laufform-Gate; (3) **Autoren-Nachzeichnung** der acht Formen unter `2·Δhalf` = 0,049 xh, bzw. der Eigenhand-Bestand als zweite Quelle, weil er mit der schweren Feder entsteht. **Nachtrag 2026-09-06, nach der Kringel-Diagnose** (`notes/kringel-binnenflaechen-2026-09-06.md`; die Mittellinien-Schleife der Komposition ist 0,10–0,20 xh enger als die der Platte, davon 0,035–0,104 xh schon in der Mittelachse und 0,004–0,082 xh beim Schritt Vorkommen → Zeile, während je Schleife drei Viertel des Kette-Defizits aus der Mittelachse geerbt sind — Median 0,75, p25 0,50 · p75 1,03; „Kette gegen Skelett" ist signiert −0,0116 und dem Betrag nach 0,0329 xh): (4) **Öffnungsweiten-Gate im Laufform-Schätzer** — eine Zeile darf keine Binnenfläche verlieren, die ihre Vorkommen tragen; macht Weg (2) aus einer Absicht eine Messgröße und trägt sofort das `p`, dessen Laufform-Zeile ihren Kringel schon heute verloren hat (`D0` 0,0095 gegen 0,2556 der Chart-Zeile); (5) **schleifen-treuer Median statt des elementweisen** — die Vorkommen vor dem Median je Schleife ausrichten, damit die Zeile die Form mittelt und nicht Form gegen Lage; (6) **Schleifen-Geometrie aus der Evidenz** für die engen Formen (die Kette hält 7 von 8 offen); (7) **Sensor zuerst: Verschmelzungs-Anzeiger** `(D0_skel − Öffnungsweite)/2` gegen `w_pen` als Report-Spalte, bevor irgendein Öffnungs-Term in einen Fit-Loss geschrieben wird. Ausdrücklich KEIN Weg: die Skala hochdrehen — und ebenso wenig die Feder dünner rechnen, bis die Löcher aufgehen | (1) eigene Pre-Reg, nach dem Feder-Entscheid; (2) mit LF12; (3) Autorenschritt (Todoist); (4) **gegangen `sep06`** als Gate (a) von LF13 — rot, aber 4 verletzte Bereiche der Basis gegen 2 des Kandidaten, keine Regression; (5) **gegangen `sep06`** und verworfen (§14 „Laufform LF13 gemessen"): der Schätzer war nie der Träger, gegen den EIGENEN Stapel kostet er 0,005–0,029 xh gegen 0,016–0,057 xh der Ernte davor, und die Decke jedes schätzerseitigen Mechanismus ist EIN Buchstabe (`sz`), weil `a`/`o`/`g` schon in den Vorkommen zu sind; eigene §7.9-Zeile unten; (6) unverändert offen und jetzt der einzige Weg, der die 0,10–0,20 xh ganz zurückholen kann; (7) **gegangen `sep06`** — gebaut und geeicht (monotone Kurve über sechs Bänder 0,609 → 0,878, `S` bei 0,970), bleibt Report-Spalte |
| **Laufform LF13 schleifen-treue Zeile** (`sep06`, Rettungswege (4)/(5) der Kringel-Diagnose) | Die Registrierung jeder Schleife auf Lage UND Größe des Stapel-Medians vor dem Median. Verworfen an Gate (a) (zwei von sechzehn Schleifenbereichen bleiben unter dem Vorkommens-Median — die Basis verletzt VIER, der Kandidat zwei, keine Regression) und Gate (c) (`bench_loss` 0,108444 → 0,108637, `pair_loss` +0,000003); (b) 0,0268 von 0,03, (d) Identitäts-Gate PASS, (e) Golden byte-gleich. Kennzahl 26 → **23** schließende Wortproben, Verdopplungen bei 0,097 22 → 21, alles über die eine Glyphe `sz`. **Der Grund ist zurechenbar und liegt vor dem Schätzer:** gegen den EIGENEN Eingangs-Stapel gemessen (der LF12-Stapel leitet alle 18 Zeilen byte-gleich ab) kostet die Zeilenbildung nur 0,005–0,029 xh, während die ERNTE davor — das Zerlegen eines Kette-Wortfits in ein zentriertes 120-Anker-Vorkommen — 0,016–0,057 xh kostet; Anteile am Restverlust Lage −22 % · Größe −1 % · Basis +21 % · Formstreuung +53 %. Und die Decke ist gemessen: nur `sz` (0,2167) und `r` (0,2177) tragen in den Vorkommen mehr als die 0,194, die bei Halbbreite 0,097 zum Offenbleiben nötig sind — `a` 0,1909, `o` 0,1654, `g` 0,1809 sind schon in der Evidenz zu | (1) **die ERNTE statt des Medians** — warum verliert die Dissektion 0,016–0,057 xh? Erst als Zerlegung messen (Fit-Residuum gegen Öffnungsweite je Vorkommen), dann ein Arm; (2) **R3** (Schleifen-Geometrie aus der Evidenz) unverändert offen und jetzt der einzige Weg, der die 0,10–0,20 xh ganz zurückholen kann; (3) **der `sz`-Teilerfolg als enger Arm** — nur dort registrieren, wo der Lage-Anteil über 50 % liegt, statt das Lineal über alle Zeilen zu belasten (Asymmetrie-Regel `aug26`); (4) Autorenweg unverändert. Ausdrücklich KEIN Weg: den Öffnungsweiten-Boden doch bauen (Decke gemessen = ein Buchstabe) | (1) **gegangen `sep06` und GESTRICHEN** (§14 „Laufform LF14"): auf EINEM chart-verankerten Bereich an beiden Enden gemessen kostet die Ernte −0,0002 bis +0,0077 xh, nicht 0,016–0,057 — die alte Zahl vergleicht zwei verschiedene Schleifen (Zuordnung über Nähe im 0,45-xh-Fenster). Eigene §7.9-Zeile unten; (2) **damit der einzige verbliebene Weg**; (3) eigene Pre-Reg; (4) Autorenschritt |
| **Laufform LF14 Ernte-Zerlegung** (`sep06`, Rettungsweg (1) von LF13) | Die Zerlegung streicht ihren eigenen Anlass: von der Kette-Bahn zum gespeicherten Vorkommen gehen −0,0002 bis +0,0077 xh Öffnungsweite verloren (40 von 44 Vorkommen unter 0,01, und von den vier übrigen verliert genau eines), das `o` gewinnt an der Fluent-Weitung, Zentrierung und Rundung bewegen im Median 0,0000 bei einem Raster-Boden von 0,0008. Der eingefrorene Sampling-Operator — der Hauptverdächtige — trägt 0,0000–0,0080. Der EINE reale Schritt ist die Nachreparatur gestrandeter Anker („**Schleifen-Sehne**"): von 235 Vorkommen werden 20 repariert, 12 davon mit messbarer Binnenfläche, und die 4, deren Reparatur innerhalb einer Schleife landet, verlieren alle vier (Median 0,0410, schlimmstenfalls 0,1068 xh), während die 8 außerhalb ≤ 0,0021 bewegen. Arm `LOOP_AWARE_REPAIR` verworfen an Gate (b) (0,0486 xh Zeilenbewegung am `Z` für 0,0045 xh Öffnung) und Gate (c) (Paare 0,148352) — **beide Lineal-Zahlen trägt die Kontrollkarte**, der Arm bewegt Wörter um −0,000044 und Paare um 0,000000; Gate (a) grün (1 → 0 Brüche von 44), (d)/(e) grün. Kennzahl 26 → **26**: drei Vorkommen gehen auf, kein Wort, weil ein reparierter Fit in einem Stapel von vier den Median nicht über 0,194 zieht (`g` 0,1743 → 0,1823) | (1) **R3** — Schleifen-Geometrie aus der Evidenz, jetzt ohne Konkurrenz: die Ernte reicht weiter, was die Kette findet, also sitzt der Verlust im Übergang Vorkommen → Zeile und davor in der Mittelachse; (2) **der Fixpunkt der Ernte** — eine frische Ernte der `sep05`-Wurzel leitet jede Zeile 0,0025–0,0283 xh neben der ab, aus der die Wurzel geschrieben wurde (`--jobs 1` = `--jobs 4` byte-gleich; Ursache `chain_seed="composed"` auf einer Komposition aus den Zeilen selbst) — messen, ob die Iteration konvergiert und ob `chain_seed="grid"` sie löst; (3) den Schalter als ENGEN Arm nur dort, wo die Reparatur der einzige Träger der Zeile ist (`g`), Asymmetrie-Regel `aug26`; (4) Autorenweg unverändert. Ausdrücklich KEIN Weg: `STRANDED_STEP_RATIO` aufweichen | (1) eigene Pre-Reg, zuerst; (2) **gegangen `sep06` als LF15** — sie konvergiert nicht, und die Chart-Saat löst sie; (3) eigene Pre-Reg; (4) Autorenschritt |
| **Kringel-Landmarke** (`sep06`, Sensor statt Arm; Autor-Entwurf 2026-09-06) | Kein Negativ, sondern die ZERLEGUNG der Kennzahl 26, mit der jeder Rettungsweg der Kringel-Diagnose jetzt ein Ziel je Schleife hat statt einer globalen Schwelle. Der eingefrorene Katalog `tools/tracebench/kringel_catalogue.json` gibt für 46 Schleifen über 27 Glyphen die Größenklasse (in Breiten der Plattenfeder) und den Zustand (`offen` · `wechselnd` · Punktkringel) — 22 klein · 16 mittel · 8 groß, 41 offen · 4 wechselnd · 1 punkt. **Auf #551s Messpfad sind 24 der 27 schließenden Wörter echte `offen`-Verluste und 3 nur `wechselnd`; über ALLE Schleifen verlieren 34 der 63 Wörter bei 0,097, dazu 19 Topologie-Verluste in 14 Wörtern.** Die d-Schleife braucht nichts (0,017 xh weiter als die Platte), die e-Öse ist der größte relative Verlust und trotzdem kein Defekt, und das `t` hat gar keinen Duktus-Schleifenbereich | (1) **R3 mit Zielgröße je Schleife** statt einer globalen: `a` +0,1055 · `o` +0,1461 · `sz` +0,1349 · `r` +0,1182 xh (neue EVIDENZ für den bestehenden Weg); (2) **ein Duktus-Schleifenbereich für das `t`** und die Prüfung der sechs range-losen Glyphen (`n` `m` `i` `u` `c`) — neuer MECHANISMUS, ohne ihn ist das größte Defizit des Katalogs für LF13/LF14 unsichtbar und der einzige Verlust, der auch bei 0,0724 bleibt; (3) **Skelett-Schleifen für Versalien** (`G` `k` `v` `P` `F` `W` ohne Laufform-Zeile, H3); (4) **Topologie-Verlust als eigene Klasse vor die Öffnungsweite** — 19 nie gezeichnete Binnenflächen holt kein Weitungsterm zurück, hier greift der Mittelachsen-Term (R4-Anzeiger) plus ein Zwei-Zug-Modell an verschmolzenen Stellen. Ausdrücklich KEIN Weg: Klassen- oder Zustandsschwellen nachziehen | (1) mit dem nächsten R3-Arm; (2) **vor dem nächsten Laufform-Write**; (3) eigene Pre-Reg; (4) eigene Pre-Reg, nach (2) |
| **Kette R3 Zwei-Züge-Modell** (`sep07`, Rettungsweg R3 der Kringel-Diagnose, Autor-Entscheid „2 ja") | **Die Binnenflächen gehen auf, und jedes der vier gemessenen Gates fällt an der AUSFÜHRUNG.** Die Feder herausrechnen statt den Klumpen skelettieren: keine Stützstelle näher als `w_pen` = 0,0968 an einer `offen`-Binnenfläche des Katalogs. Ergebnis: **107 von 108 korrigierten Schleifen offen bei 0,097** (vorher 102/108); `a`#0 0,2024 → 0,2746, `o`#0 0,1741 → 0,2516, `sz`#0 0,2231 → 0,3063. **(a) rot** — die Weite trifft die Platten-Erwartung nur in 83/108, gefordert ist sie je Schleife. **(c) rot am p90** (+0,000699), während Median (−0,000701), Vorzeichentest **14 : 3** (p = 0,0127), aiou 0,7583 → 0,7601, Chamfer beidseitig und der 63er-k0-Soll (80 → 79) besser lesen. **(b) rot** (`cross_missing` 12 → 13, `cross_spurious` 7 → 9 — **der ganze Betrag auf `will` und `Galoppieren`**, 17 der 19 dev-Wörter bewegen keinen Zähler). **(d) rot** (1 626 neue `kink`-Ereignisse, p90-Δ +16,70°). **Die Ursache ist gemessen und ist eine Länge:** die Stützstellen stehen 0,0265 xh auseinander, die deklarierte Anti-Raster-Glättung 0,0363 umfasst 1,37 davon und die C¹-Blende 0,0725 nur 2,74 — beide Fenster sind in Federbreiten gedacht und in Stützstellen zu kurz, also landet das Pixelraster der Platte (±0,5 px ≈ 0,015 xh) ungefiltert in der Federbahn, und es begrenzt zugleich, wie genau der Schub seine eigene Zielweite treffen kann | (1) **R3b — die Blende ans Lesefenster** (in derselben Runde vorregistriert und gefahren): Blenden-Maßstab von einer halben auf zwei Federn, Glättung damit auf eine Feder = das Fenster, mit dem der Sensor liest; abgeleitet, nicht gewählt, und mit vorab benanntem Preis an Gate (a); (2) **die enge Klasse** — nur die Schleifen korrigieren, deren Wort keinen Strukturzähler bewegt (`will` und `Galoppieren` sind zwei von 19; Asymmetrie-Regel `aug26`); (3) **der Öffnungs-Term im SOLVE statt danach** — jetzt belegt statt vermutet: eine punktweise Nachbearbeitung kann per Konstruktion nicht glatter sein als ihr Raster, ein Constraint im Folger-Solve schon (R4s Warnung „erst der Sensor" ist mit #556/#558 erfüllt); (4) **die Korrektur auf die EVIDENZ statt auf die Ausgabe** — dieselbe Entfaltung auf das Skelett gelegt, das der Lotse reitet und an dem sich die Kette misst (R4, Mittelachsen-Term), mit den 106 Zahlen dieses Arms als Eichung. Ausdrücklich KEIN Weg: die Feder dünner rechnen, ein Gate lockern, oder den Schub kappen, bis die Zähler wieder stimmen | (1) **gefahren `sep07` und ausgemessen**: Knicke 1 626 → 462, p90 +16,70° → +4,77°, `cross_spurious` zurück auf 7 — aber Gate (a) 83/108 → 62/112, (c) weiter rot am p90 und k0-Soll 79 → 81, also auch nicht adoptiert. **Der Weg ist damit geschlossen und trägt (3):** beides hängt an derselben Länge; (2) eigene Pre-Reg, jetzt auf EIN Wort verengt (`Galoppieren`); (3) **gefahren `sep07` als R3c und ausgemessen** — Gate (d) grün (Knicke 2 296 → 2 258 statt +1 626), Gate (a) aber nur 64/156: die Bedingung im Solve bewegt nur, was schon offen war. Der Weg ist damit ebenfalls geschlossen und trägt (4); eigene §7.9-Zeile unten; (4) **damit der aussichtsreichste**, eigene Pre-Reg |
| **Kette R3c Binnenflächen-Bedingung im Solve** (`sep07`, Konversion (3) von R3) | **Das genaue Gegenstück zu R3: die Glätte ist umsonst zu haben, die Weitung nicht.** Dieselbe Aussage über die Tinte, aber als quadratischer Hinge auf dem vorzeichenbehafteten Abstandsfeld der `offen`-Binnenflächen, den der Folger in jeder Runde sieht — normiert wie `e_geo`, über `sampling_op` auf die Anker gefaltet, `fit_word_chain` unberührt, ohne Blende und ohne eigene Abnahmeregel. **Gate (d) grün:** `kink`-Ereignisse 2 296 → **2 258** (R3: +1 626, R3b: +462), Median 8,08° → 7,53°, `wobble`-Median besser. **Gate (a) rot auf JEDER Sprosse** der Leiter {1, 4, 16, 64, 256}: 22 · 38 · 52 · **64** · 51 von 156 Treffern gegen die geforderten 149, mit einem INNEREN Maximum bei 64 (bei 256 setzt der Wächter Wörter ganz auf den Kettenfit zurück, `h`#0 steht wieder exakt auf dem Basiswert). **Die Zerlegung:** nach Klasse der Basis-Schleife trifft der Arm 61 von 124 schon offenen (Median-Bewegung +0,0227) und **1 von 13 kollabierten sowie 2 von 18 zugelaufenen (Median-Bewegung je +0,0000)** — er hilft nicht dort, wo der Defekt sitzt, weil der Tinten-Term an einer verschmolzenen Stelle selbst auf der Klumpenachse liegt. (b) rot (`cross_missing` 11 = 11, `cross_spurious` 9 → 10, dieselbe Signatur schon bei Gewicht 16), (c) rot am p90 (+0,001618) bei besserem Median (−0,000365) und `aiou` 0,7660 → 0,7611, (e) 63er-Soll **85 → 81**, (f) Headline 0,109026 · 0,148236 unverändert, (g)/(h) grün (Determinismus 63/63 strichgleich, 0,000e+00). Nebenbefund über den Wächter: 26 der 63 Wörter verlieren ihre Folge ganz oder halb (14 revert-init · 12 revert-r1 · 19 zonal · 18 sauber) — eine RUNDEN-Regel dort, wo R3 eine Regel je SCHLEIFE hatte | (1) **eine Abnahmeregel je SCHLEIFE statt je Runde** — die zonale Rückweisung (K0-Z) um die Binnenflächen-Stellen erweitern, damit eine Schleife, die ihre Kreuzung verliert, nur ihre eigenen Anker pinnt; (2) **die enge Klasse** — nur die 124 Schleifen binden, die die Basis schon offen zeichnet (dort 61 Treffer und +0,0227 Median-Bewegung, an den übrigen 31 nachweislich nichts), Asymmetrie-Regel `aug26`; (3) **der weite Geltungsbereich als eigener Arm** (`--counter-size-classes klein,mittel,gross`, gebaut und messbar, vor der ersten Armzahl zurückgenommen) — erst nach (1); (4) **die Bedingung auf die EVIDENZ statt auf das Objektiv** (R4, Mittelachsen-Term): die Feder aus dem Abstandsfeld herausrechnen, das der Tinten-Term liest, mit den 156 Zahlen dieser Runde und den 106 von R3 als Eichung. Ausdrücklich KEIN Weg: die Feder dünner rechnen, ein Gate lockern, den Wächter abschalten, das Gewicht über die Leiter hinaus treiben | (1)–(3) je eigene Pre-Reg; (4) **der aussichtsreichste**, eigene Pre-Reg — beide Klammern (R3 hinter dem Solve, R3c im Solve) zeigen auf dieselbe Ursache |
| **Laufform LF15 Ernte-Fixpunkt** (`sep06`, Rettungsweg (2) von LF14 · Autor-Entscheid „3 ja") | Kein Negativ, aber eine vertagte Adoption, und deshalb hier: die Iteration der Ernte KONVERGIERT NICHT — über drei Schritte bewegt sich die schlimmste Zeile 0,0582 → 0,0627 → 0,0627 xh, die Richtung wechselt je Zeile, der angenommene Satz wandert 235 → 232 → 239 mit, und selbst die Kringel-Ablesung überschreitet die 0,194-Marke allein durch Weiterernten. Die **Chart-Saat** (`chain_seed="chart"`, Default AUS) schneidet die Rückkopplung: `K1` = `K2` = `K3` byte-gleich samt Vorkommen, wobei `K3` aus einer 0,058 xh entfernten Karte startet. Alle vier Gates grün — (F) 0,0000, (G) `cross_missing` 12 = 12 bei `cross_spurious` 7 → 6 und dtw-Median −0,000087, (H)/(I) byte-gleich —, der Schalter bleibt trotzdem AUS, weil Bericht (J) vor der Zahl feststand und 8 von 21 Zeilen mehr als 0,03 xh bewegen: das ist eine deklarierte Re-Baseline, kein Beifahrer. Kostenseite benannt: 226 statt 235 Vorkommen (`connector_degenerate` 24 → 30, `anchor_spike` 8 → 11, Fitgüte dagegen besser: `geo_rmse` 1,002 → 0,982), `Z` unter dem Beleg-Boden, `aiou` −0,0049 und drei deutlich schlechtere Wörter (`Galoppieren` +0,0280) | (1) **Die 9 Vorkommen zurückholen** — sie fallen an Startlage-Gates, nicht an der Fitgüte: ein Zwei-Stufen-Lauf (Chart-Saat solven, dann EINMAL auf dem eigenen Ergebnis nachsolven) bliebe zeilen-unabhängig und säße näher an der Tinte; (2) **Saat aus dem gespeicherten Vorkommen** (Kandidat (c), nicht gebaut) — näher an der Tinte, trägt aber eine eigene Rückkopplung und muss sie mitmessen; (3) **das `Z` unter dem Boden** — entweder Weg (1) holt das dritte Vorkommen zurück oder die Zeile bleibt beim Bestand; (4) Autorenweg: die Adoption als Default IST ein Re-Baseline-Write. Ausdrücklich KEIN Weg: den Beleg-Boden senken, und keine Karte nur noch gegen ihre Kontrollkarte berichten — die ist die richtige Vergleichsgröße, macht die Iteration aber nicht kleiner | (1) eigene Pre-Reg; (2) eigene Pre-Reg; (3) **gegenstandslos auf dem heutigen Composer** (§14 „Laufform LF16 `sep07`": `Z` steht chart-gesät auf n = 3, also auf dem Beleg-Boden — unter ihm bleiben nur `P` und `s` wie am Bestand); (4) **gegangen 2026-09-07 (A38)**: die Chart-Saat IST der Ernte-Default, der Write hängt an der Urteilsrunde 8. Die stehende Regel „**zweimal ernten**" bleibt, hat aber ihren Anlass verloren und ist jetzt eine Selbstprüfung mit erwartetem Ergebnis (`werkzeuge.md`) |
| **Übergänge J6 Nahtverhandlung** (`sep06`, Autorenregel vom 2026-09-06) | Die Naht schließt sich, und zwar an BEIDEN Enden: über alle 240 gemessenen Nähte fällt der Betrags-Median des Abgangs von 2,67° auf **0,01°** und der der Ankunft von 10,80° auf **0,01°**, keine einzige Naht verlässt das 3°-Band nach oben, und jede verhandelbare Klasse geht auf ≤ 0,36° (Arkade→Arkade **18,57 → 0,00** über 24 Wort-Joins, sonstige 17,24 → 0,03, Gabel 14,43 → 0,01). Die Platzierung ist beweisbar unberührt (`meas_doff` 0,129 = 0,129, wenn der Arm ohne J4 läuft) — der Nahtpunkt ist der Drehpunkt. **Verworfen an Gate (c):** die Gleichzug-Verdopplungen der Paare steigen 3 → 5, und die Träger sind zurechenbar: an `dp` und `ds` muss der Verbinder **32,9°** bzw. **31,9°** tragen, die der 8°-Deckel dem Buchstaben verwehrt, und der so entstehende Haken legt sich 0,117/0,119 xh neben die eigene absteigende Tinte des `d` (Nib 0,145, Doppelungsband 0,073–0,196, Bogen 0,25–0,26 xh). Gate (b) rot nur am MAXIMUM (an der Naht +0,14°, p90 über den Blend +0,46°/+1,67°; die Maxima sitzen an Vertices mit 14,6° bzw. 133° Eigendrehung), Gate (a1) rot aus Prinzip (auf der Skala des einzelnen Segments ist der Rest die Spitzenkrümmung der Buchstaben; ihn wegzubekommen hieße gerade Auslaufstücke einzusetzen, was die Vorregistrierung als Sensor-Trickserei verboten hat). Der am selben Tag gelandete Unstetigkeits-Sensor folgt dem Arm NICHT: `cont_kink_total` 337 → 349 (Wörter) und 81 → 92 (Paare), teils mechanisch, weil `cont_measured` um 51 bzw. 86 steigt — Punkte verlassen die Ecken-Ausnahme und werden erstmals gemessen | (1) **J6b — nur verhandeln, wo sich beide Seiten treffen KÖNNEN** (`max_jump` = 2 × Deckel = 16°): post-hoc gemessen, Gate (c) grün (3 = 3), Nähte 0,00/0,02 — der Preis ist benannt, die größte Ankunftsklasse bleibt stehen (Arkade→Arkade 18,57° über 24 Joins); frische Pre-Reg nötig; (2) **J6c — den Anteil des Verbinders über seinen GANZEN Bogen verteilen** statt über 0,45 davon: der Haken entsteht, weil 33° in 0,3 xh liegen, über den vollen Bogen wäre dieselbe Drehung eine Krümmung (neuer Mechanismus, nicht derselbe Knopf weicher); (3) **der Deckel je Klasse, gemessen an den Vorkommen** — ein Schleifen-Exit darf in seiner letzten Kurve mehr drehen als eine Arkade, womit der Anteil des Verbinders genau dort sinkt, wo er heute den Haken macht; (4) **identische Punktmenge für den Unstetigkeits-Sensor**, bevor seine Kink-Zahl einem Arm zugerechnet wird (dieselbe Rahmen-Regel, die `spanmeas` für `dconn` erzwungen hat); (5) **humanbench-Runde 7** liegt gebaut. Ausdrücklich KEIN Weg: den Deckel hochdrehen, bis die Naht auch ohne Verbinder-Anteil zugeht | (1)–(3) je eigene Pre-Reg; (4) zuerst, sonst ist der Sensorbefund nicht zurechenbar; (5) **gegangen `sep08`** — die Runde ist geurteilt und ging mit 39,1 % aus, also ist auch J6b nicht lizenziert (eigene Zeile darunter, §14 „Übergänge J6 `sep08`") |
| **Kette R4 Feder-Entfaltung der Evidenz** (`sep07`, Konversion (4) von R3 UND R3c) | **Zuerst eine Diagnose, und sie schließt einen ganzen Zweig:** die Verschmelzung, auf deren Achse der Tinten-Term liegt, ist **echte Tinte und kein Raster-Artefakt**. Der Fixture-Ausschnitt ist ein unskalierter Schnitt der Platte (30–33 px/xh, `words-abb19.png` byte-identisch von Commons), und der zweite committete Scan derselben Platte trägt für sie **weniger** Auflösung, nicht mehr — `suetterlin-leitfaden-1926/p060.jpg` ist eine GANZSEITEN-Aufnahme, Handschrift-Zeilenabstand **≈ 104 px gegen ≈ 110 px** (nachgemessen nach einem Copilot-Fund, der die erste, bloß behauptete Fassung dieses Satzes zu Recht angriff); eine globale 50-%-Niveaulinie liest bei 1× ziffernweise dieselbe Binnenfläche wie die adaptive Maske, bei 4× bikubischem Grau eine **kleinere** (0,1118 gegen 0,1290 — 0,53 px im Durchmesser, die Größenordnung der deklarierten Halb-Pixel-Konvention), und der Mittelachsen-Anzeiger bleibt bei 4× fast unverändert (0,807 → 0,790). Beide vorregistrierten Schranken (+0,016 xh · +0,10) klar verfehlt. **Der Arm dazu — die Feder aus dem Attraktor herausrechnen — ist nicht adoptiert, bewegt aber als erster die richtige Klasse:** von den 33 Schleifen, die die Basis zu zeichnet, gehen **8 auf** (7 zugelaufene, 1 kollabierte: `Feinde` `F`#1 0,0025 → 0,3184), wo R3c dort 0 bewegt hat; offen bei 0,097 **124 → 129**, referenzfreier 63er-Soll **85 → 70** (R3 80 → 79, R3c 85 → 81), Gate (b) erstmals BESSER als die Basis (`cross_spurious` 9 → **7**, `retrace_spurious` 10 → 9) und Gate (d) nur noch +34 statt +1 626 (R3) bzw. +462 (R3b) Knicke. Gefallen ist er an Gate (c) — rot unter BEIDEN Lesarten (gepaart p90 +0,000834, eigene Verteilung Median +0,000385), also Kill — und an Gate (a) mit **36 von 157** statt 149. Der Preis ist scharf geschnitten: **drei vorher offene Schleifen fallen ganz zusammen** (`unter` `t`#1 · `Kugel` `K`#1 · `macht` `a`#0, je auf den Ableseboden 0,0025), das ist Topologie und keine schlechtere Weitung | (1) **die enge Klasse** — nur die 33 zu-Schleifen korrigieren: dort gewinnt der Arm 8 und kann die drei offenen, die er heute verliert, per Konstruktion nicht mehr anfassen (Asymmetrie-Regel `aug26`); (2) **ein glattes Feld statt eines Rasterbandes** — dieselbe Niveaulinie sub-pixel ausgewertet und mit `DIST_FIELD_SIGMA_PX` geglättet, R3cs eigene Konvention auf die Evidenz übertragen; zielt auf (d) und die Treffgenauigkeit von (a) zugleich; (3) **die drei Totalverluste zerlegen** (Wächter-Ausgang, ersetzter Bogen) — erst messen, dann ein Arm; (4) **eine Abnahmeregel je SCHLEIFE**, unverändert die Vorbedingung aus R3c (27 der 63 Wörter verlieren ihre Folge ganz oder halb an einer RUNDEN-Regel); (5) **die Evidenz für die ERNTE**, sobald ein Folger-Pfad existiert. Ausdrücklich KEIN Weg: die Feder dünner rechnen, ein Gate lockern, den Wächter abschalten, den Geltungsbereich auf `gross` erweitern, oder die Lesart von Gate (c) nach der Zahl auswählen | (1)–(3) je eigene Pre-Reg, (1) zuerst; (4) steht schon in §7.11; (5) hinter dem Folger-Pfad. **Der Rasterzweig ist für die Bilder im Repo geschlossen** (beide gemessen); eine GRÖSSERE IIIF-Ableitung der Tafel gibt es NICHT (Recherche 2026-09-07 auf den Autor-Entscheid zur Sammelfrage 4, „recherchieren, dann entscheiden"): der Commons-Scan der Abb. 19 (1756 × 1783 px, aus dem `words-abb19.png` byte-identisch stammt) hat in seiner Dateihistorie nur diese eine Version, die SUB Hamburg liefert die Ausgabe 1926 per IIIF mit höchstens 2278 × 3040 px je Seite und gröberem Zeilenabstand (104 gegen 110 px), der DNB-BookReader der Ausgabe 1922 gibt bei `reduce=1` 1838 × 2486 px je Seite und rechnet alles darunter serverseitig hoch (gemessen: 4×-Zoom gegen die hochgerechnete Vollseite identisch, Restdifferenz 3 Graustufen), HathiTrust, Internet Archive und Google Books führen keinen Scan, die DDB kennt nur die Hamburger Exemplare. Offen bleibt allein ein EIGENER Scan eines physischen Exemplars (1917/1922/1926, 600–1200 dpi) — Quell-Commit plus Re-Export, also eine deklarierte Re-Baseline und ein Autorenschritt, kein Arm. **Autor-Entscheid vom 2026-09-09 (A41): dieser Scan kommt NICHT** — wörtlich „glaub eher nicht, haben ja dann meine geschriebenen Wörter". Die höher aufgelöste Quelle für die Binnenflächen ist damit die Eigenhand-Ernte ([`eigenhand-erfassung.md`](eigenhand-erfassung.md)) und nicht eine bessere Aufnahme der 1922er Tafel; der Auflösungs-Zweig ist geschlossen, die fünf Rettungswege links davon sind unberührt. Der Datierungs-Nebenbefund (Commons-Dateiseite 1917 gegen `SOURCE.md` 1922) bleibt als Provenienz-Notiz stehen — für einen Kauf ist er jetzt gegenstandslos |
| **Übergänge J6 vor dem Auge** (`sep08`, humanbench-Runde 7, Rettungsweg (5) der Zeile darüber) | **Das Menschenurteil entscheidet den Arm nicht — und genau das ist der Befund.** Basis 14 : Kandidat 9 von 23 entschiedenen (**39,1 %** gegen ≥ 60 %) bei **63,5 %** Unentschiedenen (Schranke ≤ 25 %); klassenweise löst sich nichts auf, `naht-stark` steht 9 : 10 bei 34,5 % Ties und `naht-schwach` 0 : 4 bei 85,7 %. Das Instrument ist sauber: 12 Wiederholungspaare, 10/12 gleicher Arm, die 7 Seiten-Übereinstimmungen SIND die 7 Doppel-Ties (kein Paar nannte zweimal dieselbe Seite, während es eine nannte), die zwei Uneinigkeiten sind nie Basis gegen Kandidat, und die Nullprobe antwortet 6/6 richtig. **Der gemessene Grund ist die Dosis:** der Arm bewegt die Zeichnung im Median um 0,0221 xh und höchstens um 0,0681 — ein Fünftel dessen, was Runde 5 bewegt hat, deren starke Klasse bei 0,1186 xh begann und mit 9,7 % Ties entschieden wurde; entschiedene Bildschirme tragen 0,0311 xh Versatz, unentschiedene 0,0146 (innerhalb der starken Klasse trennt das kaum noch: 0,0382 gegen 0,0316, deshalb Beobachtung und keine Schranke). Autor wörtlich: „nicht besser nur bischen weniger wellen mal da oder dort". **Damit ist J6 zweimal gefallen** — `sep06` an Gate (c), weil die Dosis auf den Zwei-Buchstaben-Übungen `dp`/`ds` zu GROSS ist, und `sep08` am Auge, weil sie überall sonst zu klein ist, um gesehen zu werden | (1) **die identische Punktmenge für den Unstetigkeits-Sensor, jetzt ZUERST** — Weg (4) der Zeile darüber, unverändert im Mechanismus und neu in der Dringlichkeit: wenn das Auge bei dieser Dosis nicht entscheidet, ist S2 das einzige Lineal, das die Größe des Autors misst, und sein +12/+11 bleibt unzurechenbar, solange `cont_measured` mitwächst; (2) **J6c über den ganzen Bogen** (Weg (2) darüber) — **neu: mit einer vorregistrierten Sichtbarkeits-Vorbedingung.** Wer eine Klasse „sichtbar" nennt, nennt vorher den Versatz, bei dem er das behauptet; der gemessene Anhalt liegt zwischen 0,0221 xh (34,5 % Ties) und 0,1186 xh (9,7 %); (3) **der Deckel je Klasse aus den Vorkommen** (Weg (3) darüber) — er senkt nicht nur den Verbinder-Anteil dort, wo der Haken entsteht, er vergrößert auch den Versatz dort, wo die Regel am meisten zu tun hat; (4) **J6b bleibt ZU** — die Runde war seine Bedingung und ist unter 60 % ausgegangen; Wiedervorlage nur als frischer Arm mit frischer Pre-Reg, sinnvoll erst hinter (1); (5) **die Naht ZEIGEN statt sie im Wort zu verstecken** (neues Instrument): ein Ausschnitt je NAHT statt je Wort, dieselben Arme, dieselbe Frage — gehört als Instrument-Änderung VOR der Runde nach `menschliche-bewertung.md` §8a, und ihre Zahlen sind mit denen der Wortrunden nicht vergleichbar. Ausdrücklich KEIN Weg: die Schranken senken, die Klassen nachschneiden, bis `naht-stark` 60 % erreicht, oder dieselbe Runde ein zweites Mal vorlegen — das misst den Beurteiler, nicht die Regel | (1) zuerst und Vorbedingung für alles Weitere; (2)/(3) je eigene Pre-Reg, (3) vor (2), weil sie beide Fehlschläge zugleich adressiert; (4) gesperrt bis zu einer eigenen Vorregistrierung; (5) erst nach der Instrument-Änderung, eigene Pre-Reg |
| **Laufform-Schreibliste der Chart-Saat** (`sep08`, humanbench-Runde 8) | **Das Auge verwirft die Karte als Ganzes, und diesmal auch der Richtung nach:** Basis **20** : Kandidat **13** von 33 entschiedenen Bildschirmen (39,4 % gegen die vorregistrierten ≥ 60 %) bei **47,6 %** unentschieden (gegen ≤ 25 %) — beide Schranken, anders als bei J4, wo nur die Tie-Schranke fiel. Die Runde trägt den Anspruch (10/12 gleicher Arm bei 6/12 gleicher Seite, 12 ≥ 6 Paare) und ihr Instrument ist sauber (Nullprobe 6/6 richtig als „kein Unterschied"). **Der vorregistrierte Rückfall greift nicht:** die Liste ohne `Z` ist exakt lesbar — `Z` berührt nur `Zaum` `Zorn` `Zügel`, die übrigen 60 Bildschirme SIND die 14-Zeilen-Karte — und ändert 39,4 % / 47,6 % auf **38,7 % / 48,3 %**; `Z` trägt ein Drittel des LINEAL-Verlusts und ist am Auge die unauffälligste Zeile der Karte (1 : 1 bei einem Tie). **Die Decke liegt in der Zerlegung je Zeile** (nachgereicht, nicht vorregistriert): genau eine der 15 räumt beide Schranken, `d` mit 10 : 2 = 83,3 % bei 14,3 % Ties, und ohne die vier `und…`-Wörter sogar 10 : 0 ohne Tie; dagegen `h` **0 von 8** und `n` 3 von 15, beide über dem Klassenboden. Gegenbefund zum Lineal, je Zeile beziffert (§9a `menschliche-bewertung.md`): `p` (3 : 0 besser, Median −0,01013) und `w` (4 : 2, −0,00472), die zwei vom Wort-Lineal am stärksten belohnten Zeilen, lehnt das Auge glatt ab (0 von 2 bzw. 0 von 6), während es die vom Lineal bestrafte `d`-Zeile trägt | (1) **die `d`-Zeile als EIGENER Arm** — frische Ernte auf der heutigen Wurzel `ccb036a5eb20…`, ein Overlay aus nur dieser Zeile, eigene kurze Runde über ihre 14 Wörter; erst das macht aus der bedingten Lesart ein Verdikt (§7.11); (2) **die Zwei-Stufen-Saat** (LF15-Rettungsweg, §7.11) — sie ändert die Zeilen selbst und nicht nur ihre Auswahl, ist also ein neuer Kandidat und kein weicheres Gate; (3) **mehr Evidenz je Zeile** — `c` `w` `g` `longs` `p` `Z` scheitern an der Wortzahl (n < `MIN_PAIRED_PER_CLASS` = 8), nicht am Urteil; Weg ist die laufende Eigenhand-Ernte oder eine zweite Platte derselben Hand; (4) **der Unstetigkeits-Sensor S2** (`sep06`) über die `d`-Wörter — er hat noch keine Laufform-Runde gesehen und wäre das erste Lineal für das, was der Beurteiler an der Zeile sieht. Ausdrücklich KEIN Weg: die Schranken senken, die Nullproben aus der Tie-Quote herausrechnen, oder die `d`-Zeile aus DIESER Runde heraus schreiben | (1) braucht das „ja" des Autors zur Teil-Schreibliste und eine eigene Pre-Reg; (2)/(4) je eigene Pre-Reg; (3) läuft mit der Eigenhand-Erfassung |
...
| **Saat-Korrespondenz als VORKOMMENS-Quelle** `harvest --occurrences tintenpfad` (`sep13`, A48, Weg 1 der §7.11-Zeile „Vorkommen aus der Tintenpfad-Bahn") | **Die Zuordnung trägt, die Zeile nicht:** die Korrespondenz beweist sich auf **277 von 277** Slots (Scheiben-Rest ≤ 1e-13, kein Slot ohne einen einzigen gedeckten Anker) und setzt **92,5 %** aller Anker (30 736 von 33 240) — sie ist damit kein Schätzer, sondern eine Ablesung. Sie scheitert an der GANZEN Zeile: die Komposition schneidet die Kopplungs-Enden weg, über alle Slots fehlen **7,5 %** der Anker (2 504 von 33 240; im Median-Slot 7 von 120, den 212 unvollständigen 9), und weil ein Vorkommen ein VOLLER Ankersatz ist, bleiben **58 von 277** statt 227. Wo beide Wege einen Satz liefern, sind sie nicht dasselbe: Anker-Abstand Median **0,0542 xh**, p90 0,1436, max 0,5235 — weit über dem Ableseboden, der Arm misst also wirklich etwas anderes | (1) **die geschnittenen Enden FITTEN statt sie auszulassen** — Weg (2) der §7.11-Zeile (`core.fit` im Spann-Fenster, nur für die ungedeckten End-Anker, die gedeckten bleiben Ablesung): er adressiert genau die 7,5 %, die diesen Arm zu Fall bringen, und erbt seine Korrespondenz als Randbedingung; (2) **die Komposition Buch führen lassen** — ein Abtastindex je Mittellinien-Punkt unter `provenance=True` in `core/compose.py` macht das eine geschätzte Glied der Kette exakt und hebt die Deckung dort, wo heute der stückweise Anstieg (`d`) und die Vorlauf-Vertices den Beweis kosten; ein Autor-Entscheid, weil er `core/compose.py` anfasst; (3) **die Zeile aus den gedeckten Ankern allein** — nur zulässig mit einer vorab erklärten Umdefinition dessen, was eine Laufform misst, und daher ausdrücklich NICHT als stiller Ausweg. **Randbedingung für (1) und (2), weil beide die Korrespondenz weitertragen:** `identify_slice` rangiert die Kandidaten-Verschiebungen nach lokalem Residuum und nimmt bei Gleichstand die erste — bei einem Zug mit wiederholter Teilform könnten zwei Verschiebungen beide beweisen und die Anker woanders landen. Auf dieser Wurzel nicht beobachtet (Reste 1e-15, weit unter dem nächsten falschen Versatz), aber dieselbe Fehlerklasse, für die die Vorregistrierung die „Nächster-Punkt-Suche" abgelehnt hat: wer darauf aufbaut, bringt einen Sensor dafür mit. Kein Weg: Anker über die Bogenlänge verteilen, den Vollständigkeits-Anspruch aufweichen, oder die Deckung durch eine lockerere Beweis-Schranke kaufen | (1) zuerst, eigene Pre-Reg; (2) Autor-Entscheid; (3) nur mit erklärter Umdefinition |
| **Eigenhand-Eingabe A3 Saat-Registrierung** `--register-seed` (`sep24`, Folger-Eingabe-Leiter, Sprosse 3 — **R-gate**) | **Die Hand gewinnt, die Platte zahlt:** auf den 7 Kästen von B0001 hält A3 alle drei Bedingungen (7 von 7, cov-Median +0,2481 gegen A2, Züge 42 → 25, Absetzer 36 → 10), reißt aber den Platten-Wächter am aiou-Median (0,7929 → 0,7895, bei dtw 0,038351 → 0,036680) — nicht an der Katastrophe (`mit`, k 7,71, sy 0,129), sondern am Medianwort `die-2`; je Wort 8 besser, 10 schlechter, 1 gleich. Die Bereichsgrenzen des Werkzeugs retten den Wächter nicht (0,038371 / 0,7895) | **R-gate:** Stufe 3 NUR auf Bogen-Eingabe (ein Kasten mit gedruckter Lineatur), nie auf einem Plattenwort — der Platten-Wächter wird damit Byte-Identität per Konstruktion wie für A1/A2. Ehrliche Kehrseite, im Eintrag festzuhalten: das Tor entzieht Stufe 3 dem Platten-Lineal, statt sie dort als harmlos zu beweisen; `sep24` zeigt sie dort etwa neutral (−0,0034 am Medianwort), und diese Zahl wird weiter berichtet, nie verschwiegen | vorregistriert: §14 „Stufe 3 gezielt: R-gate + R-split `sep24b`" (eigener Commit vor jedem Arm); **gebaut** (Arm-Commit `d50f886`, `origin` `eigenhand:`) und in (a) gemessen (`sep25`): G1 63/63 byte-gleich, G2 63/63 · 19/19 — der Wächter hält per Konstruktion; die ungetorte Plattenzeile wird weiter berichtet (R-k 0,7929, R-ky 0,7895) |
| **Eigenhand-Eingabe A3 Saat-Registrierung** (dieselbe Sprosse — **R-split**) | **sy ist vermutlich Schätzer-Rauschen, k der Träger:** auf Plattenwörtern mit sy = 1,0 per Definition liest der Moden-Schätzer 0,13–1,37 (`mit` 0,13 und `mit-2` 0,57 mit feuernder Bereichsgrenze; ohne die beiden 0,74–1,37); auf dem Bogen 0,74 neben 1,00 in EINER Sitzung; die Kalibrierung R = 9/14 ist selbst ein Warnzeichen. k (Tinten- gegen Saatbreite) ist direkt gemessen und liegt überall bei 0,70–0,98 — die Saat von 1922 ist für diese Hand zu breit. Ob ein hohes sy den Platten-Median kostet, ist eine Hypothese: von den acht Wörtern mit sy > 1,15 fallen vier (`die-2`, `das`, `laden` bei 1,35; `will`) und steigen vier (`zwei` bei 1,37, `die`, `Wer`, `Galoppieren`) — das prüft erst R-split | **R-split:** die beiden Skalen als eigene Sprossen — erst k allein (R-k), dann sy + Grundlinie darauf (R-ky); dazu ein **Sensor** als Berichtsgröße, den das Lineal nicht hat: Bahn-Unstetigkeit und Kurzsehnen (das Zittern entlang der Schäfte und die Sehnen unter 0,2 xh, die das Blindurteil an A3 sah) | vorregistriert mit R-gate in `sep24b` (Sprossen R-k, R-ky), der Sensor dort nur berichtend; **(a) gemessen `sep25`: k trägt den Gewinn** (R-k gegen C0 7 von 7, +0,2481), sy + Grundlinie tragen auf der Hand nichts (R-ky gegen R-k 2 von 7, Median-Δ 0) und kosten auf der Platte den aiou-Median (0,7929 → 0,7895, k allein 0,7929); (b) offen, nominiert Plattenmaßstab + R-k |
| **Eigenhand-Eingabe R-ky: sy + Grundlinie** (`sep24b` (a), Sprosse 3 — schlägt R-k nicht) | Auf den 7 Kästen ändert die vertikale Registrierung über k hinaus fast nichts: 2 von 7 besser, Median-Δ genau 0 (*things*, *doctor* lesen sy = 1,000 und sind dieselbe Bahn), schlechtester −0,0048; ungetort auf der Platte kostet sie das Medianwort `die-2` (0,7929 → 0,7895). Der Moden-Schätzer liest auf Plattenwörtern mit sy ≡ 1 0,13–1,37 — die Zahl ist auf dieser Hand nicht von Rauschen zu trennen | (1) **R-sy-Schätzer:** ein anderer x-Höhen-Schätzer (etwa Mittelband-Histogramm der Tinte gegen die komponierte Saat), vorregistriert zuerst gegen die Plattenwörter mit sy ≡ 1 als Sensor (Streuung muss deutlich unter 0,13–1,37 fallen), erst dann auf der Hand; (2) **R-price:** ty ±4 px ist auf 31 px/xh schon 0,13 xh — ob die eingefrorene Registrierung den Grundlinien-Versatz (3–5 px) bereits auffängt, zeigt ein Plattenarm, der ty in x-Höhen ausdrückt; (3) **eine zweite Sitzung in (b):** sy kann erst dort etwas tragen, wo die geschriebene x-Höhe messbar von der gedruckten abweicht | offen, je eigene Pre-Reg; kein Weg ist „R-ky mit weicheren Gates" |
| **Eigenhand-Eingabe A1 und A2** (`sep24`, Sprossen 1–2 — „nicht besser" nach der Regel) | A1 schlägt B0 nicht (4 von 7, +0,0023) — eine Maske kann keine Tinte zum Decken hinzufügen; sie nimmt fremde weg (off 0,0226 → 0, Sehnen 2 → 0) und ist seit dem Autor-Entscheid vom 2026-09-24 trotzdem Vorgabe. A2 schlägt A1 nicht (6 von 7, +0,0237); die Registrierung (ty ±4 px) sitzt auf dem nativen Streifen in 12 von 14 Zeilen am Anschlag, bei 31 px/xh in 4 von 7 | für A2: **R-price** (die Pixelpreise und -konstanten in x-Höhen statt Umtasten, ein Plattenarm unter den dev-19-Gates, Lotse-Präzedenz v0.18/v0.19) · **R-reach** (weitere Reichweite der Buchstaben-Affinität auf dem Streifen); stehend aus der Vorregistrierung außerdem R-slant · R-grid · R-truth · R-forms · R-loops | je eigene Pre-Reg |
...
| **Vorkommens-Saat** | §14 „Laufform LF15 `sep06`" (Kandidat (c), nicht gebaut) | die dritte vorregistrierte Saat: aus dem gespeicherten Vorkommen desselben Slots statt aus einer Komposition; näher an der Tinte, aber mit eigener Rückkopplung (Vorkommen und Zeilen stammen aus demselben Lauf) | offen; erst wenn die Zwei-Stufen-Saat die 9 Vorkommen nicht holt |
| **Die vier J6-Konversionen** | §14 „Übergänge J6 `sep06`" und „J6 `sep08`", §7.9 | die Naht ist an beiden Enden zu (Abgang 2,67° → 0,01°, Ankunft 10,80° → 0,01° über 240 Nähte, Platzierung unberührt) — und die Rechnung steht auf drei Zwei-Buchstaben-Übungen, weil der Verbinder dort 33° in 0,3 xh wegdrehen muss und sich neben die eigene Tinte des `d` legt. **Seit `sep08` kommt der zweite Fehlschlag dazu:** Runde 7 ist geurteilt und entscheidet den Arm nicht (39,1 % Kandidat bei 63,5 % Unentschiedenen), weil er die Zeichnung nur um 0,0221 xh im Median bewegt | offen, neu geordnet nach Runde 7: (1) **die identische Punktmenge für den Unstetigkeits-Sensor ZUERST** — ohne sie hat kein Nachfolger ein Lineal, seit das Auge bei dieser Dosis nicht entscheidet · (2) den **Deckel je Klasse** aus den Vorkommen messen (senkt den Verbinder-Anteil dort, wo der Haken entsteht, und vergrößert den Versatz dort, wo die Regel arbeitet) · (3) **J6c**, den Verbinder-Anteil über seinen ganzen Bogen verteilen — mit einer vorregistrierten Sichtbarkeits-Vorbedingung, weil eine Runde unter ~0,02 xh Versatz nichts entscheidet · (4) **J6b ist ZU**: Runde 7 lizenzierte ihn nur bei ≥ 60 % und ging mit 39,1 % aus; Wiedervorlage nur als frischer Arm mit frischer Pre-Reg. Daneben neu: **eine Runde, die die NAHT zeigt statt sie im Wort zu verstecken** — eine Instrument-Änderung, die vor der Runde nach `menschliche-bewertung.md` §8a gehört |
| **Distanzfeld-NUR-Claim** | §14 „Kette K-E2 `aug21`", jetzt auch „Kette K-E `sep09`" | die Claim-Familie ist geschlossen, dieser Ein-Kanal-Schnitt aber nie versucht. **Seit `sep09` ist er der einzige verbliebene Weg der Familie** — Rettungsweg (1), die Urteilsrunde, ist gegangen und hat nichts entschieden | offen, frische Pre-Reg — **neu mit der Sichtbarkeits-Auflage der Runde 9**: wer eine Klasse „sichtbar" nennt, nennt den Versatz UND den Rahmen, in dem er sichtbar wäre (Runde 9 hat bei 0,24 xh nichts entschieden) |
| **Die Saat-Distanz als Kette-Arm** (der Composer-Auftrag e-Breite bekommt einen Adressaten) | §14 „Kette K-E `sep09`" (Rettungsweg 1), Mechanismus in §7.2 und in der K-C-Autopsie `aug20` | Runde 9 hat gefragt, welche Bahn der Tinte besser folgt, und der Beurteiler hat zurückgefragt, warum dasselbe `er` einmal gut und einmal „richtig richtig schlimm" aussieht. Die Antwort steht seit `aug20` gemessen da: wo die komponierte Saat um mehr als `max_delta` (0,75 xh) danebenliegt — `unter`: komponiertes `e` 1,32 xh gegen 0,65 der Hand, Saatfehler am `e`-Austritt 0,81 —, quetscht der Kettensolver die Bahn auf die falsche Tinte; in `Wer` passt dasselbe Paar, weil das `e` der Hand dort 0,81 xh breit ist | offen, eigene Pre-Reg: ein Arm, der die SAAT verändert (e-Breite im Composer oder eine `max_delta`-treue Saat je Slot), gemessen an den Papier-Umkehren der beiden Bahnen und an einer eigenen Runde. Kein Folger-Knopf — die Formulierung ist seit Arm ⑨ ausgemessen |
...
| **Rausch-Boden des Folgers** (sperrt jeden weiteren Init-Arm) | §14 „Kette K-F `sep04`", jetzt auch „Kette K-G `sep09`" | K-F hat unfreiwillig eine Nullprobe gefahren: 23 Wörter bekamen einen Init, der um höchstens 1,8·10⁻¹⁵ abwich, und trotzdem kippten 9 Wächter-Verdikte mit aiou −0,0298 … +0,0800 und 4 Verlierern unter der 0,003-Schranke. Solange dieser Boden nicht beziffert ist, kann kein Init- oder Startpunkt-Arm WORTWEISE entschieden werden. **K-G hat die Sperre `sep09` gegen sich selbst geprüft:** seine Störung (0,047–0,615 xh) liegt fünfzehn Größenordnungen über dem K-F-Rauschen, der Arm ist also als SET-Aussage entscheidbar — die Wortaussage bleibt gesperrt, und die Runde hat kein Wort einzeln adoptiert oder verworfen | offen für WORTWEISE Aussagen: den Boden mit einer erklärten Null-Störung messen; danach (a) naht-verankertes Abspielen, (b) das Wächter-Verdikt als Messgröße statt Etikett — K-G liefert die erste Eichung (Wechsel auf `revert-init` kostet im Median 0,077 aiou, der Wechsel davon weg bringt 0,103), (c) die Wiedervorlage von K-F; je eigene Pre-Reg |
| **Bedingte Saat-Registrierung** (Konversion 1+2 von K-G) | §14 „Kette K-G `sep09`", §7.9 | `--chain-seed grid` heilt, wo die Saat danebenliegt (`unter` +0,1072 aiou, 5 → 0 Papier-Umkehren), und schadet, wo nicht (unterstes Spannweiten-Terzil 5 → 8); dazu `regieren`, das gemischt startet, weil sein drittes `e` an der Blockschranke liegt und der Schalter solche Slots auslässt — als einziges von VIER gemischt startenden Wörtern schlechter, die Reparatur ist also naheliegend und nicht bewiesen | offen, frische Pre-Reg: (1) eine vorab benannte Schwelle, unter der ein Slot KEINEN Seed bekommt; (2) den Schranken-Slot klemmen statt auslassen — je eine andere AUSWAHL desselben Knopfs, also ein neuer Mechanismus und kein weicheres Gate |
...
| **Zug-übergreifende Schleifen in `loop_ranges`** (was das `t` braucht) | §14 „Kringel-Landmarke `sep06`" (Rettungsweg 2), vermessen in „Kette R3 `sep07`" | **Strukturell, und gemessen:** `loop_ranges` sucht Selbst-Kreuzungen JE PEN-ZUG, das `t` schließt seine Binnenfläche aber zwischen SEINEN ZWEI ZÜGEN — Tafelzeile 120 Anker, `stroke_starts [0, 83]`, Schleifen je Zug `[[], []]`, beide zusammen ein Loch `D0 = 0,2475`, umspannt von den Ankern **66..109**, quer über die Zug-Grenze. Ein Bereich über eine Feder-Hebung ist im heutigen Modell nicht ausdrückbar; `align_loops` müsste erst entscheiden, was er dort bedeutet | offen, eigene Pre-Reg — **vor dem nächsten Laufform-Write**: ohne ihn bleibt `t`#1 (0,0517 gegen 0,3912), das größte Defizit des Katalogs und der einzige Verlust auch bei 0,0724, für LF13/LF14 unsichtbar |
| **Ein Folger-Pfad für die Ernte** | §14 „Kette R3 `sep07`" | Der Weg von einer korrigierten Kette-Bahn zur Laufform-ZEILE ist zu: die Ernte fährt ihre eigenen Kettenfits, und §3 verbietet ihr einen Folger-Pfad. Ohne ihn erreicht kein Bahn-Arm die Kennzahl der 24 schließenden Wörter | offen, eigene Pre-Reg; **hinter dem Fixpunkt-Arm** |
| **R4b — die enge Klasse der Feder-Entfaltung** | §14 „Kette R4 `sep07`" (Rettungsweg 1) | R4 hat den Mittelachsen-Term gemessen: er bewegt als erster die richtige Klasse (8 der 33 zu-Schleifen gehen auf, R3c dort 0) und verliert dabei drei vorher offene Schleifen ganz. Auf die 33 verengt kann er die 124 offenen per Konstruktion nicht mehr anfassen — dort bewegt er ohnehin nur +0,0198 in eine Weite, die sie schon hatten | offen, eigene Pre-Reg; Eichung liegt vor: 157 Zahlen aus R4, 156 aus R3c, 106 aus R3 |
| **Ein glattes Evidenz-Feld statt eines Rasterbandes** | §14 „Kette R4 `sep07`" (Rettungsweg 2) | R4s korrigierte Evidenz ist ein ein Pixel breites Band, also erbt der Attraktor das ±0,5-px-Raster der Platte — daher die 34 neuen Knicke und die Treffgenauigkeit von nur 36 von 157. Dieselbe Niveaulinie sub-pixel ausgewertet und mit `DIST_FIELD_SIGMA_PX` geglättet ist R3cs eigene Konvention, auf die Evidenz übertragen | offen, eigene Pre-Reg; zielt auf Gate (a) und (d) zugleich, sinnvoll erst nach oder mit R4b |
| **Die drei Totalverluste von R4** | §14 „Kette R4 `sep07`" (Rettungsweg 3) | `unter` `t`#1, `Kugel` `K`#1 und `macht` `a`#0 fallen von 0,22–0,26 auf den Ableseboden 0,0025, verlieren also ihre Schleife GANZ statt sie zu verengen. Scharfe Signatur, drei Fälle, und eine andere Ursache als eine zu enge Weitung | offen: erst als Zerlegung messen (welcher Wächter-Ausgang, welcher Bogen wurde ersetzt), dann ein Arm |
...
| ~~**ß-Dekoder-Regel**~~ (Route Tintenpfad) | §14 „Tintenpfad-Arme `sep11`", §7.9; `sep12` als `ride_back=1` gebaut und mit eigener Pre-Reg gemessen (PR #595, [`../reference/verfahren-tintenpfad.md`](../reference/verfahren-tintenpfad.md)) | dev-19 3 : 0 : 16, muß ×3 0,11 → 0,03…0,05, `retrace_missing` 1 → 0; feuert auch an den Großbuchstaben-Stämmen `Pulver`/`Einen` | **entschieden 2026-09-13 (A46): so lassen, kein Kleinbuchstaben-Gate.** Gebaut, in der Ecken-Kombination; Autor-Entscheid zu den Großbuchstaben-Stämmen erledigt (§14 „Tintenpfad-Arme `sep11`" Nachtrag) |
| **Grauwert-Stopp der Spitzen-Lesung** (Route Tintenpfad) | §14 „Tintenpfad-Arme `sep11`"; `sep12` als `tip_grey_stop=1` gebaut (PR #595) | Papier-Strecke grau 1,34 → 0,72 auf 13 Zeilen, 12 : 5 gegen #592, alle Gates; reicht bis in die Haken-Walks | **gebaut, in der Ecken-Kombination** |
...
| **Vorkommen aus der Tintenpfad-Bahn** (Route Tintenpfad) | §14 „Tintenpfad-Adoption `sep12`", jetzt „Laufform A48 `sep13`" | Der Ernte-Schalter bewegt nur die BAHN. Eine Laufform-Zeile ist ein ANKERSATZ (fester Anker-Index je Tafelzeile, über den medianisiert wird), der Tintenpfad dekodiert eine BAHN ohne Anker. Zwischen beiden fehlt eine Zuordnung; eine per Bogenlänge erfundene würde still umdefinieren, WAS eine Laufform misst — und zwar unsichtbar, weil jede Zahl weiter plausibel aussähe | **Weg (1) ist gemessen und geschlossen** (Autor-Entscheid **A48**, `sep13`, §14 „Laufform A48 `sep13`"): die **Saat-Korrespondenz** trägt als ZUORDNUNG — sie beweist sich auf allen 277 Slots und setzt **92,5 %** der Anker —, aber nicht als VORKOMMEN: die Komposition schneidet die Kopplungs-Enden des Buchstabens weg, also haben im Median **7 von 120** Ankern dort gar keine Stelle, und die Ganz-oder-gar-nicht-Regel einer Zeile lässt 58 von 277 übrig (Fit-Basis 227). Ehrliches Negativ an Gate (G); Rettungswege in §7.9. **Offen bleiben (2)** die Tafel-Anker an die geschnittene Spanne FITTEN (`core.fit` im Spann-Fenster) — der Weg, der genau die fehlenden Enden adressiert, und nach A48 der nächste — **und (3)** die Statistik beim Buchstabenfit lassen, der heutige Stand. Gate für (2) unverändert: die Ernte muss Fixpunkt bleiben (zweiter Lauf byte-gleich) |
| **`k0eval` soll die Folger-Identität lesen** (Lineal) | §14 „Tintenpfad-Adoption `sep12`" (Nachtrag 2026-09-13, A47) | Die sechs `STACK_FLAGS` kommen aus `meta.weights` der ersten Zeile; der Tintenpfad hat keinen Struktur-Wächter, also lesen sie dort `None` und der Sensor meldet bei JEDEM Folger-Wechsel Abweichung — er sagt damit nichts, wo er am meisten sagen müsste. **Entschieden 2026-09-13 (A47):** die Buchung von Gate (4) der Adoptionsrunde als „nicht lesbar" bleibt, das Verdikt ruht auf den Gates (1)–(3) | offen, eigener Arm mit eigener Vorregistrierung und eigenem PR: ein FOLGERBEWUSSTES Stack-Kriterium — der Sensor muss erkennen, dass zwei verschiedene FOLGER verglichen werden, und dann die Flags des jeweiligen Folgers lesen (oder die Prüfung ausdrücklich als „nicht anwendbar" melden) statt einer Warnung, die nichts bedeutet. Kleine Tool-Änderung, aber am LINEAL — nicht während einer Runde ändern (Frozen-Ruler-Regel) |
| **Stufe 3 gezielt: R-gate + R-split** (Streifen-Eingabe) | §14 „Folger-Eingabe-Leiter `sep24`", §7.9 | A3 hebt die Hand 7 von 7 und reißt den Platten-Wächter (−0,0034 am Medianwort); sy liest auf der Platte 0,13–1,37 (`mit` 0,13 und `mit-2` 0,57 mit feuernder Bereichsgrenze; ohne die beiden 0,74–1,37) | **vorregistriert** (§14 „… `sep24b`"); **(a) gemessen `sep25`** (§14 „Stufe 3 gezielt `sep24b`, Stufe (a)"): R-k besteht, R-ky schlägt R-k nicht — nominiert für (b) ist das Paket Plattenmaßstab + R-k. (b) wartet auf S0182–S0188 vom nächsten Bogen B0005 — gezogen ist seit dem 2026-09-21, Schlüssel `mn-suetterlin-2026-09-21` (berichtigt 2026-09-24); danach der Blindlauf des Autors auf dem Paar, das in (b) besteht |
| **R-price auf dem Streifen** | §14 „Folger-Eingabe-Leiter `sep24`", Beob. 2 | ty ±4 px nativ in 12 von 14 Zeilen am Anschlag | offen, eigene Pre-Reg (Plattenarm, dev-19-Gates) |
...
| **humanbench-WORT-Runde** | §14 „Methodik-Lücke `aug16`", K-E1/K-E2 | der einzige benannte Tie-Breaker für ruler-indifferente Fälle | **erste Runde gefahren `sep02`** (Basis vs. LF11, Fassung A2): das Instrument steht und war verlässlich (10/12), ein formales Verdikt trug die Runde aber nicht — der Unentschieden-Anteil lag mit 34,9 % über der Schranke von 25 % (§14 „Laufform LF11 — humanbench-Wortrunde"). **Für K-E ist sie am `sep09` gefahren** (Runde 9, Autor-Entscheid A2; §14 „Kette K-E `sep09`") — Basis = Produktions-Kette v5, Arm = `--mark-claim`, gefolgte BAHNEN statt Kompositionen, also die Genauigkeitsfrage und die Mittellinien-Anzeige der Buchstabenrunden. **Sie hat den Tie-Breaker nicht gebrochen:** 44 von 44 Bildschirmen „kein Unterschied", kein entschiedener Bildschirm, `adopt: false` — der Modus kann einen Fall entscheiden, aber nicht diesen, weil beide Arme dieselbe dominante Störung tragen. Damit ist der Schritt als AUTORENSCHRITT erledigt; was offen bleibt, ist die Instrument-Frage (§7.9, Weg (3): den Ort zeigen statt ihn im Wort zu verstecken) |
...
| ~~**Chart-Saat als Default**~~ | §14 „Laufform LF15 `sep06`", jetzt „Laufform LF16 `sep07`" | sie ist der einzige gemessene Ernte-Fixpunkt, aber ihre Übernahme leitet Zeilen um mehr als 0,03 xh neu ab — ein Re-Baseline-Write, und der ist Autorensache | **entschieden 2026-09-07 (A38): ja.** `chain_seed="chart"` ist der Ernte-Default (`tools/laufform/harvest.py`); Trace-Bench und Folger behalten `composed`. Der WRITE der chart-gesäten Zeilen bleibt getrennt und hängt an der Zeile darunter |
| ~~**Runde 8 urteilen — und danach der Write der 15 Zeilen**~~ | §14 „Laufform LF16 `sep07`", jetzt „Laufform LF16 `sep08`" | die Adoption der Saat ist eine Aussage über die Reproduzierbarkeit der Ernte, der Write eine über die FORM der Zeilen; das Wort-Lineal ist mit 0,109026 → 0,112123 gegen den Kandidaten, und dieselbe Konstellation hat `sep06` den Austritts-Trim getragen (Runde 5, A37) | **Geurteilt 2026-09-08, kein Write.** Basis 20 : Kandidat 13 von 33 entschiedenen (39,4 % gegen ≥ 60 %) bei 47,6 % unentschieden (gegen ≤ 25 %) — anders als bei J4 zeigt auch die Richtung zur Basis, die Runde ist dabei verlässlich (10/12) und die Nullprobe 6/6 sauber. Die vorregistrierte Ohne-`Z`-Liste ändert das nicht (38,7 % / 48,3 %). Der Write der Karte ist damit erledigt und findet nicht statt; **offen ist nur noch die `d`-Zeile** — die Zeilen-Zerlegung gibt ihr 83,3 % bei 14,3 % Ties und der Autor hat sie von sich aus genannt. Sie braucht sein „ja" und einen eigenen Arm (Zeile in der KI-Tabelle oben), nicht diesen Write |
| ~~**Runde 10 urteilen — und danach der Write der EINEN `d`-Zeile**~~ | §14 „Laufform LF17 `sep09`" (Autor-Entscheid A40), jetzt „Laufform LF17 `sep10`" | ein Laufform-Write ist ein Autorenvorgang und eine deklarierte Re-Baseline; und die Runde kann sich per Konstruktion keine Adoption selbst erteilen (26 bit-identische Nullproben sind ein Tie-Boden von 65 %, nötig für die blinden Wiederholungen) | **Geurteilt 2026-09-10, und geschrieben (A44).** 48 von 48 Bildschirmen in einem Zug, 177 s, ohne Notiz. `d-rein` **0 : 10 bei 0 % Ties**, `nullprobe` 26/26, 8/8 Paare gleicher Arm — der vorregistrierte Auslöser ist gefallen, und der Autor hat noch am selben Abend die eine Zeile geschrieben (Snapshots davor/danach, Readback-Abweichung 0,000000). Die Re-Baseline danach trägt die Wort-Headline 0,108339. Offen bleibt allein der `d-und`-Gegenbefund (Zeile in der KI-Tabelle oben) |
| **B0005 schreiben und einlesen** (die sieben Referenzstreifen S0182–S0188) | §14 „… `sep24b`" (Fassung 3a), „Trainingssatz `sep20`" (Nachtrag 2026-09-24) | Stufe (b) braucht ungesehene Kästen, die die Ziehung vom 2026-09-21 in `practice` gelegt hat, und die sieben Referenzwörter stehen erst auf dem nächsten Bogen; B0002–B0004 sind seit dem 2026-09-08 gedruckt und unterwegs und tragen sie nicht. **Nichts zu ziehen:** gezogen ist seit dem 2026-09-21 (`mn-suetterlin-2026-09-21`, Archiv-Commit `9efe356`) — `training_set --draw` wird für diese Hand nie wieder aufgerufen | offen: B0005 im Admin erzeugen, schreiben und einlesen |
