...
| **Die offenen J4-Konversionen** (2 von 4 sind gegangen) | §14 „Übergänge J4/J4b `sep02`", §7.9 | der Naht-Knick ist beseitigbar, aber `dconn` kann per Konstruktion nicht darüber urteilen: (2) `dspan` hat das `sep04` behoben, (3) die Wortrunde hat den Arm `sep06` mit 34 : 2 bestätigt — die Details stehen in §7.9 | offen: **(1) die Ankunftsseite** — der Trim verschlechtert sie messbar (`seam_arr_median` −2,83° → −3,87° auf der `sep05`-Wurzel), eigener Arm mit eigener Pre-Reg und `dspan`-Gates von Anfang an; **(4) den Flick eine Stufe tiefer gar nicht erst lernen** (Endblenden-Familie LF5/LF6), unverändert ein Laufform-Arm ohne eigene Messung |
