...
| **Zug-übergreifende Schleifen in `loop_ranges`** (was das `t` braucht) | §14 „Kringel-Landmarke `sep06`" (Rettungsweg 2), vermessen in „Kette R3 `sep07`" | **Der Grund ist gefunden und er ist strukturell:** `core.aggregate.loop_ranges` sucht Selbst-Kreuzungen JE PEN-ZUG (`for (lo, hi), line in zip(bounds, lines)`), das `t` aber schließt seine Binnenfläche zwischen SEINEN ZWEI ZÜGEN. Auf der `eaa195aa`-Wurzel: `t`-Tafelzeile 120 Anker, `stroke_starts [0, 83]`, Schleifen je Zug `[[], []]` — beide Züge für sich sind schleifenfrei —, beide Züge zusammen ein Loch `D0 = 0,2475` bei (0,359, 0,403), umspannt von den Ankern **66..109**, also quer über die Zug-Grenze 83. Ein Bereich, der eine Feder-Hebung überspannt, ist im heutigen Datenmodell nicht ausdrückbar; `align_loops` müsste erst entscheiden, was er dort bedeutet | offen, eigene Pre-Reg — **vor dem nächsten Laufform-Write**, denn ohne ihn bleibt das größte Defizit des Katalogs (`t`#1, 0,0517 gegen 0,3912) für LF13/LF14 unsichtbar und ist der einzige Verlust, der auch bei der ausgelieferten Feder 0,0724 bleibt |
| **Ein Folger-Pfad für die Ernte** | §14 „Kette R3 `sep07`" | Der Weg von einer korrigierten Kette-Bahn zur Laufform-ZEILE ist zu: die Ernte fährt ihre eigenen Kettenfits, und §3 verbietet ihr ausdrücklich einen Folger-Pfad. Ohne ihn kann kein Bahn-Arm die Kennzahl der 24 schließenden Wörter überhaupt erreichen | offen, eigene Pre-Reg; **hinter dem Fixpunkt-Arm** (LF14-Nebenbefund: eine frische Ernte landet 0,0025–0,0283 xh neben der Zeile, aus der die Wurzel geschrieben wurde) |
