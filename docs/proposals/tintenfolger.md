...
| Lotse v0.11 Stufe "all" (`aug19`) | Zonen-Rides/Brücken-Pinning scheitert um genau EIN Galoppieren-Doppel-X (Netto 8 > 7); die Doppel-X-Duplikate sind 4 der 6 Rest-Spurious | EIN X je Knoten-Cluster (Begradigung der Fenster-Teilbahn je Pass durch den Knoten) — schaltet "all" mit frischer Pre-Reg wieder frei | eigene Pre-Reg |
