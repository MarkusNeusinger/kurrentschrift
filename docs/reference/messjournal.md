...
### Laufform LF13 `sep06` — gemessen: der Schätzer war nie der Träger, und der Arm fällt an zwei Gates

Umgebung wie vorregistriert: Wurzeln **`eaa195aa7c84`** /
**`0fbde2d72b64`** (`exported_at` 2026-09-05T21:33:12+00:00 bzw.
21:33:11+00:00), jeder Lauf mit `--expect-root` und
`OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1`, Basis-Headline ziffernweise
reproduziert (**0,108444 · 0,148236**). **Es wurde nichts geschrieben,
keine Wurzel neu exportiert und kein Lineal angefasst.**

Das Lineal des Arms ist an der Kennzahl des Blatts abgenommen: die
Zählregel gibt auf dieser Wurzel **26 von 63** mit der Aufschlüsselung
des Blatts, Glyphe für Glyphe (`a` 12 · `o` 5 · `sz` 4 · `G` 3 · `g` 3 ·
`r` 2 · `v` 2 · `k` 1).

#### Teil 1 — die Zurechnung: H2 gehört zu drei Vierteln der ERNTE, nicht dem Median

**Die Vorbedingung steht.** Der LF12-Erntestapel leitet alle 18
gespeicherten Zeilen bei Δs 0,16 xh wieder ab, größte Ankerbewegung
**0,000000000 xh** über alle 18 — er IST der Eingangs-Stapel, und damit
misst diese Zurechnung den Schätzer und nicht eine Vergleichsmenge.

Und damit kippt der Betrag von H2. Das Blatt maß „Zeile gegen ihre
Vorkommen" an den Kette-WORTFITS und fand −0,004 bis −0,082 xh. Gegen den
eigenen Stapel gemessen ist es **ein Drittel bis ein Viertel davon**:

| Glyphe | n | Kette (Blatt) | Vorkommen (Stapel) | Zeile | Kette→Ernte | Ernte→Zeile |
|---|---|---|---|---|---|---|
| `a` | 13 | 0,2236 | 0,1909 | 0,1858 | **−0,0327** | −0,0051 |
| `o` | 4 | 0,1826 | 0,1654 | 0,1767 | **−0,0172** | **+0,0113** |
| `sz` | 5 | 0,2705 | 0,2167 | 0,1876 | **−0,0538** | −0,0291 |
| `g` | 4 | 0,2377 | 0,1809 | 0,1743 | **−0,0568** | −0,0066 |
| `r` | 18 | 0,2332 | 0,2177 | 0,2091 | **−0,0155** | −0,0087 |

Der Schritt, den das Blatt nicht sehen konnte, ist der GRÖSSTE der
beiden: die **Ernte** — das Zerlegen eines Kette-Wortfits in ein
zentriertes, auf 120 Chart-Anker parametrisiertes Vorkommen — kostet
0,016–0,057 xh, der Schätzer danach 0,005–0,029 (am `o` gibt er sogar
zurück). Paarweise je Vorkommen gerechnet, nicht aus Medianen, steht der
Erntschritt bei `a` −0,0115 · `o` −0,0074 · `sz` −0,0436 · `r` −0,0033.

**Und die Zerlegung des Rests** (Fenster 0,25 xh, über die vier Bereiche,
die mehr als 0,005 xh verlieren; Anteile am Verlust im Median):

| Bereich | gesamt | Lage | Größe | Basis | Rest | Renderer |
|---|---|---|---|---|---|---|
| `a` 61:80 | +0,0051 | −0,0025 | −0,0000 | +0,0043 | +0,0033 | +0,0019 |
| `sz` 84:99 | **+0,0291** | **+0,0186** | −0,0005 | −0,0006 | +0,0115 | +0,0020 |
| `g` 39:52 | +0,0066 | −0,0018 | −0,0007 | −0,0031 | +0,0122 | +0,0060 |
| `r` 53:77 | +0,0087 | −0,0014 | **+0,0060** | +0,0038 | +0,0003 | +0,0011 |
| **Median-Anteil** | | **−22 %** | **−1 %** | **+21 %** | **+53 %** | |

Die Fehlregistrierung der LAGE trägt also nur beim `sz`, und dort mit
64 % des Verlusts; über die vier Bereiche führt der **Rest** — die
Formstreuung, die keine Registrierung fassen kann. Die vorregistrierte
Auswahlregel („der größte Anteil") zeigt damit auf M3, den
Öffnungsweiten-Boden — und der ist zugleich der Mechanismus, den die
Vorregistrierung selbst als den schwächsten benannt hat. Warum er
trotzdem nicht gebaut wurde, steht unten unter „Selbst entschieden", und
es ist eine gemessene Begründung, keine Abwägung.

**Die Zahl, an der der ganze Arm hängt.** Eine Binnenfläche ist bei
Halbbreite 0,097 offen, wenn `D0 > 0,194`. Die Vorkommen selbst tragen:
`a` 0,1909 · `o` 0,1654 · `g` 0,1809 — alle drei **zu**. Nur `sz`
(0,2167) und `r` (0,2177) liegen darüber. **Die Decke JEDES
schätzerseitigen Mechanismus, auch die eines Öffnungsweiten-Bodens am
Vorkommens-Median, ist damit ein einziger Buchstabe.** Die
vorregistrierte Decke von 18 Wörtern war die Decke der ERREICHBARKEIT;
die Decke der EVIDENZ ist kleiner, und sie war vor dieser Runde nicht
bekannt.
...
| | Basis | 0,15 | 0,25 | 0,50 |
|---|---|---|---|---|
| `bench_loss` | **0,108444** | 0,108637 | 0,108871 | 0,108710 |
| `pair_loss` | **0,148236** | 0,148239 | 0,148239 | 0,148239 |
| Gate (a): Bereiche unter dem Vorkommens-Boden | **4** | **2** | 2 | 2 |
| größte Ankerbewegung | 0 | 0,0268 | 0,0268 | 0,0257 |

#### Die Gates

| Gate | Ergebnis | |
|---|---|---|
| **(a) Öffnungsweite** | `h[68:108]` 0,5497 < 0,5617 · `z[26:46]` 0,2022 < 0,2064 | **rot** |
| **(b) Bewegungsmaß** | max 0,0268 xh von 0,03 | grün |
| **(c) Lineal** | 0,108444 → **0,108637** (+0,000193) · 0,148236 → 0,148239 | **rot** |
| **(d) Kette-Identität** | `--candidate authored` auf der Kandidaten-Wurzel: **PASS** (dtw 0, Chamfer 0/0) | grün |
...
also, deren Vorkommen die Öffnung überhaupt tragen; die Vorhersage aus
Teil 1 trifft ohne Rest. `gleichzug_doublings` bei Halbbreite 0,097
(Wurzel-Kopie mit `constant_nib_units` 0,097): **22 → 21**; bei der
ausgelieferten Feder unverändert 14 → 14, `bench_loss` dort 0,101254 →
0,101260.

#### (R4) Der Verschmelzungs-Anzeiger — gebaut, geeicht, Report-Spalte

`(D0_Skelett − Platte) / (2 · w_pen)` über die 202 gemessenen
Binnenflächen, `w_pen` = 0,0968 xh: Median **0,689** (p10 0,531 · p90
0,879). **Die Nullprobe ist eine Kurve, kein Schwellwert** — je weiter
die Binnenfläche, desto weniger Platz hatten die zwei Federzüge zu
verschmelzen, und der Anzeiger steigt über sechs Bänder monoton:

| Platte (xh) | 0,00–0,12 | 0,12–0,20 | 0,20–0,30 | 0,30–0,45 | 0,45–0,60 | > 0,60 |
|---|---|---|---|---|---|---|
| n | 31 | 62 | 13 | 40 | 33 | 23 |
| Anzeiger | 0,609 | 0,646 | 0,646 | 0,733 | 0,795 | **0,878** |

Das `S` allein steht bei 0,970 — dort IST die Mittelachse der Federweg.
Kein Umkehrpunkt, sechs Bänder, die Richtung des Modells: der Sensor ist
abgenommen und bleibt **ausschließlich Report**, wie die Vorregistrierung
verlangt.

#### Verdikt

**Verworfen an (a) und (c)**, mit einem zurechenbaren Teilerfolg (4 → 2
verletzte Bereiche, 26 → 23 Wörter, Verdopplungen 22 → 21, keine
Regression). Der Schalter bleibt AUS. Die Karte liegt trocken im
Sitzungs-Scratchpad (`laufform-loop-card/`, 18 Zeilen, `P`/`p`/`s`
unberührt) — als Beleg, nicht als Empfehlung.

**Der eigentliche Ertrag ist Teil 1**, und er verschiebt die Arbeit: die
Zeilen-Ableitung ist nicht der Ort, an dem die Kringel verloren gehen.
Vor ihr liegen die Ernte (0,016–0,057 xh) und die Mittelachse selbst
(0,035–0,104 xh, Blatt H0), und hinter allem die Chart-Form (H3). Ein
Schätzer, dessen Evidenz schon zu ist, kann keine Öffnung liefern.

#### Rettungswege

1. **Die ERNTE statt des Medians** (neu, der gemessene Hebel): warum
   verliert die Zerlegung eines Kette-Wortfits in ein zentriertes
   120-Anker-Vorkommen 0,016–0,057 xh Öffnungsweite? Kandidaten sind die
   Chart-Prior-Regularisierung des Dissektions-Fits und die
   Neuparametrisierung auf die Ankerzahl der Tafel. Erst als Zerlegung
   messen (Fit-Residuum gegen Öffnungsweite je Vorkommen), dann ein Arm.
2. **R3 unverändert offen** — Schleifen-Geometrie aus der Evidenz statt
   aus dem Anker-Median; die Kette hält 7 von 8 engen Stellen offen, und
   das ist die einzige Quelle, aus der die 0,10–0,20 xh ganz zurückkommen
   können.
3. **Der `sz`-Teilerfolg als eigener, enger Arm**: die Registrierung nur
   dort anwenden, wo die Zurechnung sie trägt (Lage-Anteil > 50 %), statt
   auf allen Zeilen — die Asymmetrie-Regel `aug26` in Reinform. Das
   Lineal verliert +0,000193 über ALLE Zeilen für einen Gewinn an EINER.
4. **Autorenweg unverändert** (`sep05`): die engen Formen nachzeichnen
   bzw. den Eigenhand-Bestand als zweite Quelle; das Blatt beziffert die
   Strecke mit +0,10 bis +0,20 xh je Schleife.

**Ausdrücklich kein Weg:** den Öffnungsweiten-Boden (M3) doch bauen und
auf die Vorkommen setzen — die Decke ist gemessen und beträgt einen
Buchstaben; und die Feder dünner rechnen, bis die Löcher aufgehen.

**Selbst entschieden.**

* **M3 nicht gebaut, obwohl die Auswahlregel darauf zeigt.** Die Regel
  war „größter Anteil"; der Anteil ist der Rest, und M3 ist der einzige
  Kandidat, der hineinreicht. Gebaut wurde er trotzdem nicht, und zwar
  aus einer ZAHL dieser Runde: seine Decke ist der Vorkommens-Median, und
  der liegt bei `a`, `o` und `g` unter 0,194 — M3 könnte an genau
  derselben einen Glyphe etwas ausrichten wie die Registrierung, wäre
  aber ein Knopf statt eines Mechanismus. Die Abweichung von der Regel
  ist damit benannt statt still, und sie ist gemessen statt abgewogen.
* **Die Ähnlichkeit statt der reinen Verschiebung.** Die Vorregistrierung
  beschrieb M1 als Translation; die Zurechnung hat die Größe daneben
  gemessen und beim `r` +0,0060 gefunden, also ist beides gebaut und
  beides einzeln berichtet (`--no-loop-scale` ist der Kontrollarm).
* **Zwei unabhängige Schleifen-Finder.** Der Mechanismus braucht einen
  rasterfreien (er muss im Schreibpfad laufen können), die Messung einen,
  der genau die Binnenflächen findet, die das Lineal misst. Sie gegen
  einander abzunehmen war billiger als einem von beiden zu glauben.
* **`f` weiter nicht mitgemessen**, wie im Blatt: eine Einschnürung ist
  eine andere Messgröße.

**Grenzen.** `o` steht auf n = 4, `g` und `sz` auf 4–5 — die Mediane
kleiner Mengen sind Einzelablesungen mit Median-Etikett. Der
Erntschritt ist an den Kette-Schleifen des Blatts gepaart, dessen
...
### Kette R3 „Zwei-Züge-Modell" `sep07` — Vorregistrierung: die verschmolzene Schleife ist zwei Kapseln, nicht ein Klumpen

Autor-Entscheid vom 2026-09-06 („2 ja"): nach dem Katalog ist das
Zwei-Züge-Modell der nächste Hebel. Der Leitsatz dazu ist seiner:
**der Tinte richtig folgen, dann bleiben die Binnenflächen offen** — die
Schleifen der Platte SIND an der Plattenfeder offen, und eine komponierte
Schleife, die zuläuft, hat etwas verloren, was die Hand hatte. Das ist
Rettungsweg **R3** der Kringel-Diagnose und nach den Negativen LF13
(`sep06`) und LF14 (`sep06`) der einzige, der die 0,10–0,20 xh ganz
zurückholen kann.

**Basis, unverändert und nicht neu exportiert.** `suetterlin-1922`
`exported_at` 2026-09-05T21:33:12+00:00 `root_digest` **`eaa195aa7c84…`**,
`suetterlin-1922-pairs` 2026-09-05T21:33:11+00:00 **`0fbde2d72b64…`**;
`OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1` in jedem Aufruf. Die
Wort-Bench-Headline dieser Wurzel ist **0,108444 · 0,148236**; sie steht
hier als Nachweis der Basis, nicht als Ergebnis — dieser Arm fasst
`core/` nicht an und kann sie gar nicht bewegen.

#### Der Mechanismus in drei Sätzen

Wo zwei Federzüge um eine kleine Binnenfläche VERSCHMELZEN, ist der
Tintenklumpen die Vereinigung zweier Kapseln von bekannter Federbreite,
und die Mittelachse dieses Klumpens ist nicht mehr der Federweg — genau
der H0-Befund von #551 (die lokale Halbbreite am Loch ist 0,0667 statt
0,0968). Statt den Klumpen zu skelettieren wird die Feder
HERAUSGERECHNET: eine Gleichzug-Feder der Halbbreite `w` malt die
Minkowski-Summe ihrer Bahn mit der Scheibe vom Radius `w`, ein Punkt der
Binnenfläche ist also genau dann eingefärbt, wenn irgendeine Stützstelle
näher als `w` an ihm liegt — **keine Stützstelle darf näher als `w_pen`
an einer Binnenfläche stehen, die die Platte offen hält**. Die Korrektur
schiebt jede Stützstelle, die das verletzt, entlang des Abstandsfeldes
der Binnenfläche genau so weit hinaus, dass die Bedingung gilt, und
blendet die Verschiebung an beiden Enden des korrigierten Laufs über
eine halbe Feder Bogenlänge glatt aus.

Das ist die Kapsel-Union rückwärts gelesen: wo die Bahn heute der Achse
des Klumpens folgt, trennt das Hinausschieben die beiden Durchgänge auf
mindestens `2·w_pen` — genau den Abstand, den eine Zwei-Kapsel-Union mit
überlebendem Loch braucht. Die Restgröße, nach der das Modell benannt
ist, heißt `stroke_separation` = Klumpenbreite − `2·w_pen`.

#### Wo er eingreifen darf, und wo ausdrücklich nicht

* **Nur auf Schleifen des eingefrorenen Katalogs** (#556,
  `tools/tracebench/kringel_catalogue.json`) mit Zustand **`offen`** und
  Größenklasse **`klein`** oder **`mittel`** — den Klassen, in denen das
  GERÄT entscheidet. `wechselnd` und `punkt` bleiben unberührt (die
  Platte schließt sie selbst), `groß` ebenso: dort überlebt drei Viertel
  des Lochs jede Feder, eine Bewegung dort wäre Bewegung ohne Gegenwert.
* **Nur wo die Platte im SELBEN Vorkommen ein Loch zeigt.** Ohne Loch
  gibt es kein Abstandsfeld, gegen das man entfalten könnte; gemessen
  wird dann nur noch `stroke_separation` der Klumpenbreite, und der Arm
  verweigert — auch wenn die Trennung positiv ist, denn ohne Loch sagt
  nichts, auf welcher Seite des Klumpens welcher Durchgang lief, und das
  zu wählen wäre zur Messzeit erfundener Duktus.
* **Nur wo die Bahn die Schleife überhaupt zeichnet.** Eine Schleife
  unter dem Splitterboden 0,05 xh ist ein **Topologie-Verlust** und
  gehört nach #556 vor die Öffnungsweitenfrage, zum Mittelachsen-Term
  (R4) — kein Weitungsterm holt eine nie gezeichnete Binnenfläche zurück.
* **Ausdrücklich kein Weg:** die Feder dünner rechnen, die Klassen- oder
  Zustandsschwellen des Katalogs nachziehen, oder einen Öffnungs-Bonus in
  den Fit-Loss schreiben (die R4-Warnung: erst der Sensor, dann der Arm).

#### Der Einsatzpunkt — der kleinste ehrliche

Die Korrektur läuft in `tools/pairlab/follow.py` **nach der letzten
Runde** auf der zusammengesetzten Federbahn, vor der Draht-Kappung; der
Schalter ist `--zwei-zuege`, Vorgabe AUS. Das ist der kleinste Punkt, an
dem der Eingriff überhaupt möglich ist, und er wurde aus vier Gründen
gewählt: (i) es ändert **keinen Kettenlauf** — die Leitplanke von §3
verbietet das, und ein Term im Solve wäre genau der Knopf, vor dem die
R4-Skizze warnt; (ii) es bewegt **kein `core/`-Byte**, das Golden ist
damit per Konstruktion byte-gleich; (iii) es liest die **Platte** und
EINE eingefrorene Federkonstante, **keine Laufform-Zeile** — der
Fixpunkt, den LF14 als Nebenbefund gefunden hat (eine frische Ernte
leitet jede Zeile 0,0025–0,0283 xh neben der ab, aus der die Wurzel
geschrieben wurde), kann sich hier also nicht schließen; (iv) es ist der
erste Punkt, an dem die ganze Wortbahn in EINEM Rahmen vorliegt.

#### Zwei deklarierte Konventionen, VOR der ersten Armzahl

1. **Das halbe Pixel.** Eine Distanztransformation misst von
   Pixelmitte zu Pixelmitte, die Rasterlesung eines Lochs ist also auf
   jeder Seite ein halbes Pixel großzügig. Die Erwartung ist die des
   Katalogs, wörtlich (`D0_Platte = Tinte + 2·w_pen` mit
   `Tinte = 2·max EDT`); damit die erreichte Weite in DERSELBEN Konvention
   landet, zielt der Schub auf die Niveaulinie `w_pen + 0,5 px` statt auf
   `w_pen`. Ohne das verfehlen sich die beiden Seiten um ein Plattenpixel
   = 0,03 xh bei 30–35 px je x-Höhe, mehr als die Toleranz des Gates.
   Das ist der Ablese-Boden, den #551 selbst nennt (±0,5 px ≈ ±0,015 xh),
   benannt statt in ein Ergebnis eingerechnet.
2. **Die Messung der erreichten Weite** ist dieselbe wie im Katalog:
   `D0`, der Durchmesser des größten Kreises im Gebiet, das die
   geschlossene Mittellinien-Schleife umschließt — hier in einem Fenster
   um die Binnenfläche gerastert (800 px je x-Höhe, Ableseboden
   0,0025 xh), ohne Splitterboden, damit eine KOLLABIERTE Schleife als
   kleine Zahl sichtbar bleibt und nicht weggefiltert wird (die Warnung,
   die die Kringel-Diagnose an R1 hängt).

#### Die Gates

| | Gate | Bestanden, wenn |
|---|---|---|
| (a) | **Öffnungsweite je Schleife** | je korrigierter Schleife \|`D0` korrigiert − (`Tinte` dieses Vorkommens + 2·`w_pen`)\| ≤ 0,02 xh, UND jede korrigierte `offen`-Schleife ist bei Halbbreite 0,097 offen (`D0` > 0,1936). `wechselnd` und `punkt` bleiben unberührt — geprüft, nicht behauptet |
| (b) | **Struktur** | Identitäts-Gate PASS und `cross_missing`/`cross_spurious` nicht schlechter als die Basis |
| (c) | **Bahnmaß** | `dtw_xh` (Median und p90) gegen die gespeicherten Vorkommen nicht schlechter als die Basis |
| (d) | **Unstetigkeit** | kein neues `kink`-Ereignis über der Schwelle (#558, 11,537°) an der Naht der Korrektur zum übrigen Zug — die Korrektur muss BLENDEN |
| (e) | **Wort-Bench** | 0,108444 · 0,148236 unverändert (Zeilen unberührt → byte-gleich) |
| (f) | **Golden** | `pen=None` byte-identisch |
| (g) | **Fixpunkt** | die Korrektur hängt NICHT an den Zeilen — nachgewiesen an ihren Eingängen, nicht behauptet |

#### Die Decke der Kennzahl, vorab beziffert

**Die 24 bewegt dieser Arm nicht, und das ist keine Enttäuschung, sondern
Arithmetik.** Die 24 schließenden Wörter von #556 sind eine Eigenschaft
der KOMPONIERTEN Wörter, also der gespeicherten Laufform-Zeilen; dieser
Arm korrigiert die Kette-BAHN. Der Weg von hier zur 24 führt über die
Ernte (Bahn → Vorkommen → Zeile), und der bleibt in diesem PR
ausdrücklich zu:

* die Ernte hat nach der stehenden Leitplanke von §3 **keinen
  Folger-Pfad** — ihn zu legen ist ein eigener Arm, keine Nebenwirkung;
* **LF14s Nebenbefund verbietet den Write ohnehin vorerst**: eine frische
  Ernte der `sep05`-Wurzel leitet jede Zeile 0,0025–0,0283 xh neben der
  ab, aus der die Wurzel geschrieben wurde. Eine Karte auf dieser Basis
  trüge eine nicht zurechenbare Differenz; der Fixpunkt-Arm muss zuerst.

Deshalb wird hier **keine Karte erzeugt** und keine Zeile neu abgeleitet,
und Gate (e) ist per Konstruktion grün statt per Messung knapp. Was
dieser Arm bewegen KANN, ist die Zahl auf der Bahn-Seite: die
`kringel_lost`-Spalte des Tintenfolgers, gemessen an denselben
Katalogschleifen — Basis dev-19 **12 von 43 `offen`-Schleifen in 10 von
19 Wörtern**. Das ist die Kennzahl dieses Eintrags; die 24 gehört dem
Nachfolge-Arm.

**Kill-Kriterien.** (i) Gate (b) oder (c) rot → verworfen, unabhängig
von der Öffnungsweite: Struktur schlägt Distanz, und Distanz schlägt
Weitung. (ii) Gate (a) unter der Hälfte der korrigierten Schleifen →
das Modell trifft sein eigenes Ziel nicht und ist kein Mechanismus,
sondern eine Verschiebung. (iii) Jede nachträgliche Lockerung einer
Schwelle dieses Blocks ist ausgeschlossen; was hier steht, steht vor der
ersten Armzahl.

#### Zwei Korrekturen am Werkzeug, nach dem Kalibrierlauf und vor dem Arm

Wie bei #556 stehen sie hier statt in einer Fußnote. Beide entstanden auf
einem Kalibrierlauf, der die Korrektur OFFLINE auf die gespeicherten
Basisbahnen legte (dieselbe reine Funktion, kein zweiter Solve), und
beide sind Teil des Mechanismus, nicht des Lineals:

1. **Die Abnahmeregel „Schleifen-Schluss".** Der Schub trennt die beiden
   Äste einer Schleife — und kann dabei die Selbst-Kreuzung aufziehen,
   die sie überhaupt schließt. Im Kalibrierlauf war das an `v`#1
   (0,2380 → 0,0064), `K`#1 (0,2115 → 0,0214) und `p`#1 (0,5784 →
   0,3140) sichtbar: eine Weitung, die ihren eigenen Gegenstand
   zerstört. Sie wird jetzt **zurückgenommen** statt gemeldet — dieselbe
   Bauart wie `structure_guard`, eine Abnahmeregel auf der Struktur, kein
   weiterer Term.
2. **Die kollabierte Schleife ist eine eigene Klasse.** Wo die Bahn unter
   0,05 xh Weite trägt (`F`#1 mit 0,0025), zeichnet sie die Schleife
   nicht, und ein Weitungsterm ist das falsche Werkzeug. Der Arm
   verweigert dort mit eigenem Grund und schiebt die Klasse an R4
   weiter, statt sie mit dem Rest zu vermischen.

**Was in demselben Lauf auffiel und ausdrücklich NICHT geändert wurde:**
17 von 128 Schleifen verweigern, weil eine Stützstelle der Kette-Bahn
INNERHALB der Platten-Binnenfläche steht. Das ist ein Befund über die
...
auf den EINEN Schalter. Die Basis reproduziert die Kampagnen-Zahlen des
`sep05`-Re-Baselines ziffernweise (dev-19 dtw-Median **0,045384**, p90
**0,087826**, schlechtestes `muß` **0,107134**, aiou **0,7583**,
...
Der Unstetigkeits-Sensor (#558) liest, je Stützstelle und index-gleich,
die Kette-Bahn vor und nach der Korrektur, im Umkreis einer Bogensehne um
jede bewegte Stützstelle und ohne die Umkehrecken:
...
| Gate | Ablesung | Urteil |
|---|---|---|
| (e) Wort-Bench | `--set all --expect-root eaa195aa,0fbde2d7`: **0,108444 · 0,148236**, unverändert | **grün** (per Konstruktion: keine Zeile, kein `core/`-Byte) |
...
Geschrieben, bevor R3b gerechnet wurde. Der Befund oben nennt eine
Ursache, die keine neue Idee braucht, sondern eine Länge: **die Blende
ist kürzer als das Fenster, mit dem der Sensor liest.** Eine
Verschiebung, die sich INNERHALB von 0,145 xh ändert, ist per Definition
das, was `kink` misst — der Sensor liest die Wendung über eine halbe und
eine ganze Federbreite und nennt eine Konzentration darin einen Knick.
Also muss die Glättung mindestens dieses Lesefenster abdecken.

* **Ein Faktor:** der Blenden-Maßstab `b`, mit Rampe = `b` und
  Anti-Raster-Glättung = `b/2`, wie bisher. Heute `b` = 0,0725 (halbe
  Feder), also Glättung 0,0363. **R3b setzt `b` = 0,29 (zwei Federn),
  also Glättung 0,145 = eine Feder.** Der Wert ist NICHT gewählt, sondern
  abgeleitet: er ist der kleinste, bei dem die Glättung das Lesefenster
  des Sensors ganz überdeckt. Beide Werte sind Sprossen derselben
  eingefrorenen Leiter von #558 (halbe Feder · eine Feder · zwei Federn);
  keine dritte Zahl kommt hinzu.
* **Der erwartete Preis, vorab benannt:** eine längere Glättung frisst die
  Spitze des Schubs. Gate (a) MUSS also schlechter werden; die Frage ist,
  ob es sein Kriterium noch hält. Das ist die vorregistrierte Vorhersage:
  Gate (d) wird besser, Gate (a) schlechter, und der Arm ist nur dann eine
  Konversion, wenn (a), (b), (c) und (d) GLEICHZEITIG stehen.
* **Gates:** dieselben (a)–(g), Wort für Wort, ohne eine einzige gelockerte
  Schwelle.
* **Das Messmittel:** R3b wird OFFLINE auf den gespeicherten Basisbahnen
  gerechnet — dieselbe reine Funktion, dieselben Optionen, kein zweiter
  Solve. Zulässig ist das nur, weil die Draht-Kappung auf dieser Wurzel
  nie greift (keine Warnung in beiden Läufen), die gespeicherten Bahnen
  also die zusammengesetzten sind; und es wird nicht angenommen, sondern
  **abgenommen**: der Offline-Pfad muss den Folger-Arm mit denselben
  Optionen byte-genau reproduzieren, bevor er mit anderen gefahren wird.

**Die Abnahme des Messmittels, vor der ersten R3b-Zahl.** Der Offline-Pfad
mit den Optionen des Folger-Arms gegen dessen eigenen Kandidaten gehalten:
**größter Koordinatenunterschied 0,000e+00 xh über alle 63 Wörter**, keine
einzige abweichende Strichform. Damit ist er dasselbe Instrument und nicht
nur ein ähnliches.
...
und Glätte wählen, weil beides an derselben einen Länge hängt — eine
glatte Bahn, die ihre Bedingung exakt einhält, gibt es in dieser
Formulierung nicht. Genau das ist die Rechtfertigung für Konversion (3)
der §7.9-Zeile: die Bedingung „nicht näher als `w_pen` an dieser
Binnenfläche" gehört als NEBENBEDINGUNG in den Folger-Solve, wo eine
glatte Bahn ohnehin entsteht und der Solver den Ausgleich rechnet, statt
als Nachbearbeitung, die ihn per Konstruktion nicht rechnen kann.
...
| | Wörter | Paare |
|---|---|---|
| ohne den Austritts-Trim | `bench_loss` **0,108444** | `pair_loss` **0,148236** |
| Runde-5-Kandidat (`exit_trim`) | 0,109026 | 0,148236 |
| `seam_dep_abs_median` ohne Trim / mit | 12,67 / **2,30** | 56,36 / 56,36 |
| `seam_arr_abs_median` ohne Trim / mit | 10,80 / **10,80** | 9,87 / 9,87 |
| `gleichzug_doublings` | 14 | 3 |

**Nachtrag vom selben Tag, nach dem Bau dieses Arms.** Der Autor hat den
Austritts-Trim adoptiert (Entscheid **A37**, §14 „Übergänge J4 `sep06` —
Adoption"): `exit_trim` ist seit dem 2026-09-06 der Produktions-Default, und
die zweite Zeile oben IST damit die Produktion. Die Zahlen dieses Eintrags
bewegen sich dadurch nicht — sie wurden von Anfang an gegen den
Runde-5-Kandidaten gemessen, weil die Vorregistrierung ihn als Basis nennt —,
aber ihre Beschriftung: „J4" heißt ab hier der ausgelieferte Stand, und die
erste Zeile ist die pre-adoption Basis, die nur noch mit `--no-exit-trim`
erreichbar ist.

**Gates, fixiert vor der ersten Arm-Zahl.** Basis ist der Runde-5-Kandidat
(der Trim an — seit A37 die Produktion), gemessen wird auf der verhandelbaren
Klasse und zusätzlich ungefiltert.
...
**Die Leiter.** „J4" ist der Runde-5-Kandidat und seit A37 der ausgelieferte
Stand, „J6" der vorregistrierte Arm darauf, „J6b" der post-hoc verengte (nur
Nähte, deren beide Seiten sich auch treffen KÖNNEN: `max_jump` = 2 × Deckel =
16°). Die beiden unteren Zeilen zeigen J6 OHNE den Trim, weil die Verhandlung
die Austrittsseite selbst löst.

| Arm | Wörter | Paare | `seam_dep`/`seam_arr` (W) | `seam_arr` (P) | Verdopplungen W/P | `comp_coverage` W | `meas_doff`/`dconn` W |
|---|---|---|---|---|---|---|---|
| ohne Trim (pre-adoption) | 0,108444 | 0,148236 | 12,67 / 10,80 | 9,87 | 14 / 3 | 0,101580 | 0,129 / 0,113 |
| J4 = Produktion seit A37 | **0,109026** | **0,148236** | 2,30 / 10,80 | 9,87 | 14 / 3 | 0,101667 | 0,166 / 0,152 |
...
**Nebenbefund, und seit A37 einer mit Adressat.** J6 löst die Austrittsseite
OHNE den Trim und billiger als er: `seam_dep_abs_median` 12,67 → **0,02**
gegen seine 2,30, bei **+0,000376** Wortlineal gegen seine +0,000582 — und
ohne seine Nebenwirkung auf die Ankunft (`seam_arr_median` −2,83° → −3,87°,
die offene J4-Konversion (1)). Die beiden Regeln sind darum keine Ergänzung,
sondern Alternativen: der adoptierte Trim schneidet den Stummel weg, die
Verhandlung dreht beide Seiten aufeinander zu, und beide zielen auf dieselbe
Naht. Sollte J6 je adoptiert werden, ist die Frage ausdrücklich zu stellen, ob
der Trim daneben noch etwas beiträgt — gemessen wurde hier beides zusammen,
weil die Vorregistrierung den damals gültigen Kandidatenstand als Basis nennt,
und die Runde-7-Basis ist deshalb der ausgelieferte Stand, nicht die
Alternative.
...
steht nicht in diesem Abschnitt, sondern — gefallen — im eigenen datierten
Eintrag darunter.
...
### Übergänge J4 `sep06` — Adoption auf Autor-Entscheid A37: `exit_trim` wird Default (deklarierte Re-Baseline)

**Autor-Entscheid A37 (2026-09-06): „1 ja" — `exit_trim` wird Default.** Der
Entscheid folgt der Empfehlung des Abschnitts oben und der LF11-Regel: die
Schwellen sind eine Adoptions-AUTOMATIK, keine Erlaubnisschranke für den
Autor, und `adopt: false` bleibt richtig gebucht. **Keine Zahl des Abschnitts
oben ist angefasst** — hier steht nur, was der Entscheid ausgelöst hat.

**Was umgestellt wurde.** `compose_word(..., exit_trim=…)` steht auf `True`
(`core/compose.py`); `EXIT_TRIM_MIN_KINK_DEG` bleibt bei 0,0, also die volle
J4-Klasse und nicht die in dieser Runde gemessene und verworfene
J4b-Verengung. Die Messwerkzeuge folgen dem Standard statt ihn zu
überschreiben: `wordbench.run`, `humanbench.wordarm` und `pairlab.spanmeas`
kennen jetzt `--no-exit-trim` (die pre-adoption Basis) statt `--exit-trim`,
und `wordarm` liest den Default per `inspect.signature` aus dem Composer —
ein Arm, der nichts sagt, zeichnet damit ab heute, was Prod zeichnet. Der
Schalter selbst bleibt: die Basis ist ein Arm wie jeder andere.

**Die Re-Baseline, gemessen auf UNVERÄNDERTER Wurzel** (`eaa195aa7c84…` /
`0fbde2d72b64…`, `--expect-root`, `OPENBLAS_NUM_THREADS=1
OMP_NUM_THREADS=1`) — also gepaart vergleichbar, anders als bei einem
Wurzelwechsel:

| | Wörter | Paare | `seam_dep_median` | `…_abs_median` | `seam_arr_median` | `gleichzug_doublings` |
|---|---|---|---|---|---|---|
| vorher (`--no-exit-trim`) | 0,108444 | 0,148236 | +7,59° | 12,67 | −2,83° | 14 |
| **nachher (Default)** | **0,109026** | **0,148236** | **−0,70°** | **2,30** | −3,87° | 14 |
| Δ | **+0,000581** | **0,000000** | −8,29° | −10,37 | −1,04° | 0 |

63 von 63 Wörtern und 33 von 33 Paar-Drills gescoret, keiner übersprungen,
keiner gefallen. `worst_word` wandert von `han` 0,232609 auf **`regieren`
0,233052**, `worst_pair` bleibt `In` 0,283819. Komponenten:
`comp_transition` 0,089804 → 0,091028 · `comp_coverage` 0,101580 → 0,101667 ·
`comp_width` 0,162397 unverändert (der Trim fasst die Naht, nicht die
Breite). **Die Verdopplungen bei der ausgelieferten Feder bleiben bei 14** —
der Trim macht keinen neuen Weißkeil zu, was bei einer Regel, die Tinte
WEGNIMMT, die richtige Richtung ist, aber nicht selbstverständlich war.

**Der Unstetigkeits-Sensor stimmt zu, und er hat davon nichts gewusst.** S2
(§14 „Übergänge S2 `sep06`") ist am selben Tag gebaut, eingefroren und an den
Runden 5 und 6 abgenommen worden — auf der Komposition OHNE den Trim, und ohne
dass eine seiner Konstanten aus dieser Runde stammt. Auf derselben Wurzel
gegen dieselbe Basis gemessen:

| | `cont_kink_total` | `cont_kink_deg_median` | `cont_wobble_median` | `cont_bow_join_median` | `cont_curv_loss_median` |
|---|---|---|---|---|---|
| vorher | 402 | 19,75 | 2,707 | 0,0091 | 0,0336 |
| **nachher** | **337** | **18,52** | **2,482** | **0,0042** | 0,0336 |

**65 Unstetigkeits-Ereignisse weniger**, die verbliebenen flacher, der Wobble
kleiner und die Pfeilhöhe IN DER NACHBARSCHAFT DER VERBINDER — die Spalte, die
genau diese Naht misst — mehr als halbiert. Das ist eine unabhängige
Bestätigung und kein Gate: S2 ist Report-Spalte, sein eigener Eintrag hält
ausdrücklich fest, dass keine einzelne Spalte über alle Klassen
richtungstreu ist, und keine seiner Zahlen hat an dieser Adoption
mitentschieden. Sie ist trotzdem der Ort, an dem zwei Instrumente, die nichts
voneinander wissen, dasselbe sagen: das Auge mit 34 : 2 und der Sensor mit
−65 Knicken. Umgekehrt heißt das auch: die S2-Zahlen des Nachbar-Eintrags
stehen ab heute auf der Komposition VOR dem Trim — sie sind dort richtig
gebucht und werden nicht angefasst.

**Das Golden-Fixture ist deklariert neu gebacken** (`REGEN_GOLDEN=1`).
Nachgemessen, bevor der Regen lief: **10 der 11 Wörter bewegen sich**, `wovon`
nicht (seine Austritte sind Rück-, Bogen- und Arm-Austritte, die die Klasse
per Konstruktion ausschließt). Kein Wort gewinnt oder verliert ein Draw-Item
— die Item-Zahlen stehen vorher wie nachher bei 6 … 18 —, es bewegen sich je
Wort 2 bis 8 Items, nämlich die getrimmten Buchstaben und ihre Verbinder.
**Der Satz aus `CLAUDE.md` hält:** eine EINZELNE Glyphe mit `pen=None` bleibt
byte-identisch — über alle 23 Glyphen der Golden-Payloads geprüft, 0 bewegen
sich, weil die Regel ohne folgenden Verbinder gar nicht feuern kann. Was sich
ändert, ist die Komposition eines Wortes, und das ist genau die Zusage, die
eine deklarierte Re-Baseline gibt.

**Zwei Messschichten waren mitzuziehen, und die eine ist eine Reparatur.**

1. `tools/pairlab/prodconn.py` hat seine eigene Ankündigung eingelöst. Ihre
   Docstring nannte den Austritts-Trim als benannte Blindstelle: die Regel
   ERSETZT die Rückgabe des Verbinder-Generators in `compose_word`s eigenem
   Block, „harmlos, solange der Schalter aus ist — wird er Default, muss diese
   Funktion dieselbe Nachbearbeitung bekommen, sonst misst die Dissektion
   still die falsche Kurve". Genau das ist jetzt geschehen: der Rekorder
   zeichnet zusätzlich `_cut_exit_stub` auf — die Funktion läuft genau dann,
   wenn der Trim wirklich angewandt wurde, also trägt sie die ENTSCHEIDUNG und
   nicht deren Nachbildung —, und `replay` verschiebt den aufgezeichneten
   Stummel mit dem Buchstaben und ruft `_exit_trim_index`/`_straight_to`
   erneut auf. Die Regel wird nirgends nacherzählt; welcher Sample der Schnitt
   an der bewegten Geometrie trifft, entscheidet weiterhin `core`. Die
   Parität-Tests sagen das jetzt in zwei Sätzen statt einem: ein ungetrimmter
   Join reproduziert den aufgezeichneten Aufruf Punkt für Punkt, ein
   getrimmter reproduziert die beiden Invarianten der Regel (der
   Kopplungspunkt bleibt, der Zug ist eine Gerade).
2. **Die Kette ist nachweislich NICHT betroffen, und das ist gemessen, nicht
   angenommen.** Ihr Init-Default ist der eingefrorene Spiegel
   (`CONNECTOR_INIT_MIRROR`; der Produktions-Init ist als K-F `sep04` gemessen
   und verworfen), und ihr Wächter-Soll kommt aus der Komposition. Letzteres
   nachgerechnet: `ductus_soll` über alle 63 Wortproben liefert **126 Zeilen,
   von denen sich 0 bewegen** — Kreuzungen 292, Zonen 177, Züge 111,
   Berührungen 89, Überlappungen 11, vorher wie nachher gleich. Der Trim
   schneidet einen Stummel und begradigt einen Verbinder; die TOPOLOGIE des
   komponierten Wortes rührt er nicht an. Damit stehen `k0eval`, der
   Struktur-Wächter und jede Kette-Zahl auf unverändertem Boden.

**Was dieser Nachtrag NICHT erledigt und was deshalb fällig ist.** Der Lotse
fährt als Karte die KOMPONIERTE Bahn (§14 „Laufform LF12 `sep05`", Absatz
„Der Lotse bewegt sich strukturell mehr als die Kette"), und die hat sich
soeben geändert. Seine dev-19-Zahlen vom `sep05` (dtw 0,053393 · p90 0,116199
· aiou 0,7493) stehen damit auf einer Komposition, die es nicht mehr gibt.
Das ist **keine** Zahl, die dieser PR nebenbei korrigieren darf: eine
Routen-Messung ist eine eigene Runde mit dem Protokoll aus
`/verify-trace` (gepinnte BLAS-Threads, `--expect-root`, `--compare` gegen den
Basisbericht). Sie ist als offener Arm in
[`../proposals/tintenfolger.md`](../proposals/tintenfolger.md) §7.11
eingetragen und vor der nächsten Lotse-Aussage fällig. Die Kette braucht
sie nach dem Befund oben nicht.
...
1. **Die `d`-Zeile als eigener Arm** (Vorschlag 1 oben) — neuer Zuschnitt,
   ...
   bedingte Lesart. **Nachtrag 2026-09-09: gegangen.** Der Autor hat den
   Vorschlag mit A40 angenommen; die frische Ernte, das Overlay aus der
   einen Zeile und die Runde 10 stehen im Eintrag „Laufform LF17
   `sep09`". Dieser Abschnitt bleibt Wort für Wort, wie er ist — die
   Zahlen dort sind die der Runde 8.
...
Geschrieben, bevor ein Mensch einen Bildschirm dieser Runde gesehen hat
— und der Plan-Teil ist committet, bevor das Wort-Lineal den Kandidaten
zum ersten Mal gesehen hat. Anlass ist der **Autor-Entscheid A40 vom
2026-09-09**: „Die `d`-Zeile als eigener Laufform-Arm: ja". Damit ist
Rettungsweg (1) des Eintrags „Laufform LF16 `sep08`" geöffnet, den jener
ausdrücklich als Vorschlag führte, weil eine Teil-Schreibliste jenseits
von „ohne `Z`" nicht vorregistriert war.

#### Was dieser Arm ist — und was er ausdrücklich nicht ist

**Er ist EINE Zeile.** Der Kandidat komponiert mit den gespeicherten
Zeilen der Wurzel plus einem Overlay aus genau der frisch geernteten
`d`-Zeile; die übrigen zwanzig bleiben Zeichen für Zeichen stehen, die
Basis ist die Wurzel, wie sie ist. **Er ist NICHT die Runde 8 noch
einmal:** deren Kandidat bewegte fünfzehn Zeilen zugleich, weshalb die
83,3 % der `d`-Spalte eine BEDINGTE Lesart sind und kein Arm. Genau
diesen Freiheitsgrad isoliert dieser Eintrag — den, den ein Write
installieren würde.
...
Die Behauptung, die aus Runde 8 herüberkommt: **die `d`-Zeile trägt auch
allein.** Vorregistriert als Richtung und Schranke: in der Klasse
`d-rein` gehen **≥ 60 % der ENTSCHIEDENEN** Bildschirme an den
Kandidaten. Widerlegt ist sie, wenn die Klasse an die Basis geht oder
unentscheidbar bleibt — beides verwertbar, aber nicht dasselbe (siehe
die Sichtbarkeits-Auflage unten).
...
([`runde-08-zeilen.json`](../../data/humanbench/runde-08-zeilen.json)).
Gegenprobe vor dem Bau: `arm_gap` > 0 für genau diese Wörter.

| Klasse | Schnitt | Sichtbarkeit vorab |
|---|---|---|
| `d-rein` | die Wörter, die die Zeile ziehen, ohne `und…` | **behauptet, aber an eine Zahl gebunden** (Auflage unten) |
| `d-und` | die vier Wörter, in denen `d` auf `u` trifft | nicht behauptet |
...
Wörter stand die Zeile bei 10 : 0. Ob das dem `u`, dem `n` oder ihrer
Kombination gehört, hat jene Runde nicht entschieden und kann diese mit
n = 4 auch nicht — die Klasse ist **beschreibend**. Getrennt steht sie,
weil ein gepoolter Anteil genau die Aussage verwischen würde, die
Runde 8 scharf gemacht hat.

**Die Zahl der Nullproben ist eine Rechnung, keine Geste** (§8a): der
Wiederholungstopf ist `n − min_gap − REPEAT_JITTER`, bei
`--min-repeat-gap 5` und `MIN_PAIRED_REPEATS` = 6 also `n` ≥ 36 und bei
vierzehn bewegten Wörtern **mindestens 22 Nullproben**. Vorregistriert
ist die Regel, nicht eine gefällige Zahl: gezogen mit benannter Saat aus
den Wörtern ohne `d`-Zeile, genommen die kleinste Menge, die die
geplanten Wiederholungen trägt; Saat und Zahl stehen in Teil 2.
...
Wörter, also einen Boden von ≈ 61 %. **Diese Runde kann sich eine
Adoption deshalb nicht selbst erteilen.** Sie ist der J4/Runde-5-Fall in
seiner schärfsten Form (§8a-Nachtrag: die Gesamtquote misst die
Mischung, die der Plan selbst gewählt hat) und legt ihr Ergebnis dem
Autor mit der Klassentabelle daneben vor.

**Was daraus NICHT folgt** — dieselben drei „kein Weg" wie im Eintrag
„Laufform LF16 `sep08`": die Schranken senken, die Nullproben aus der
Tie-Quote herausrechnen, die Klassen nach den Zahlen neu schneiden. Was
die Runde trägt, ist die Lesart, die §8a für eine schmale Klasse selbst
vorschreibt — **Kandidatenanteil auf den bewegten Wörtern,
Unentschieden-Quote auf den unbewegten, nebeneinander gelesen** — plus
die drei Zahlen je Klasse aus Schritt 4, für die `d-rein` mit n = 10
über dem Klassenboden liegt.

#### Die Sichtbarkeits-Auflage (§8a, Nachtrag aus Runde 7)

Wer eine Klasse „sichtbar" nennt, nennt vorher den Versatz, bei dem er
das behauptet. Die Stützpunkte: Runde 5 entschied bei **0,1186 xh**
(9,7 % Ties), Runde 7 bei **0,0221 xh** nicht (34,5 %). Runde 8 begrenzt
die Auflage: dort lag `zeile-stark` bei 0,1972 xh und trotzdem bei
54,2 % Ties — **der `arm_gap` sagt zwischen Runden etwas und innerhalb
einer Runde fast nichts.**
...
das Auge zuletzt nicht entschied. Darunter wird sie ausdrücklich NICHT
...
* **`d-rein` ≥ 60 % Kandidat unter den Entschiedenen, ≤ 25 % Ties INNERHALB
  ...
  schreiben, als deklarierte Re-Baseline mit Neu-Export beider Wurzeln
  und Ledger-Zeile; das Skript liegt vorbereitet (Teil 2).
* **Richtung klar, nur die klasseninterne Tie-Quote gerissen** →
  Autor-Entscheid mit der Klassentabelle daneben (J4/Runde-5-Muster).
* **`d-rein` geht an die Basis** → kein Write, und die 83,3 % der Runde 8
  schließen als das, was sie waren. Weiter geschnitten wird nicht — bei
  n = 10 wäre jeder Teilschnitt Rauschen.
* **Die Klasse bleibt unentschieden** → kein Write, und der nächste
  ...
  (Rettungsweg 4 der LF16-`sep08`-Zeile).
* **`d-und` entscheidet gar nichts** — n = 4, beschreibend, so gebaut.

#### Kill-Kriterien

* **Weniger als 6 gespiegelte Paare oder ≤ 7/12 Arm-Übereinstimmung** →
...
* **Die Ernte ist kein Fixpunkt** — zwei Läufe, die nicht byte-gleich
  sind, sind ein Befund und keine Iteration; dann gibt es keinen
  Kandidaten.
* **Die Zeile fällt an einem der drei Write-Gates** (Boden, Sprung,
  Kopf) → eine Runde über eine Zeile, die der Endpunkt nicht annähme,
  kann nichts lizenzieren.
...
Runde urteilt über KOMPOSITIONEN ganzer Wörter: was sie misst, ist der
Unterschied, den diese Zeile im Wortbild macht. Über `n` und `h`, die
Runde 8 mit Anspruch abgesagt hat, und über die sechs Zeilen unter dem
Klassenboden sagt sie nichts.

#### Teil 2 — die Konstruktionsmessungen (nach dem Plan, vor der Runde)

**Die Wurzeln.** `ccb036a5eb20…` (`exported_at`
2026-09-07T20:07:03+00:00) und `0fbde2d72b64…` — die Produktionswurzeln
seit dem Komma-Ausschluss (#578), unverändert von Platte gelesen; jeder
Lauf mit `--expect-root` und `OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1`.
Basis-Headline auf ihnen: **0,108153 · 0,148236**, `worst_word`
`regieren` 0,233052 — ziffernweise der ausgelieferte Stand.

**Die Ernte ist wieder ein Fixpunkt.** Zweimal gefahren mit den
Produktions-Defaults (Chart-Saat seit A38), `--path chain --sets words
--min-n 1 --jobs 4`: Vorkommen und Drafts **byte-identisch** (`sha256`
`9e60cae9e1c7…` bzw. `cd33454643b5…`), die Wortspuren bis auf das Feld
`seconds` (Wanduhr) ebenfalls. **227 von 277 Buchstaben angenommen**
(82 %), Gates `connector_degenerate` 31 · `anchor_spike` 8 ·
`not_converged_local` 6 · `geo_rmse` 5 — Zahl für Zahl die Kostenseite,
die LF16 auf der Vor-Komma-Wurzel maß.

**Die Zeile.** `d`, n = **11** Vorkommen, 120 Anker, und sie nimmt alle
drei Gates des Schreibwegs: Boden 3 ✓, **Sprung 2,28** (≤ 2,95), **Kopf
2,1°** (≤ 15°). Abstand zur GESPEICHERTEN Zeile, Anker für Anker: Median
**0,00609** · p90 0,01308 · Maximum 0,04451 · rms 0,00964 xh.
...
`exported_at` der Wurzel unterscheiden sich) — die Basis-Tafeln sind
dieselben Bilder. Was Runde 8 auf den `d`-Wörtern gesehen hat, hing also
weder an einer anderen Zeile noch an einer anderen Basis; was fehlte,
war der isolierte Freiheitsgrad.

**Die Zuordnung, dreifach geprüft.** 14 Wörter ziehen die Zeile —
`Feinde` `Soldaten` `das` `daß` `der` `der-2` `der-3` `die` `die-2`
`laden` `und` `und-2` `und-3` `und-4`, **exakt die Liste der Runde 8**.
Gegenprobe (a): `arm_gap` > 0 für genau diese 14, **exakt 0** für die
übrigen 49. Gegenprobe (b): auch das Wort-Lineal bewegt genau diese 14.

**Das Lineal — eigene Zahlen, nie die Headline.**

| Overlay | Wörter | Δ Basis | Paare | `worst_word` |
|---|---|---|---|---|
| Basis (Bestand) | 0,108153 | — | 0,148236 | `regieren` 0,233052 |
| **die `d`-Zeile allein** | **0,108339** | **+0,000186** | 0,148236 (byte-gleich) | `regieren` 0,233052 |

Die Paar-Zahl steht unbewegt, und der Grund bestätigt die Zuordnung ein
drittes Mal: die Abb.-20-Drills sind zu kurz für das Lauflängen-Gate,
komponieren chart-treu und sehen gar keine Laufform-Zeile. Zerlegt
(Asymmetrie-Regel `aug26`) über die 14 Wörter: **5 besser : 9
schlechter**, Median **+0,000168**, Summe +0,011763 — dieselbe Bilanz
5 : 9, die Runde 8 für die `d`-Spalte bedingt las, jetzt unbedingt. Je
Klasse `d-rein` 3 : 7 (Median +0,001459) und `d-und` **2 : 2** (Median
−0,000098); die Extreme `der-2` +0,007563 und `Feinde` +0,004178 gegen
`Soldaten` −0,004612 und `laden` −0,002505.

**Damit steht die LF16-Konstellation noch einmal da, nur schmaler:**
Lineal gegen den Kandidaten, Auge (Runde 8) für ihn. Dieselbe
Konstellation trug `sep06` den Austritts-Trim (Runde 5, 34 : 2, A37) und
`sep08` die 15-Zeilen-Karte NICHT (39,4 %). Vom Lineal entschieden wird
sie hier ohnehin nicht: +0,000186 ist ein Sechzehntel dessen, was die
ganze Karte gekostet hätte (+0,003097), und liegt auf der Skala, auf der
der Trim adoptiert wurde (+0,000582).

**Die Sichtbarkeits-Zahl, wie die Auflage sie verlangt.** Median-
`arm_gap` von `d-rein`: **0,1030 xh** (Minimum 0,1017; `d-und` liegt bei
denselben 0,1030) — **über der vorregistrierten Schranke von 0,05**,
Sichtbarkeit wird also behauptet. Sie liegt dicht am Versatz, bei dem
Runde 5 entschied (0,1186), und beim **4,7-Fachen** dessen, bei dem
Runde 7 es nicht tat. Die Dosis ist auch nicht dadurch kleiner geworden,
dass nur eine Zeile läuft: in Runde 8 bewegten dieselben Wörter im
Median 0,1953 xh mit fünfzehn Zeilen zugleich, für `das` und `daß` waren
...
offen: 40 − 5 − 25 = **10** im Wiederholungstopf, 8 genommen, also über
...
Der Arm ist kein Negativ — er ist ungeurteilt. Die Wege stehen trotzdem
vorab, weil die Runde eines produzieren kann:

1. **Der Unstetigkeits-Sensor S2** (`sep06`) über die `d`-Wörter — gebaut,
   eingefroren, und noch keine Laufform-Runde gesehen. Läuft er
...
4. **Eine Runde, die den BUCHSTABEN zeigt statt das Wort** — eine
   ...
   ihre Zahlen wären mit denen der Wortrunden nicht vergleichbar.

**Ausdrücklich kein Weg:** die Schranken senken, die Nullproben
herausrechnen, die Klassen neu schneiden — oder die Zeile ohne
Urteilsrunde schreiben, weil das Lineal sie diesmal fast nichts kostet.

#### Selbst entschieden (benannt, nicht versteckt)

* **Drei Klassen statt vier** — eine `lineal-verlierer`-Klasse wie in
  Runde 8 hätte hier n < 3, weil die Lineal-Verluste dieses Arms eine
  Größenordnung kleiner sind.
* **`--min-repeat-gap 5` statt der 15 des Wortmodus** — bei 14 bewegten
  Wörtern trägt keine andere Einstellung sechs Wiederholungen; die
  Spiegelung, nicht der Abstand, erzwingt das neue Urteil (§8a).
* **26 Nullproben statt der 22 des Bodens** — vier Bildschirme mehr geben
  dem Wiederholungstopf die Marge, aus der die 8 Paare kommen (bei 22
  wären es genau 6 ohne Reserve).
...
Overlay-Zahl dieser Vorregistrierung und wird es bleiben, bis ein Write
sie zur Basis macht.
### Kette K-G „Saat-Registrierung" `sep09` — Diagnose und Vorregistrierung: der Startort ist der Hebel, den der Beurteiler benannt hat

**Anlass: Autor-Entscheid A39 vom 2026-09-09 („Saat-Abstand als nächster
Kette-Arm: ja").** Runde 9 hat die Claim-Familie geschlossen und dabei
etwas anderes benannt: „warum sieht das er in `Wer` gut aus, in
`unter`/`regieren` richtig schlimm?" Die Antwort im Eintrag „Kette K-E
`sep09`" war eine These — die SAAT liege dort zu weit von der Tinte —,
gestützt auf die `aug20`-Autopsie zu K-C, Befund (d), und auf einen
ad-hoc-Zähler. Dieser Abschnitt macht daraus zuerst eine Messung über
alle 63 Wortproben und registriert danach EINEN Arm vor.

Reihenfolge, bindend: **Diagnose zuerst, dann der Arm.** Was hier unter
„Vorregistrierung" steht, stand vor der ersten Kandidatenzahl fest.

#### Zwei Sensoren, benannt und eingefroren

Beide sind **Berichte, keine Lineale**: an kein Gate gebunden, in keinem
`bench_loss`, und beide leben in `tools/` (nichts an `core/`, nichts an
`word_metric.py`, nichts an den Wurzeln).

**(1) Papier-Umkehr** (`tools.tracebench.reversals`, Glossar). Eine
Umkehr ist ein Vertex der Kandidaten-EIGENEN Polylinie, dessen beide
anliegenden Segmente (nach Ausdünnen auf ≥ 0,5 Bildpunkt) `cos < −0,7`
einschließen; sie zählt als **Papier**-Umkehr, wenn ihr Bildpunkt Papier
ist — Grauwert oberhalb der Mitte zwischen Tinten- und Papierniveau
DIESES Ausschnitts (`--paper mask` liest stattdessen die eingefrorene
`ref_mask`). Nicht resampelt: das Lineal-Resampling würde genau das kurze
Hin und Her löschen, das hier gezählt wird. Die Trennung Papier/Tinte ist
die Lehre der Runde 9 und dieselbe wie beim Kringel-Katalog — ein Sensor
ohne Landmarken-Bewusstsein misst den Duktus statt des Defekts:
`Galoppieren` trägt auf beiden Armen **24 Umkehren in der Tinte und 0 im
Papier**, und der Autor hat genau dieses Wort als gut aussehend benannt.

**Offenlegung zur Abnahme, ohne Beschönigung.** Der ad-hoc-Zähler der
Runde 9 war nie im Repo; sein Wortlaut ist damit nicht rekonstruierbar,
und der eingefrorene Sensor reproduziert dessen **29 : 27 nicht exakt**.
...
komponierten Platzierung, exakt `chain._letter_spec`) von der
K-C-gereinigten Tinte, in xh, in zwei Hälften mit verschiedenen Heilungen:

* **Saat-Versatz** — die ganze-Buchstaben-Verschiebung, die die
  beschränkte Rastersuche des Folgers selbst findet
  (`harvest._grid_fits`, dieselbe Funktion, die die Deckungsfenster
  liefert). Ein reiner Platzierungsfehler; der Kettenlöser kann ihn über
  seinen Slot-Block absorbieren, soweit dessen Schranken reichen
  (`FIT_DX_UNITS` 0,6 / `FIT_DY_UNITS` 0,20 xh).
* **Saat-Rest** — der Anker-Abstand, der NACH dieser besten Verschiebung
  bleibt. Keine Platzierung nimmt ihn weg: er ist die Form des
  komponierten Buchstabens gegen die Form der Hand (das komponierte `e`
  1,32 xh breit gegen 0,65 xh der Hand, `aug20`).

#### Die Diagnose über alle 63 Wörter

Wurzel `suetterlin-1922` `exported_at` 2026-09-07T20:07:03+00:00
`root_digest` **`ccb036a5eb20…`** (Paare `0fbde2d72b64…`, unberührt),
jeder Lauf mit `--expect-root`, BLAS gepinnt. 63 von 63 Wörtern gemessen.

**Erster Befund, und er räumt eine These ab: „die Saat liegt weiter weg
als `max_delta`" ist als ABSTAND falsch.** Kein einziger Saat-Anker der
63 Wörter liegt weiter als **0,68 xh** von irgendeiner Tinte, das
Set-Maximum des Saat-Rests ist 0,648 — die 0,75-xh-Schranke wird nirgends
gerissen. Der Satz aus der `aug20`-Autopsie („Saatfehler am `e`-Austritt
0,81 xh gegen `max_delta` 0,75") ist kein Abstand ZUR Tinte, sondern eine
Korrespondenz: der Austritt liegt nah an der FALSCHEN Tinte. Genau das
ist der Mechanismus, den Runde 9 beschreibt — nicht „erreicht die Tinte
nicht", sondern „erreicht die richtige Tinte nicht, und die falsche liegt
bequem in Reichweite".

**Zweiter Befund: die Frage des Beurteilers hat eine Zahl.** Slot für
Slot, `e`→`r` in drei Wörtern (Verschiebung in xh, Saat-Rest Median/Max):

| Wort | Papier-Umkehren (Basis) | `e`: Versatz | `e`: Saat-Rest med/max | Spannweite `shift_x` über das Wort |
|---|---|---|---|---|
| `Wer` | **0** | (+0,000, +0,000) | 0,031 / 0,177 | 0,375 |
| `er` | **0** | (+0,065, −0,032) | 0,032 / 0,116 | 0,161 |
| `unter` | **5** | (−0,161, −0,032) | 0,097 / **0,376** | 0,290 |
| `regieren` | **4** | (+0,100 / +0,367 / −0,600) | bis 0,335 | **0,967** |

In `Wer` verlangt das `e` **keine** Verschiebung; in `unter` muss es
zurückgezogen werden und trägt den höchsten Saat-Rest des Wortes; in
`regieren` verlangen die acht Slots Verschiebungen von −0,600 bis +0,367
xh — die Buchstaben ziehen um **0,967 xh auseinander**, und ein Slot
steht an der Blockschranke. Eine globale Verschiebung (die Kette hat
genau eine) kann das per Konstruktion nicht reparieren. Das ist die
Breiten-Geschichte der K-C-Autopsie, ohne eine Buchstaben-Segmentierung
der Hand zu brauchen.

**Dritter Befund: welche Hälfte den Defekt vorhersagt.** Über die 44
beurteilten Wörter, Papier-Umkehren der Basis gegen die Saat-Statistiken
(Pearson · Spearman · Partial bei kontrollierter Slot-Zahl, weil lange
Wörter mehr Gelegenheiten haben):

| Spalte | Pearson | Spearman | partiell \| Slots |
|---|---|---|---|
| **Saat-Rest (`rest_max`)** | **+0,691** | **+0,507** | **+0,592** |
| Saat-Rest p90 | +0,569 | +0,360 | +0,437 |
| Spannweite `shift_x` | +0,417 | +0,234 | +0,157 |
| Versatz (`shift_max`) | +0,378 | +0,219 | +0,151 |
| Saat-Abstand (`seed_max`) | +0,398 | +0,300 | +0,160 |
| Slot-Zahl allein | +0,447 | +0,475 | — |

In Terzilen, je Slot normiert (13 der 44 Wörter tragen überhaupt eine
Papier-Umkehr):

| Terzil | Saat-Rest | Papier-Umkehren je Slot | betroffene Wörter |
|---|---|---|---|
| niedrig | 0,116–0,236 | **0,020** | 1 von 14 |
| mittel | 0,238–0,285 | 0,059 | 3 von 14 |
...
| Saat-Abstand (`seed_max`) | +0,345 | +0,278 | +0,161 |
...
**Der Arm ist `--chain-seed grid`.** Statt jeden Slot-Block bei null (der
komponierten Platzierung) zu starten, startet er bei der Verschiebung,
die die Rastersuche für diesen Buchstaben gefunden hat — genau der
`slot_shift_init`-Weg, den `chain.fit_word_chain` seit der
Runde-2-Schlichtung trägt und den `follow.py` als Schalter schon hat.
Basis ist die **Produktions-Kette v5 ohne jedes Flag**; alles andere ist
Byte für Byte derselbe Stapel.

**Warum genau dieser und kein anderer.**

* Er ändert die **SAAT**, nicht den Anspruch und nicht das Ziel: die
  Zielfunktion, die Gewichte, der Wächter, die Evidenz und das Lineal
  bleiben unangetastet. „Das ändert, welches Becken der Abstieg betritt,
  nie, was gemessen wird" — der Satz steht seit der Runde-2-Schlichtung
  im Docstring und ist hier die ganze Begründung.
* Er ist **werkzeugseitig**: `core/` wird nicht angefasst, die goldene
  Kompositions-Fixture bleibt unberührt, und kein `/write/*`-Vertrag
  bewegt sich.
* Er kann **keine Struktur erfinden**: der Seed ist eine reine
  Translation je Buchstabe aus einer ganzzahligen Rastersuche innerhalb
  ±0,6/±0,20 xh, und Slots, deren Rastersuche an ihrer eigenen Schranke
  endet (`at_bound`), bekommen gar keinen Seed. Die Werte werden zudem
  knapp INNERHALB der Blockschranken geklemmt, damit kein Seed den
  `slot_at_bound`-Test vorab auslösen kann.
* Er zielt auf die Hälfte, die **der Folger überhaupt heilen kann**. Die
  andere Hälfte — der Saat-Rest, also die komponierte Buchstabenbreite —
  ist der seit `aug16` offene **Composer-Auftrag e-Breite** (§7.2) und
  ausdrücklich KEIN Folger-Arm; er wird hier nicht angefasst, weil er
  `core/compose.py` bewegt, also Produktoberfläche unter der goldenen
  Fixture.
...
offen und ZUERST fällig: K-F (§14 `sep04`) hat unfreiwillig eine
Nullprobe gefahren, in der 23 Wörter einen um höchstens **1,8·10⁻¹⁵**
abweichenden Init bekamen und trotzdem **neun Wächter-Verdikte**
kippten, mit aiou −0,0298 … +0,0800 und vier Verlierern unter der
0,003-Schranke. Diese Runde umgeht den Vorbehalt nicht, sie stellt sich
...
2. **Für das EINZELNE Wort bleibt der Vorbehalt gültig.** Die Auflage
   ...
   (`revert-init` · `revert-r1` · `zonal` · angenommen) berichtet. Ein
   Wort, das sich NUR über einen gekippten Wächter-Ausgang bewegt, wird
   als solches benannt. Die Gates (a)–(e) sind Set-Aussagen, keine
   Wortaussagen — Gate (a) bleibt trotzdem ein Veto-Gate, weil es die
   schlimmste Einzelbewegung begrenzt und nicht ihren Grund erklärt.

**Basis, ziffernweise nachzuweisen vor der ersten Arm-Zahl** (Kette v5,
Wurzel `ccb036a5eb20…`): dev-19 **dtw 0,045881 med · p90 0,088356 · aiou
0,7660 · Marken 0/0/0 · `cross_missing` 11 / `cross_spurious` 9**,
63er-Soll-Abstand **85**. Weicht die frisch gefahrene Basis davon ab,
bricht die Runde ab, statt die Abweichung wegzuerklären.

**Gates.**

* **(a) Deckung.** Kein dev-19-Wort verliert mehr als **0,003 aiou** —
  das stehende Gate, unverändert und nicht aufweichbar.
* **(b) Struktur.** `cross_missing` ≤ 11, `retrace_spurious` ≤ 10,
...
**Kill-Kriterium:** (a) oder (c) rot = keine Adoptionsvorlage, Punkt.
(e) rot bei sonst grünen Gates ist ein **ehrliches Negativ mit Befund**:
dann ist die Platzierungs-Hälfte nicht der Träger, und die Rettungswege
gehen an die Form-Hälfte.

**Was ein Ergebnis ausdrücklich NICHT auslöst.** Auch ein rundum grüner
Arm macht `--chain-seed grid` nicht zum Default: der Folger-Default
ändert die gefahrene Bahn, ist damit ein Re-Baseline-Write und
Autorensache. Diese Runde darf ihn VORLEGEN, mehr nicht.
...
3. Die Wörter, deren Papier-Umkehren in der FORM-Hälfte sitzen
   (`Zügel`: Spannweite 0,133 bei Saat-Rest 0,354), heilen **nicht**.
...
| (c) Referenzfrei | 63er-Soll-Abstand ≤ 85 | **85 → 74** (11 besser · 45 gleich · 7 schlechter) | **GRÜN** |
...
| (f) Determinismus | zweiter Lauf strich-identisch | **63/63 strich-identisch** samt Registrierung. Die DATEIEN sind nicht byte-gleich, und zwar aus genau einem Grund: `meta.timings.seconds` und das `seconds` je Runde sind Wanduhr (105 Runden-Felder, 63 Wort-Felder). Kein anderes Feld weicht ab | **GRÜN** |
| (g) Stack | `k0eval` meldet beide Stapel gleich | `stack_mismatch: false`, alle sechs Flags identisch (`chain_seed` ist kein Stack-Flag, und das ist richtig so: der Stapel ist derselbe, nur der Startort nicht) | **GRÜN** |
...
**Die 63er-aiou-Lage, vollständig statt nur auf dev-19:** über alle 63
Wörter min **−0,1782** (`Gewehr`) · Median −0,0004 · max **+0,1118**
(`han`), **15 Wörter unter der −0,003-Schranke**. Auf dev-19 sind es
zwei; die dev-19-Zahl ist die vorregistrierte, die 63er die
vollständigere, und beide stehen hier, damit niemand die kleinere
zitieren kann.
...
**Die Wächter-Ausgänge, wie vorregistriert berichtet — und sie sind der
eigentliche Ertrag.** `k0eval` klassifiziert je Wort: Basis
`revert-init` 17 · `revert-r1` 11 · `zonal` 18 · `clean` 17; Arm
`revert-init` 17 · `revert-r1` **5** · `zonal` **20** · `clean` 17 ·
`halved` **4**. **28 der 63 Wörter wechseln ihr Verdikt**, und die
aiou-Enden liegen fast vollständig in diesem Wechsel:

| Klasse | n | aiou-Median | Spanne | Soll |
|---|---|---|---|---|
| wechselt ZU `revert-init` | 6 | **−0,0770** | −0,1782 … −0,0221 | −5 |
| wechselt WEG von `revert-init` | 6 | **+0,1034** | +0,0104 … +0,1118 | +1 |
| Verdikt unverändert | 35 | −0,0005 | −0,0249 … +0,0301 | ±0 |

**Die ganze Soll-Verbesserung (−11) sitzt in den 28 Wörtern mit
gewechseltem Verdikt; die 35 mit gleichem Verdikt bewegen sie um 0.**
Das ist die K-F-Lehre in Zahlen: was der Saat-Arm auf dem Lineal tut,
geht über den Wächter, nicht direkt über die Tinte. Die K-F-Auflage
bleibt darum in Kraft — **kein einziges Wort wird aus dieser Runde
einzeln adoptiert oder verworfen** —, und sie liefert nebenbei die
erste Eichung für den offenen Arm „Wächter-Verdikt als Messgröße statt
Etikett" (§7.11).
...
`regieren` ist eines von **vier** Wörtern des Satzes, deren Rastersuche
an ihrer eigenen Schranke endet (`Soldaten`, `macht` und `schießen` sind
die anderen). Slot 6, das dritte `e`, will **−0,600 xh** — genau
`FIT_DX_UNITS`. Der Schalter überspringt solche Slots per Konstruktion
...
**Und der gemischte Start allein erklärt es NICHT** — das ist eine
Korrektur an der ersten, bequemeren Fassung dieses Absatzes, die aus
einem fehlerhaften Sensor stammte (er rechnete die Schranken-Lage aus
der normierten Verschiebung nach, statt `at_bound` der Rastersuche zu
lesen; PR-Review-Befund, Zähler korrigiert und Inventar neu gefahren).
Von den vier gemischt startenden Wörtern wird nur `regieren`
schlechter: `Soldaten` **4 → 3**, `schießen` 1 : 1, `macht` 0 : 0. Der
gemischte Start ist also eine notwendig aussehende, aber ersichtlich
NICHT hinreichende Bedingung. Was `regieren` von den drei anderen
unterscheidet, ist die Größe: sein Schranken-Slot verlangt mit −0,600 xh
die größte Verschiebung des Satzes, und er sitzt im Wort mit der größten
Spannweite (0,967 gegen 0,781 · 0,812 · 0,500). Der Rettungsweg unten
bleibt derselbe, seine Begründung ist jetzt aber eine schwächere: er ist
die naheliegende Reparatur, nicht die bewiesene.
...
#### Runde 11, gebaut und ungeurteilt

`temp/runden-sep09/kette-k-g/humanbench/runde-11-k-g.html` (4,4 MB) mit
`runde-11-plan.md` daneben. **73 Bildschirme** = alle **63** Wörter +
10 gespiegelte Wiederholungen, Saat 20260011, **Zoom 4× auf einem
Ausschnitt von ±1,5 xh** um die größte Arm-Trennung
(`--window-xh 1.5`, die Instrument-Änderung dieses PRs,
`menschliche-bewertung.md` §3.4a). Arm-Prüfsummen im Stempel: Basis
`82d857515188c0f3` · K-G `93d148449403df73`; beide Arme registrieren auf
allen 63 Wörtern identisch, `--registration-from` war nicht nötig.
Die Frage ist wieder die Genauigkeitsfrage (`VERGLEICH/11`), weil beide
Arme Bahnen sind.

Vier Klassen, vor jedem Label geschnitten und je mit dem Versatz UND dem
Rahmen, in dem er sichtbar wäre: `zickzack-heilt` **9** (Papier-Umkehren
fallen; 0,0255–0,3192 xh) · `zickzack-schlimmer` **7** (steigen;
0,0318–0,5886, unter dem Klassenboden 8, also namentlich) · `bewegt`
**39** (gleich viele Umkehren, Bahn bewegt; keine Sichtbarkeit
behauptet) · `kaum-bewegt` **8** (< 0,02 xh, der **Nullproben-Ersatz** —
strich-identische Paare gibt es in dieser Runde nicht, weil kein Wort
unbewegt bleibt). Unentschieden-Boden **12,7 %** gegen die Schranke
≤ 25 %.

**Was die Runde ausdrücklich NICHT kann:** den Arm adoptieren. Gate (a)
ist gerissen, und ein Menschenurteil hebt kein Lineal-Gate auf. Sie
beantwortet die INSTRUMENT-Frage der Runde 9 und die Richtungsfrage;
ihre Zahlen sind mit denen der Wortrunden 5–9 nicht vergleichbar.
