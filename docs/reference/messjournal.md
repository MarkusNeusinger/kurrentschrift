...
| sep09 | Laufform | [LF17 `d`-Zeile als eigener Arm (A40) + Runde 10](#laufform-lf17-sep09--vorregistrierung-die-d-zeile-als-eigener-arm-a40-und-die-runde-10) | Pre-Reg + Konstruktionsmessung · **Runde 10 gebaut, Urteil offen** (Autor-Entscheid A40) | Rettungsweg (1) der LF16-`sep08`-Zeile mit EINEM Freiheitsgrad: gespeicherte Zeilen plus ein Overlay aus der frisch geernteten `d`-Zeile, Ernte wieder Fixpunkt (zwei Läufe byte-gleich). **Zeile und Basis-Arm kommen byte-identisch mit denen der Runde 8 heraus** — der Komma-Ausschluss berührt kein `d`-Wort, die Wurzel-Einschränkung jenes Eintrags geht für sie auf null. Die Zeile nimmt alle drei Write-Gates; das Lineal ist wieder leicht dagegen (0,108153 → 0,108339, zerlegt 5 : 9), die Sichtbarkeit ist vorab an eine Zahl gebunden und trifft sie (Median-`arm_gap` 0,1030 gegen 0,05). Vorab benannt auch, was die Runde NICHT kann: die Tie-Schranke über alle Bildschirme ist durch die nötigen Nullproben per Konstruktion gerissen |
