...
`s`#0 und `k`#2), und die **e-Öse ist der größte relative Verlust überhaupt**
(0,07 gezeichnet, wo die Platte 0,38 hält) und zugleich **wechselnd** — die
Platte zeigt sie nur in 1 von 4 Vorkommen offen, also kein `offen`-Defekt,
sondern genau der Fall „je nach Tinte".
...
gelieferte Feder zulaufen lässt; `wechselnd` steht getrennt daneben,
