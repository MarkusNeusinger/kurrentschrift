...
| sep13 | **Saat-Korrespondenz als Vorkommens-Quelle** (`harvest --occurrences tintenpfad`, Autor-Entscheid **A48**, Weg 1 der §7.11-Zeile) | die Anker einer Laufform-Zeile über die SAAT des Dekoders zuordnen statt über die Bogenlänge: Zustand → Saat-Probe → Stelle im komponierten Buchstaben → Tafelanker, das eine Glied ohne Compose-Buchführung je Punkt affin BEWIESEN (lokal, weil der `d`-Anstieg stückweise ist) | **ehrliches Negativ an der Abdeckung, Mechanismus bestätigt:** Gate (K) steht, wie vorregistriert über die Körper-Items gelesen, bei **328 von 334 = 98,2 %** (Schranke 80 %, Beweis-Reste Median 8,0e-15 je Slot, max 1,03e-13); die Zuordnung greift damit auf **277 von 277** Slots, und gesetzt werden **92,5 %** aller Anker (30 736 / 33 240) — als Vorkommens-Quelle fällt sie, weil die Komposition die Kopplungs-Enden wegschneidet: **7,5 %** aller Anker ohne Stelle (Median-Slot 7 von 120), 65 vollständige Slots, **58 von 277** Vorkommen gegen 227 der Fit-Kontrolle, Zeilen mit n ≥ 4 5 statt 16. Fixpunkt grün (zwei Ernten byte-gleich), `core/` unberührt. Lineal asymmetrisch: gegen die stehende Karte Wörter **0,104883** (−0,003456) bei Paaren +0,001070 (Schranke gerissen, kein Kill); gegen die schlüsselgleiche Kontrolle (0,108951 / 0,148119) Wörter −0,004068 und gepaart **31 : 21 : 44** — auf viel dünnerer Evidenz (`e` n = 3 gegen 30). Nichts adoptiert, nichts geschrieben; `--apply` verweigert den Arm | „Laufform A48 `sep13`“ |
| sep24 | **Folger-Eingabe-Leiter** auf der Eigenhand (`tools/eigenhand/pfad.py`; Dekoder unverändert A45) | drei kumulative Eingabestufen, eine je Sprosse: Beschriftungszonen · Plattenmaßstab 31 px/xh · Saat-Registrierung (sy und Grundlinie aus Skelett-Moden, k aus Tinten- gegen Saatbreite); explorativ, 7 Kästen von B0001 | **A1 und A2 nicht besser, A3 raus am Platten-Wächter:** A3 hebt die Hand in 7 von 7 Kästen (cov-Median 0,736 → 0,959), reißt aber den dev-19-aiou-Median (0,7929 → 0,7895 bei dtw 0,038351 → 0,036680). Die Beschriftungszonen sind trotzdem Vorgabe (Autor-Entscheid als Korrektur der Eingabe; der Plattenpfad bleibt byte-gleich), die anderen beiden aus; Rettungswege R-gate und R-split (§7.9) | „Folger-Eingabe-Leiter `sep24`“ |
| sep25 | **Stufe 3 gezielt, Stufe (a)** auf der Eigenhand (`--register-seed {k,ky}` hinter dem Stufe-3-Tor, Arm-Commit `d50f886`; Dekoder unverändert A45) | Leiter C0 (Beschriftungszonen + Plattenmaßstab) → R-k (nur die x-Skala der Saat) → R-ky (dazu sy + Grundlinie); Stufe 3 nur auf Bogen-Eingabe (R-gate), zerlegt (R-split); explorativ, dieselben 7 Kästen | **R-k besteht, R-ky schlägt R-k nicht — k trägt den Gewinn:** R-k gegen C0 7 von 7, cov-Median 0,736 → 0,960, schlechtester +0,0597; R-ky gegen R-k 2 von 7, Median-Δ 0. Platten-Wächter per Konstruktion (G1 63/63 byte-gleich, dev-19 0,038351 · 0,048012 · 0,7929; G2 63/63, 19/19). Ungetort auf der Platte: R-k aiou-Median 0,7929 (unverändert), R-ky 0,7895. Nominiert für (b): Plattenmaßstab + R-k; (b) wartet auf B0005 | „Stufe 3 gezielt `sep24b`, Stufe (a)“ |
...
- **Vorkommen aus der Tintenpfad-Bahn** — Weg (1), die **Saat-Korrespondenz**,
  ...
  werden davon 92,5 % der Anker) und fällt als
  Vorkommens-Quelle, weil die Komposition die Kopplungs-Enden wegschneidet
  (7,5 % aller Anker ohne Stelle, Median-Slot 7 von 120, 58 statt 227
  Vorkommen). Offen ist
