...
| sep09 | **K-E vor dem Auge** (Runde 9 geurteilt) | derselbe `mark_claim`, ein Knopf; gefolgte BAHNEN als Mittellinie, Genauigkeitsfrage | **Ehrliches Negativ, die Claim-Familie ist endgültig zu:** 44 von 44 Bildschirmen „kein Unterschied" (100 % gegen ≤ 25 %), kein entschiedener Bildschirm, `adopt: false`. Nullproben 6/6 richtig, also keine Antwortgewohnheit; die Dosis erklärt es auch nicht (fünf Wörter über den 0,1186 xh der Runde 5, bis `Zügel` 0,3221 — alle unentschieden). **Beide Bahnen tragen dieselbe dominante Störung** (Zickzack-Umkehren im Papier 29 : 27, ad-hoc-Zähler, kein Lineal); der Claim heilt `Sprünge` 4 → 0 und verschlechtert `regieren` 4 → 9. Keine Zahl der `sep07`-Zeile bewegt sich, keine Headline. Hebel laut Beurteiler: die SAAT-Distanz (§7.2 e-Breite), nicht der Claim | „Kette K-E `sep09`" |
| sep09 | **K-G Saat-Registrierung** (Autor-Entscheid A39, Rettungsweg 1 der Runde 9) | `--chain-seed grid` — der Slot-Block startet bei der Verschiebung, die die beschränkte Rastersuche für DIESEN Buchstaben gefunden hat, statt bei der komponierten Platzierung. Ein Startort, kein Anspruch und kein Objektiv: Zielfunktion, Gewichte, Wächter, Evidenz und Lineal unverändert | **Ehrliches Negativ, nicht adoptiert — und der wertvollste Verlierer der Kampagne.** Gerissen: (a) aiou an `Wer` −0,0187 und `das` −0,0074 (über alle 63: 15 Verlierer, min −0,1782 an `Gewehr`), (b) `cross_missing` **11 → 13**, (d) dtw-p90 **+0,000425** bei Median −0,000034. Grün: (c) 63er-Soll **85 → 74**, (e) Papier-Umkehren **49 → 42**, (f) Determinismus 63/63 strich-identisch (nur Wanduhr-Felder weichen ab), (g) Stapel identisch. **`unter` — das Wort, das der Beurteiler der Runde 9 benannt hat — gewinnt +0,1072 aiou und verliert alle fünf Papier-Umkehren**, weil die Basis dort ihre erste Runde ganz verwirft und der Arm nicht; `Wer` verliert. Die aiou-Enden hängen am WÄCHTER, nicht an der Tinte: 28 von 63 Wörtern wechseln ihr Verdikt, zu `revert-init` −0,0770 Median, davon weg +0,1034, unverändert −0,0005 — und die gesamte Soll-Verbesserung sitzt in den 28. Zerlegung: ohne `regieren` (einziges Wort mit einem Slot AN der Blockschranke, startet gemischt) stehen die Umkehren 45 → 32. Zieht erstmals den K-D-Wiedervorlage-Auslöser (`regieren` 0,427 xh Papier-Exkursion). Konversionen in §7.9: bedingte Saat je Slot · den Schranken-Slot klemmen statt auslassen | „Kette K-G Saat-Registrierung `sep09`" |
...
denselben Kanälen; stehende Wege §7.9: humanbench-Tie-Breaker — **`sep09`
gegangen und ausgegangen**, Runde 9 entscheidet nichts (44 von 44 „kein
Unterschied"), die Familie ist damit endgültig zu ·
Distanzfeld-NUR-Claim als einziger offener Weg, frische Pre-Reg) · A2 (SDM + Dichtebewusstheit, Welle 2 — Ziele
