...
| Pfad | Inhalt | Status |
|---|---|---|
| `/admin` | **Einstieg: die Vorlagen-Auswahl.** Alles darunter gehört zu genau einer Quelle und ihrer Hand, also steht die Wahl am Anfang statt in einem Menü; die aktive Vorlage steht danach in der **Scope-Leiste** unter dem Header (`sections/admin/shell/ScopeBar.tsx`) und führt mit einem Klick hierher zurück — der frühere Vorlagen-Chip im Header war derselbe Link und ist entfallen (`sections/admin/shell/StartView.tsx`). Die Wahl merkt sich der Browser in `localStorage`; Vorgabe ist `CONFIG.sourceId` (`app/src/global-config.ts`) — dieselbe Konstante hat Doppeldienst: Sie ist die Quelle, aus der die ÖFFENTLICHEN Seiten rendern (heute die Sütterlin-Ausgangsschrift 1922), UND die Vorauswahl des Admins | existiert |
| `/admin/buchstaben[?g=<key>][&ansicht=liste\|galerie&filter=&sort=&seite=]` | **Buchstaben.** Ohne `g` die Alphabet-Übersicht (ehemals `/admin/vergleich`-Tab) als **Arbeitsliste**: je Buchstabe EINE Zeile — Glyph, Schlüssel, Score-Chip, Abzüge, die Chips gesperrt · Laufform · Vorkommen · Korb und **kein Bild**; sie klappt an Ort und Stelle die VIER Flächen auf (Original (Chart-Crop) · Tafel-Form (Variante 0) · Laufform (Variante 100) · „Median & Vorkommen", die H1-Aggregat-Skizze), `ansicht=galerie` bringt die Kartenwand zurück (Vorgabe V14). Der Listenzustand (`ansicht` · `filter` · `sort` · `seite`) steht in der URL, nie im `localStorage`; Werte, Rückfall-Regel und Module im Glossar unter „Arbeitsliste" und in §7. Mit `g` der einzelne Buchstabe mit allen Werkzeugen: Tafel-Ausschnitt + Einrichtungs-Wizard + Diagnose + aufklappbarem Chart-Editor (ehemals `/admin/chart`), Tafel-Form neben Laufform, zuschaltbar Landmarken- und Abzugs-Linse ([Werkbank §8/§9](../proposals/optimierungs-werkbank.md)), die Vorkommen aus den Wörtern, die H1-Statistik samt Frische-Chip und Differenz-Skizze, die Absprünge zu Übergängen/Wörtern — und am Fuß, bewusst abgesetzt, der **Laufform-Übernahme-Block** mit Bestätigungsdialog (`sections/admin/letters/`, Issue #270) | existiert |
| `/admin/uebergaenge[?l=<key>][&r=<key>][&ansicht=liste\|galerie&filter=&sort=]` | **Übergänge.** Ohne Paar die Matrix aller Zweierkombinationen EINES Buchstabens (ehemals `/admin/paare`) als **Arbeitsliste**: je Zelle die Kombination und ihre Zähler in Worten — Vorkommen, Übersteuerung, Korb-Aufträge — und **kein Bild**; `ansicht=galerie` komponiert sie zusätzlich (V14). Chips mit Zählern, UND-verknüpft; kein `seite`, weil der Anker bereits blättert. `l=` allein ist kein Paar, sondern dieser Anker — auch zurück aus einem Übergang. Daneben das Freitextfeld für JEDE Kombination. Mit Paar die komponierte Verbindung, die H2-Statistik „gemessen vs. komponiert", die dissezierten Vorkommen und — als letztes Mittel — der Paar-Editor (`sections/admin/joins/`, `sections/admin/pairs/`) | existiert |
| `/admin/woerter[?w=<text>&s=<specimen>][&ansicht=liste\|galerie&filter=&sort=&status=&reiter=woerter\|andere\|nachgefahren&seite=]` | **Wörter.** Ohne `w` die Wortproben der Vorlage als **Arbeitsliste** (ehemals `/admin/vergleich`-Tabs Wörter/Andere Hand): je Wortprobe EINE Zeile — Wort, Bahnen, Nachfahr-Status, „Andere Hand" bei den Abb.-22-Proben, Korb-Aufträge, der Loss sobald berechnet — und **kein Bild**; sie klappt an Ort und Stelle die zwei Flächen auf (Original · Wie geschrieben), `ansicht=galerie` bringt die Kartenwand zurück (V14). Sechs Achsen (Entscheid Q5 a), Werte im Glossar; `filter` trägt hier den FREITEXT „Proben filtern". „Scores berechnen & sortieren" bewertet weiter den GANZEN Reiter und schreibt seine Rangfolge als `sort=schlechteste`, solange ein berechneter Loss sie trägt. Das Feld „Wort oder Satz" ist keine Achse: es schreibt ein Subjekt (`w=`). Mit `w` ein beliebiger Text: wie die Engine ihn schreibt, woraus er besteht (Buchstaben + Übergänge als Absprünge) und — wo eine Platte ihn enthält — die nachgefahrene Bahn mit Vorkommens-Overlay, Score und Wort-Editor (ehemals `/admin/belege` + `/admin/werkbank`-Rückgrat; `sections/admin/words/`) | existiert |
| `/admin/eigenhand[?reiter=bestand\|streifen\|statistik\|drucken][&item=&wort=]` | **Eigenhand.** Die einzige Admin-Ansicht, die zu einer HAND gehört statt zu einer Vorlage. Seit dem `?reiter=`-Split (Plan V2) EINE Route mit vier Unteransichten; ohne `reiter` — und bei jedem unbekannten Wert — landet man auf `bestand`. `reiter` heißt „welcher Reiter dieser Seite"; `ansicht` gehört der Liste/Galerie-Umschaltung der Übersichten (V14, Entscheid Q1 c) und wird hier nie als Unteransicht gelesen. **bestand**: Hand-Auswahl, stehendes Setup der Hand (Feder · Tinte · Papier · Gerät), die Zähler (Streifen belegt/unterwegs/geplant, Fassungen, Bögen), die Warteschlange, welche Zeichen und Übergänge belegt sind — gemessen an dem, was der Streifenplan hergibt, Groß-/Kleinbuchstaben, Ligaturen, Ziffern und Sonderzeichen getrennt — und die Quoten. **streifen**: die GESCHRIEBENEN Streifen, jede gespeicherte Fassung auf Klick, samt Ausschnitt je Wort — admin-gesichert, `private, no-store`, nie im Repository; der Filter steht in `item` (`a`, `a@medial`, `a>b`) und `wort` und wird deshalb von einer Zeichen-Zelle der Bestand-Ansicht als LINK gesetzt, nicht mehr durch einen Sprung innerhalb der Seite. **statistik**: was die Tinte sagt — heute die Feder-Halbbreite der Hand (`nib_median`/`nib_readings` aus demselben Bestand-Abruf, serverseitig `core.eigenhand.befund.hand_nib` über die angenommenen, gemessenen Fassungen der Kartei; eine fehlende Messung bleibt „nicht gemessen" und wird nie Null) und die Tintentreue-Verteilung (die Ampel je Kasten aus `GET /eigenhand/pfade/{hand}`, über die ganze Hand GEZÄHLT und nie zu einer Zahl verrechnet: folgt · folgt teils · folgt nicht mit dem entscheidenden Sensor, grau mit seinen Gründen, auf Wunsch je Fassung; „vorläufig", solange die Schwellen geliehen sind; ein Link in die Nachfahr-Liste nur, wo eine Listenachse genau diese Kästen wählt), daneben beschriftete Leerflächen für Belegverlauf und Ausschnitt-Stapel. **drucken**: der Bogendruck (Stapel erzeugen, PDF öffnen). Der Bestand wird EINMAL für alle vier gelesen, die zuletzt gedruckten Bogen-Ids liegen in der Schale. Die HAND dagegen liegt im äußeren `AdminProvider` (`shell/handScope.ts`): ein Arbeitsbereich wie die Vorlage, in der Scope-Leiste auf jeder Admin-Seite, einen Vorlagen-Wechsel überdauernd und immer vom Stil der Vorlage (→ Hand-Stil-Kopplung, V19, je Stil gemerkt). Die Auswahl hier bietet darum nur die Hände dieses Stils und bleibt die eine Stelle, an der GEWÄHLT wird. Die Scans bleiben lokal; hochgeladen wird hier nichts (`sections/admin/eigenhand/`, [`../proposals/eigenhand-erfassung.md`](../proposals/eigenhand-erfassung.md) §7.1–§7.2) | existiert |
...
- `sections/admin/letters/` — `LetterView` (`/admin/buchstaben`): Übersicht
  über `LetterOverview`, das die Werkzeugleiste, den Score-Read und „Neu
  laden" hält und darunter entweder die **Arbeitsliste** (`LetterList`,
  Vorgabe) oder die **Galerie** (`compare/GlyphComparison`) zeigt. Die Zeile
  der Liste trägt Glyph, Schlüssel, Score, Abzüge und Chips, aber kein Bild;
  aufgeklappt mountet sie dieselbe `compare/CompareCard` wie die Galerie
  (`unmountOnExit`, also lädt eine geschlossene Zeile nichts). Welche Zeilen
  überhaupt auf dem Schirm sind, entscheidet EINE Stelle — Filter, Sortierung
  und Seite aus der URL über `shell/listState.ts` und `letters/letterRows.ts`
  —, damit Liste und Galerie nie verschiedene Antworten geben. Die Karte
  selbst zeigt unverändert jeden autorierten Buchstaben mit
  ...
  (Vorkommen · Abstand · „neu"), Bestätigung, danach der Bericht. Zwei
  zuschaltbare Linsen, beide zu, beide `unmountOnExit`: `LandmarkPanel`/
  `LandmarkOverlay` auf der geschriebenen Form (Werkbank §8) und
  `PenaltyPanel`/`PenaltyOverlay` — die **Abzugs-Linse** — über dem
  Tafel-Ausschnitt in dessen eigenen Pixeln (Werkbank §9); ihre reinen
  Entscheidungen (Markenform je Kategorie, Legenden-Chips, „gespeichert"-
  Zeile, Pin-Stellung) stehen in `letters/penaltyLens.ts`, die Farben in
  `styles/paper.ts` `penalty`.
- `sections/admin/joins/` — `JoinView` (`/admin/uebergaenge`): Matrix +
  Freitext-Kombination, komponierte Verbindung, H2-Statistik, Vorkommen,
  Paar-Editor. Die Matrix (`pairs/PairMatrix`) ist die **Arbeitsliste** der
  Ansicht: sie hält den Ankerbuchstaben (`l=`), die Werkzeugleiste und den
  Override-Read, das Zeilenmodell `pairs/pairRows.ts` sagt je Zelle, was sie
  weiß, und `pairs/PairCells.tsx` zeichnet sie — in der Vorgabe als ZÄHLER
  IN WORTEN ohne Komposition, in der Galerie zusätzlich komponiert. Unter
  einem fokussierten Übergang steht dieselbe Matrix `embedded`: eigener
  Anker, komponierte Zellen, nichts davon in der URL — sie ist ein
...
- `sections/admin/eigenhand/` — der Bestand einer HAND (nicht einer Vorlage)
  und der Bogendruck. `EigenhandView` ist seit dem `?reiter=`-Split (V2)
  nur noch die **Schale**: sie hält die Hand, den EINEN Bestand-Abruf
  hinter allen vier Unteransichten, die zuletzt gedruckten Bogen-Ids und
  den Umschalter, und rendert dann genau eine von `BestandView`,
  `StripsPanel`, `StatistikView`, `DruckenView`. Ein Abruf je Ansicht hätte
  denselben Payload viermal geholt und dabei den `angenommen`-Zähler
  verloren, an dem die Streifen-Galerie ihren Cache bricht; deshalb bleibt
  auch das eine Fehler-Gatter oben statt vier Spinnern unten. Der
  Umschalter ist eine `ToggleButtonGroup` aus `ToggleButton
  component={RouterLink}` — Links, damit Mittelklick, „Link kopieren" und
  der Zurück-Knopf weiter tragen; MUI-`Tabs` hat in `app/src` keinen
  Präzedenzfall. Die Zahlen kommen fertig aus `GET
  /eigenhand/bestand/{hand}` — dieselbe Rechenschicht, die das Terminal
  druckt —, das PDF wird geholt statt verlinkt, weil das Admin-Token in der
  Entwicklung ein HEADER ist, den ein `<a href>` nicht mitschickt. Daneben
  `SetupPanel` (das stehende Setup der Hand) und `StripsPanel` (die
  geschriebenen Streifen): dessen Bilder werden aus demselben Grund als
  Blob geholt und zusätzlich erst auf Klick, weil ein Streifen ~350 KB
  wiegt und zum reservierten Datensatz gehört; die Object-URLs werden von
  Hand wieder freigegeben. Die Herkunft einer Bahn steht dort als eigener
  Chip und darf den Folger NENNEN — „automatisch (Tintenpfad)" oder „von
  Hand" —, weil die Streifenzeile ihr `verfahren` selbst führt; die Platte
  bekommt in `werkbank.provenanceTraced` nur „automatisch", weil
  `word_instances` keinen Folger notiert. Beschriftet wird die Herkunft von
  ...
  *bestand* ein Block „Am Rechner weiter" in Server-Reihenfolge, an der
  ...
  `pfadRohzahlen`) bleiben testbar daneben liegen. Was der Bestand trägt,
  holt keine Fläche noch einmal: auch die Feder-Halbbreite der
  Statistik-Ansicht steht im Bestand-Payload (`nib_median`), weil die
  gespeicherten Streifenbilder ein Opt-in-Upload sind und eine Hand, deren
  Pixel im Privatarchiv liegen, sonst als ungemessen gälte. Die eine
  Ausnahme ist die Kasten-Frage, die der Bestand NICHT beantwortet: die
  Tintentreue-Verteilung (`TintentreuePanel`, seit 2026-09-25) liest wie
  Nachfahr-Liste und Galerie `GET /eigenhand/pfade/{hand}` über
  `useEigenhandPfadBoxes` und zählt ihn mit der reinen
  `tintentreueVerteilung.ts` — über dieselben Zeilen (`stripBoxRows.ts`)
  wie die Liste, damit „4 folgen nicht" hier und die vier roten Zeilen dort
  nicht auseinanderlaufen. Ein Link in die Liste steht nur, wo eine
  bestehende Achse (`filter=`/`status=`) GENAU die gezählten Kästen wählt;
  eine Achse je Ampelstufe hat die Liste mit Absicht nicht.
