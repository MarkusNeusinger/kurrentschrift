...
Grundform (Variante 0), im Gegensatz zur gemessenen Laufform. Welche
Aufgabe eine Tafel gegenüber Platte und Eigenhand hat, steht unter
**Drei Rollen**.

**Drei Rollen (Tafel · Platte · Eigenhand)** — die Aufgabenteilung der
drei Sorten Schriftmaterial, mit denen das Projekt arbeitet
(Autor-Entscheid 2026-09-07). Die **Tafel** (gemeinfreie Lehrtafel: Loth
1866, Petzendorfer 1889, Sütterlin-Ausgangsschrift 1922, Koch 1928) ist
das Historische — sie liefert die Geometrie der kanonischen Templates
und die Belege der Schriftkunde. Die **Platte** ist der gedruckte Bogen
mit ganzen, geschriebenen WÖRTERN derselben Vorlage (Sütterlin 1922,
...
**Pfeilhöhe** *(sagitta)* — wie weit ein Zug von der geraden Verbindung
...
die Doppelstrich-Evidenz am ß-Stamm (`ride_back_ink_ratio`, feuert auf
der Tafel nie: Stämme lesen 1,0–1,3 Federbreiten). *Arme der Ecken-Runde
(`sep12`, dieselbe Regel, Artefakte unter `temp/tintenpfad-sep11/ecken/`):*
`hairpin_tip=1` — die **Haken-Spitze**, an jedem Haken auf einem Strang
wird die Spitze wie ein Laufende gelesen, Strangrest und Kamm-Walk bis zum
Maskenende, hin und zurück (die abgeschnittene Ecke am Umkehrpunkt war die
Ausdünnung, die eine halbe Feder vor der Tintenspitze endet); `ride_back=1`
— **Rückfahrt statt Absetzen**, ein Saat-Absetzer, dessen Landung hinter
der Feder auf ihrem eigenen Strang liegt, wird über dieselbe Schiene
zurückgefahren (der ß-Stamm hinunter und hinauf, wie die Hand schreibt;
ohne Tinten-Evidenz eine reine Dekoder-Regel); `tip_grey_stop=1` — der
**Grauwert-Stopp**, jeder Spitzen-Walk (Laufenden und Haken) endet einen
Schritt, bevor der Grauwert des Crops Papier liest, der Papier-Test des
...
(`components.collinearity`): Läuft ein gerader Strich durch eine Kreuzung,
sollen die Gerade davor und die dahinter EINE Flucht bilden (Winkel `δθ`,
...
*Technisch:* `tools/wordbench/roots.py` — EINE Umsetzung für alle Bänke
(`root_digest`, `check_expected_roots`, `announce_roots`, der Flag-Helfer
`add_expect_root_argument`), Kopfzeilen `root:` / `digest=`, volle Digests
im `--json`-Report unter `roots`; im Wordbench prüft derselbe Vorspann
...
**Sichtbarkeitsschwelle** *(visibility floor)* — der Betrag, um den ein Arm
die Zeichnung bewegen muss, damit eine **Wortrunde** ihn überhaupt entscheiden
KANN; abgelesen am `arm_gap` (dem symmetrischen Tintenversatz je Wort, in
x-Höhen). Sie ist eine Eigenschaft des Instruments und **keine Schranke**:
gemessen wird sie an Runden, die dieselbe Achse geschnitten haben — Runde 5
mit einer starken Klasse ab 0,1186 xh und 9,7 % Unentschiedenen, Runde 7 mit
einer starken Klasse ab 0,0221 xh und 34,5 % (innerhalb der Runde 7 tragen die
entschiedenen Bildschirme 0,0311 xh gegen 0,0146 der unentschiedenen, innerhalb
der starken Klasse trennt der Versatz aber kaum noch). Ein Mindest-`arm_gap`
als Adoptionsgate wäre dasselbe Nachstimmen, das die Vorregistrierungs-Praxis
verhindert; was folgt, ist eine **Bau-Auflage**: wer eine Klasse „sichtbar“
nennt, nennt in der Vorregistrierung den Versatz, bei dem er das behauptet.
Der andere Ausweg ist nicht die Schranke, sondern der Rahmen — ein Ausschnitt
je Naht statt je Wort, also eine Änderung am Instrument.
→ menschliche-bewertung.md §8a („Der Auswerteplan“), messjournal.md §14
(„Übergänge J6 `sep08`“)
...
**Bahn-Arm** *(trace arm)* — eine **Arm-Datei**, deren Inhalt keine
Komposition ist, sondern eine **gefolgte Bahn**: die Mittellinie, die der
Tintenfolger über der Tinte der Platte gefahren ist, aus einer
`tools.tracebench`-Kandidatendatei in den Rahmen des Fixture-Eintrags
gerechnet (`tools/humanbench/tracearm.py`, `sep07`, Runde 9 zur
K-E-Claim-Trennung). Er trägt weder `fills` noch Strichbreiten, und daran
hängen zwei Folgen, die niemand einstellt: die Seite zeichnet ihn als
**gecaste Mittellinie über der unverblassten Platte** statt als Tinte auf
verblasstem Grund, und die Runde stellt die **Genauigkeitsfrage**
(`VERGLEICH`) statt der Echtheitsfrage — „echter geschrieben" hat an einer
Mittellinie nichts zu greifen. Eine Bahn-Runde ist mit einer
Kompositions-Wortrunde deshalb nicht vergleichbar. *Technisch:*
`build.py::draws_ink` liest die Frage an den Armen ab und schreibt sie in
Umschlag und Stempel → menschliche-bewertung.md §8a
...
Gruppe fehlender oder überschüssiger Pixel, ein Randstück, ein Stück
Mittellinie neben dem Skelett. Jede trägt ihren Teil der gezeigten
...
Registrierung und bewegt jede Saat (→ Platten-Wächter). Die dritte hat seit
`sep24b` zwei Formen (**R-split**): `k` liest nur die x-Skala, gegen die
x-Höhe des Falls, Grundlinie und x-Höhe bleiben gedruckt; `ky` ist die ganze
anisotrope Registrierung von `sep24`. Und sie steht hinter dem
**Stufe-3-Tor** (**R-gate**): sie läuft nur auf einem Fall, der aus einem
Bogen-Streifen geschnitten ist (`origin` beginnt mit `eigenhand:`); jeder
andere Fall — ein Plattenwort trägt `fixture:` oder `live:` — kommt
unverändert zurück, den Grund in `meta.input`. Damit ist der Platten-Wächter
auch für die dritte Stufe Byte-Identität per Konstruktion, mit der Kehrseite,
dass sie dem Platten-Lineal entzogen statt dort als harmlos bewiesen ist.
...
`register_seed_width` · `PLATE_MODE_CALIBRATION` · `scale_composed_x`),
`tools/eigenhand/pfad.py` (`FollowerInput` · `STANDARD_INPUT` ·
`adapt_case` · `STRIP_ORIGIN_PREFIX` · `SEED_REGISTRATIONS`,
`--mask-labels` / `--no-mask-labels` · `--resample-plate` ·
`--register-seed {k,ky}`),
...
### Admin-Redesign (geplant, 2026-09-17)

Die Begriffe des Plans `proposals/admin-redesign.md` — jeder Eintrag trägt
„geplant", bis ein PR ihn in Code bringt; die Phase-0-PRs streichen die
...
ohne Reiter. Die Wortlaute, damit die Routen-Karte sie nicht wiederholen
muss: Wörter `sort=reihenfolge|schlechteste`,
`status=alle|offen|nachgefahren|unvollstaendig`,
`reiter=woerter|andere|nachgefahren`; Übergänge
`sort=alphabet|vorkommen` und die Chips
`mit-uebersteuerung|mit-korb|ohne-vorkommen`. Eine Rangfolge, die keine
Zeile mehr trägt, verlässt die URL, statt in ihr stehen zu bleiben: nach
„Neu laden", in einem Reiter ohne Messung und bei einem eingefügten Link
ohne Scores fällt `sort=schlechteste` auf die Vorgabe zurück
(`words/wordRows.ts: rankingIsStale`) — sonst behauptete der Link eine
Ordnung, die die Liste nicht hat. → proposals/admin-redesign.md §5.0,
§5.1 (Idee 4), Vorgabe V14
...
Mindestbelegung des Bestands zählt angenommene Fassungen, die Bahn-Deckung
zählt Bahnen. → proposals/admin-redesign.md §6.4

**Belegleiste (geplant)** — je Buchstabe und Übergang eines Textes die
Wortkästen der Hand, die ihn tragen: die Brücke zwischen Platte und
Eigenhand, weil sich nur 24 von 140 Platten-Wörtern mit dem Streifen-Plan
...
archiviert und ist Trainings- und Prüfmenge des Zuordners. Korrigiert wird
die NAHT zwischen zwei Buchstaben, nie eine Grenze allein: ein Sample gehört
zu einem Buchstaben, also wandert der Nachbar mit — sonst überlappen die
...
fehlt" / „veraltet". Keine Distanzschwelle, kein neues Kriterium — das Gate
...
Platte · Eigenhand) als feste Spalten einer Detailfläche; jede Zahl trägt
den Namen ihrer Hand, zwei Hände sind ein Nebeneinander, nie eine Summe.
Dass die zweite Hand auf einer Vorlagen-Fläche stehen darf, hat die
Leitplanke „genau eine Quelle/Hand" bewegt: seit dem Autor-Entscheid vom
2026-09-18 (Q3 a) heißt sie „genau eine Hand als Subjekt; eine zweite Hand
...
**Streifen-Editor** — die Fläche, auf der der Autor einen Wortkasten mit dem
Stift NACHFÄHRT: aus einer → Nachfahr-Zeile geöffnet, Vollbild, alle Werkzeuge
über der Zeichenfläche. Ein ZWEITER, schlanker Editor neben dem Platten-Editor
(Autor-Entscheid G vom 2026-09-20) — keine Props-Naht durch den getesteten
Plattenfluss, aber dieselben Bausteine: EINE Zeichenfläche
(`shell/TraceCanvas`) und EIN Registrierungsmodul für beide, weil kopierte
Geometrie auseinanderläuft und die zwei Editoren dann uneins darüber sind, wo
ein Zug liegt. Drei Dinge unterscheiden ihn von der Platte und sind der Grund
für die eigene Fläche: die Unterlage ist ein **Blob** (die Streifen-Route ist
admin-gegatet und `private, no-store`, ein nacktes `<image href>` schickt
keinen Admin-Header); der Rahmen ist die gespeicherte → Registrierung der Bahn
MINUS dem Kastenrechteck, sodass eine gezeichnete, gespeicherte und wieder
gelesene Bahn auf denselben Pixeln landet; und **„Speichern & weiter"** geht
zum nächsten Kasten der Listen-Ordnung, ohne die Fläche zu verlassen — auf der
ETag-Marke, die der Schreibvorgang selbst zurückgibt, also ohne zweiten Read
innerhalb einer Fassung. Neben der Bahn steht das **Absetzer-Soll** mit
Warnung bei Abweichung (Prüfstein 7: sonst liefert Nachfahren still eine neue
Strichreihenfolge per Bild), und die → Buchstabengrenzen werden gezeigt und
sind mit dem Stift verschiebbar. Ein Kasten OHNE Bahn zeichnet auf der
gedruckten Lineatur des Bogens und sagt das („Saat", nie eine Messung); zwei
Dinge bietet die Fläche bewusst nicht an: einen → Skip-Eintrag (den weist der
Kasten-Schreibweg zusammen mit `authored` ab) und ein Neu-Folgen.
*Technisch:* `app/src/sections/admin/eigenhand/StripTraceEditor.tsx` mit
`stripTraceFrame.ts` (Rahmen-Rundreise), `letterSpans.ts` (Grenzen),
`LetterSpanLayer.tsx`, geschrieben über `PATCH …/pfade/{box}`.
...
(Tintenpfad) und der Span-Zuordner dauerhaft besser werden. Der Autor am
...
100 (`LAUFFORM_VARIANT`), und sie gehört faktisch der Platten-Hand.
Autor-Entscheid 2026-09-18 (Q19 a): das Band wird ein Datum der Hand
(`hands.laufform_variant`: Platte 100, Eigenhand 200), „Band ≥ 100 =
Laufform je Hand". Zwei Bedingungen hängen daran: die öffentliche Route
...
Statistik → Laufform → Vorschau. Sie folgt aus dem Leitsatz des Autors vom
2026-09-18: das Optimierungsziel ist die Eigenhand — beliebig viele
Beispiele, dauerhaft wachsend, „bis das system sie perfekt schreiben
kann" —, die Platte bleibt Maßstab und „so ok". Das Cockpit der Phase 4 und
die Arbeitslisten ordnen sich nach ihr. *Technisch:* nichts gebaut.
→ proposals/admin-redesign.md §4.5 (Leitsatz 2), §15.1

**Wortkasten (Admin-Redesign, geplant)** — der Name, den der Plan für die
Wortkiste in der Oberfläche vorschlägt („Kasten"): die Ortsangabe eines
Wortes auf dem Streifen. Bis zur Umsetzung heißt das Feld in
`eigenhand-erfassung.md` Wortkiste. *Technisch:* `boxes[]` der
Streifen-Liste, `rect_px`. → Wort-Ausschnitt (Eigenhand);
proposals/admin-redesign.md §5.0
...
**Tintenboden** — die Untergrenze, unter der eine geschriebene Zeile nicht
mehr klein gesetzt, sondern umgebrochen wird: **14 px x-Höhe**, also 14 px
je Template-Einheit (Grundlinie = 0, Mittelband = 1). Der Wert ist der
Typo-Boden des Design-Systems (§9: „Caption ≥ 14 px“) — geschriebene Formen
bekommen mindestens, was gesetzte Schrift bekommt, und brauchen eher mehr:
Was ein Sütterlin-u vom n trennt, sitzt *innerhalb* des Mittelbands und ist
ein Bruchteil davon, während eine gedruckte Letter sich als Ganzes
unterscheidet. Gemessen: Der Website-Audit 2026-09-02 fand einen
...
Texte auf schmalen Geräten: Der Text wird an **Wortgrenzen** in mehrere
Zeilen gebrochen, und **jede Zeile ist eine eigene Komposition** — ein
eigener `GET /write/word`, ein eigener durchgehender Federzug von ihrem
Anstrich bis zu ihrem Auslauf. „Zug um Zug“ gilt damit je ZEILE, nicht je
Text; der Zeilenanfang bekommt seinen richtigen Anstrich, weil
`core/shaping.py` die Wortposition ohnehin je Slot vergibt. Alle Zeilen
teilen sich **eine** x-Höhe (die der breitesten), wie eine Hand über einen
Absatz hinweg; die Schrift läuft links bündig, und die Feder hält am
Zeilenbruch einen längeren Takt als beim Absetzen (`LINE_BREAK_PAUSE_MS`).
Verworfen wurden dabei: Maßstab-Boden mit horizontal scrollender Fläche und
eine viewportgekoppelte Zeichenobergrenze mit Hinweis. Ehrlich benannte
Grenze: Ein einzelnes Wort, das breiter ist als der Rahmen, wird **nicht**
getrennt — es bleibt eine Zeile unter dem Tintenboden und zieht (als
breiteste Zeile) den ganzen Block mit hinunter. **Nicht zu verwechseln mit
der Unantastbaren Lineatur** des Übungsblatts: Dort ist ein Umbruch
