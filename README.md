...
**The approach — analysis-by-synthesis with a ductus prior.** The image supplies geometry and ink width; the *ductus* (stroke order and pen-lift points, known a priori for normed pre-1900 Kurrent) supplies the missing dimension. A canonical ductus template is fitted to the scan's skeleton and distance-transform profile. The library key is `(style, glyph, variant)` — `style` is the script family (Kurrent · Sütterlin · Offenbacher), and within it there is one authored form per glyph: long `ſ` and round `s` are separate glyphs with their own ductus, not one `s` with a context switch, while a `variant` is a structurally different form of the same glyph (the two `F` shapes of a teaching chart). A slot's word position is render context, assigned at composition time, not part of the key — and transitions fall out of `exit`/`entry` tangents, so there is no bigram explosion.
...
**Open-core.** The code is MIT — see [LICENSE](LICENSE), whose opening paragraph states that scope: MIT covers the code, `/data/` is licensed per source, and the learned dataset is reserved. The **learned data** this project produces — the curated glyph templates, the ductus, and the script statistics / trained reading models built in the admin — is **not** covered by MIT and stays reserved, even with the repo public; reuse by arrangement only. That reservation is an authentication gate on the API, not a robots rule: the website's pages are open to search, retrieval, citation and model training (`app/public/robots.txt`), every API read that carries the dataset needs the admin credential (`tests/test_api_public_surface.py` pins the split), and the composed `/write` renders — the one public surface derived from it — stay out of training (`api.kurrentschrift.ink/robots.txt`). The public-domain sources under `/data/` keep their own free license. If the methodology is useful to your work, see [CITATION.cff](CITATION.cff).
