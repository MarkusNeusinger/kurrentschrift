...
describe('seo coverage', () => {
  ...
  it('llms.txt links every public content route', () => {
    ...
    // Word-boundary match, not substring: a line linking a sibling file such
    // as /schriftkunde.md must not count as the link to the /schriftkunde
    // page. The literal parts are regex-escaped — the dots in the origin
    // would otherwise match any character and quietly weaken the guard.
  ...
  it('llms.txt announces the prerendered pages for clients without JavaScript', () => {
    // The crawler pages (app/prerender/, served by the API behind the nginx
    // bot map) must not ship unannounced — llms.txt is where the no-JS
    // agents they exist for look first, and it tells the unmapped ones where
    // the text lives instead.
    const txt = pub('llms.txt');
    expect(txt).toContain('vorgerenderte HTML-Fassung');
