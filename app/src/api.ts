...
export const getSource = (): Promise<SourceOut> => apiFetch(src('')).then(asJson<SourceOut>);
...
export const getBboxes = (): Promise<BboxOut[]> => apiFetch(src('/bboxes')).then(asJson<BboxOut[]>);
...
export const getGlyphs = (): Promise<GlyphSummary[]> => apiFetch(src('/glyphs')).then(asJson<GlyphSummary[]>);
