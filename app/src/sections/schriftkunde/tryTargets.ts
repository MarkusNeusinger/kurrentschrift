...
//
// Lives in its own module (not the view) so the Node-run Markdown mirror
// (lib/seo/schriftkundeMarkdown.ts) can read the same mapping without
// importing React. Relative imports WITH the .ts extension on purpose: the
// mirror is loaded by a plain Node script via type stripping, which knows
// neither the @/ alias nor extensionless resolution
// (`allowImportingTsExtensions` covers this in tsconfig).
