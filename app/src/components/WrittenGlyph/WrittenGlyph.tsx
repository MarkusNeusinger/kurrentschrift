...
export function WrittenGlyph({ glyphKey, sourceId = CONFIG.sourceId, durationMs = GLYPH_WRITE_MS, height = 220, tight = false, maxWidth, cacheBust, data: dataProp, variant = 0, onUnavailable, surfaceBg = SURFACE_BG, inkColor, animate: animateProp = true, showReplay = true }: Props) {
  ...
  // rather than from the caller's constant, and the ↺ gets its ground out of it.
  ...
  // Latest-callback ref, so the fetch effect below does not restart whenever
  // the caller passes a fresh closure. Written in an effect rather than during
  // render (react-hooks/refs): a render-phase write is unsound under concurrent
  // rendering, and this ref is only ever read from the async continuation
  // below, which cannot run before the commit's effects have flushed.
  const onUnavailableRef = useRef(onUnavailable);
  useEffect(() => {
    onUnavailableRef.current = onUnavailable;
  });

  // Reset the transient per-glyph state DURING RENDER rather than in the effect
  // below — React's documented "adjusting state when a prop changes"
  // (react-hooks/set-state-in-effect). The guard mirrors the effect's inputs
  // exactly, and the reset now lands in the same render as the change, so an
  // instance reused for another glyph never paints one frame of the previous
  // one's error state or ink.
  // The separator is written as the ESCAPE `\0`, not as a literal NUL byte: as
  // raw bytes in the file they made git classify this source as binary, so no
  // diff of it has ever rendered — for a reviewer or for Copilot. The key's
  // runtime value is unchanged.
  const shownFor = `${sourceId}\0${glyphKey}\0${variant}\0${cacheBust ?? ''}`;
  const [shown, setShown] = useState<{ key: string; data: GlyphRenderData | null | undefined }>({
    key: shownFor,
    data: dataProp,
  });
  if (shown.key !== shownFor || shown.data !== dataProp) {
    setShown({ key: shownFor, data: dataProp });
    setError(null);
    setUnavailable(false);
    // Whatever the shared cache already holds for the new glyph; null means a
    // spinner while the fetch below runs.
    if (!dataProp) setFetched(peekRenderGlyph(sourceId, glyphKey, cacheBust, variant) ?? null);
  }
  ...
  // The cap is whichever is smaller: what the caller allows, and what the frame
  // actually offers. Sizing the box from the caller's constant alone left it
  // taller than the ink the browser then squeezed into the frame — the box was
  // a claim, not a measurement (audit 2026-09-02, finding 28).
  const maxW = Math.min(maxWidth ?? GLYPH_MAX_W, frameW > 0 ? frameW : Infinity);
  ...
  // Ground for the ↺ so it does not sit inside the letter: in the quiz a narrow
  ...
  return (
    <Box
      ref={setFrameEl}
      sx={{ position: 'relative', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', ...ground }}
    >
      <svg
        width={displayW}
        height={finalH}
