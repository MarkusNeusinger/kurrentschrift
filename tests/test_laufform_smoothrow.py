...
thinner than the floor is not re-derived from too little evidence — it keeps
its stored row, so the card can never carry a row `PUT …/laufform` would refuse
with a 422. `--floor 1` is the explicit author statement and puts the old
behaviour back, which is what reproduces the LF11 card of `sep02`.
...
def test_a_key_under_the_floor_keeps_its_stored_row(tmp_path: Path):
    root = _root(tmp_path)
    ...
    assert rows["a"] == STORED, "a thin key must not be re-derived at all"
...
def test_a_key_without_fits_keeps_its_stored_row_and_says_so(tmp_path: Path):
    root = _root(tmp_path)
...
def test_an_unstored_key_under_the_floor_is_left_out_entirely(tmp_path: Path):
    ...
    assert any("skip a" in line for line in report)
