...
def test_per_letter_rates_count_a_letter_on_both_sides_of_a_join() -> None:
    ...
    assert rates["e"] == {"n": 2, "base": 1, "chain": 1}
    assert rates["d"] == {"n": 1, "base": 1, "chain": 1}
...
def _segment(kind: str, *, converged: bool, slot: int | None = None, key: str | None = None) -> ChainSegment:
    return ChainSegment(
        kind=kind,
        slot_index=slot,
        key=key,
...
def _chain_fit() -> ChainFit:
    return ChainFit(
        ...
        slot_a=0,
        segments=[
            _segment("letter", converged=True, slot=0, key="d"),
...
def test_fill_chain_segments_reads_the_contract_by_kind() -> None:
    ...
    assert row["chain_l_converged"] is True and row["chain_c_converged"] is False
    ...
    assert row["chain_converged"] is True
