...
async def test_trace_preview_returns_raw_and_refined_without_writing(api: Harness, synthetic_chart_path: str):
    ...
    res = await api.client.request(
        "POST",
        f"/sources/{source_id}/templates/n/trace-preview",
        json_body={"glyph": "n", "raw_path": _bar_raw_path(), "n_anchors": 24},
        headers=api.admin_headers(),
    )
    assert res.status == 200, res.body
...
async def test_resample_ladder_and_happy_path(api: Harness, synthetic_chart_path: str):
    ...
    res = await api.client.request(
        "POST", f"/sources/{bare_source}/templates/n/resample", json_body={}, headers=api.admin_headers()
    )
    assert res.status == 409

    # Bbox but no canonical → 404.
    await api.client.request(
        "PUT", f"/sources/{bare_source}/bboxes/n", json_body=_bbox_body(), headers=api.admin_headers()
    )
    res = await api.client.request(
        "POST", f"/sources/{bare_source}/templates/n/resample", json_body={}, headers=api.admin_headers()
    )
    ...
    await api.seed_template(style_id, bare_source, "n", "n")
    res = await api.client.request(
        "POST", f"/sources/{bare_source}/templates/n/resample", json_body={}, headers=api.admin_headers()
    )
    assert res.status == 409
    ...
    res = await api.client.request(
        "POST", f"/sources/{source_id}/templates/n/resample", json_body={}, headers=api.admin_headers()
    )
    assert res.status == 423
    res = await api.client.request(
        "POST", f"/sources/{source_id}/templates/n/resample", json_body={"force": True}, headers=api.admin_headers()
    )
    assert res.status == 200, res.body
    out = res.json()
...
async def test_diagnostic_404s_and_happy_path(api: Harness, synthetic_chart_path: str):
    _, source_id = await api.seed_style_and_source(chart_path=synthetic_chart_path)
    res = await api.client.request("GET", f"/sources/{source_id}/templates/n/diagnostic", headers=api.admin_headers())
    ...
    res = await api.client.request(
        "GET", f"/sources/{traced_source}/templates/n/diagnostic", headers=api.admin_headers()
    )
    assert res.status == 200, res.body
    out = res.json()
...
async def test_quality_409_without_pixel_meta_then_scores_after_trace(api: Harness, synthetic_chart_path: str):
    ...
    res = await api.client.request("GET", f"/sources/{source_id}/templates/n/quality", headers=api.admin_headers())
    assert res.status == 409

    traced_source = await _seed_traced_glyph(api, synthetic_chart_path)
    res = await api.client.request("GET", f"/sources/{traced_source}/templates/n/quality", headers=api.admin_headers())
    assert res.status == 200, res.body
    out = res.json()
...
async def test_fit_404_without_canonical(api: Harness, synthetic_chart_path: str):
    ...
    res = await api.client.request("GET", f"/sources/{source_id}/templates/n/fit", headers=api.admin_headers())
    assert res.status == 404
...
async def test_get_crop_raw_and_mask_render_png(api: Harness, synthetic_chart_path: str):
    _, source_id = await api.seed_style_and_source(chart_path=synthetic_chart_path)
    res = await api.client.request("GET", f"/sources/{source_id}/bboxes/n/crop")
    ...
    for view in ("raw", "mask"):
        res = await api.client.request("GET", f"/sources/{source_id}/bboxes/n/crop", params={"view": view})
...
async def test_write_single_glyph_payload_and_404(api: Harness, synthetic_chart_path: str):
    ...
    res = await api.client.request("GET", f"/sources/{source_id}/write/glyphs/x")
    assert res.status == 404
...
async def test_write_word_decomposes_missing_ligature_over_http(api: Harness):
    ...
    for key, glyph in (("B", "B"), ("u", "u"), ("c", "c"), ("h", "h")):
        await api.seed_template(style_id, source_id, key, glyph)
...
async def test_put_bbox_rejects_degenerate_geometry(api: Harness):
    ...
    for bad in (
        ...
        res = await api.client.request(
            "PUT", f"/sources/{source_id}/bboxes/n", json_body=bad, headers=api.admin_headers()
        )
