...
- **Archive snapshots: create freely, never destroy** — take one before anything that can overwrite geometry, never write into or remove an existing one, check row counts before filing, never print archive contents, and treat restoring as prod-touching (`tools/dbsnapshot`, author directive 2026-08-08; the runbook is `/dbsnapshot`).
- **Never echo secret values into the transcript** — verify by exit code or metadata.
- **Modify repo files only with the Edit/Write tools, never via Bash heredocs/sed — appending with `>>` counts.** A failed edit anchor means re-read and re-anchor, never a scripted rewrite (2026-08-14 and 2026-08-21 slips).
