...
- **Every PR adds a changelog fragment** — `changelog.d/<slug>.md` in the CHANGELOG's own format (`### Category` over bold-titled English bullets; `changelog.d/README.md`), NEVER a bullet in `CHANGELOG.md` itself: that shared spot is where every sibling merge used to conflict, and the CI job „Changelog (fragment)" refuses both a missing fragment and a bullet written into `[Unreleased]`. Same gate locally: `uv run python -m tools.changelog check --base origin/main`. Exempt: data-only commits (provenance lives in `SOURCE.md`), a PR labelled `skip-changelog`, and Dependabot's (skipped by author). Cutting a release — the fold, the tag, and the rule that a GitHub release is that section CONDENSED, never copied (author rule, 2026-08-28) — is `/release`.
