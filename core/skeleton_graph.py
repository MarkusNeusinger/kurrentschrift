"""The skeleton segment graph — pure geometry over a thinned ink mask.
...
everything here is a function of the ink mask alone. It lives in `core/`
because two consumers need it and only one of them ships `tools/`: the writing
-order recovery of `tools/routeg` (which re-exports this module unchanged) and
the Streifen-Befund of `core/eigenhand/befund.py`, whose readings the API
serves. Moved here in the Befund PR; the code is the routeg original, so a
number measured before the move is the same number after it.
"""
