...
def compose_word(
    ...
    *,
    ...
    def end_swing() -> None:
        ...
        # A deep exit (x's under-baseline loop) would run far past
        # SWING_MAX_RUN — the samples are TRUNCATED at the cap further down,
        # preserving the curve's steep start instead of flattening it.
        cot_out = d_out[0] / d_out[1]
        cot_end = 1.0 / math.tan(math.radians(SWING_END_DEG))
        run = rise * (cot_out + cot_end) / 2
        if run <= 0:
            return
        p3: Point = (p0[0] + run, p0[1] + rise)
